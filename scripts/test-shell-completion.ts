 ...
 * `powerdirector update`. Use it to verify the completion installation flow
...
\`powerdirector update\`, \`powerdirector doctor\`, and \`powerdirector onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# PowerDirector Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`powerdirector completion --install\` later to enable.`));
    return;
  }
