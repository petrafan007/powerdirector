...
async function main() {
  ...
  const sessionId =
    process.env.POWERDIRECTOR_ZAI_FALLBACK_SESSION_ID ??
  ...
  const run1 = await runCommand(
    "run1",
    ["powerdirector", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["powerdirector", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
