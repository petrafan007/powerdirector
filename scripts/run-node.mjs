...
const shouldBuild = (deps) => {
  if (deps.env.POWERDIRECTOR_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.POWERDIRECTOR_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[powerdirector] ${message}\n`);
};

const runPowerDirector = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["powerdirector.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runPowerDirector(deps);
  }
