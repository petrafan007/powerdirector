...
# Build and bundle PowerDirector into a minimal .app we can open.
# Outputs to dist/PowerDirector.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/PowerDirector.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="PowerDirector"
BUNDLE_ID="${BUNDLE_ID:-ai.powerdirector.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/powerdirector/powerdirector/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/PowerDirector/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :PowerDirectorBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :PowerDirectorGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/PowerDirector"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/PowerDirector"
fi
chmod +x "$APP_ROOT/Contents/MacOS/PowerDirector"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/PowerDirector" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/PowerDirector/Resources/PowerDirector.icns" "$APP_ROOT/Contents/Resources/PowerDirector.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/PowerDirector/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying PowerDirectorKit resources"
POWERDIRECTORKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/PowerDirectorKit_PowerDirectorKit.bundle"
if [ -d "$POWERDIRECTORKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/PowerDirectorKit_PowerDirectorKit.bundle"
  cp -R "$POWERDIRECTORKIT_BUNDLE" "$APP_ROOT/Contents/Resources/PowerDirectorKit_PowerDirectorKit.bundle"
else
  echo "WARN: PowerDirectorKit resource bundle not found at $POWERDIRECTORKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running PowerDirector"
killall -q PowerDirector 2>/dev/null || true
