...
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., powerdirector-alerts)
...
STATE_FILE="$HOME/.powerdirector/auth-monitor-state"
...
send_notification() {
    ...
    # Send via PowerDirector if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use powerdirector
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via PowerDirector to $NOTIFY_PHONE..."
            powerdirector send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: PowerDirector Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
