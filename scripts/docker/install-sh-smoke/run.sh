...
INSTALL_URL="${POWERDIRECTOR_INSTALL_URL:-https://powerdirector.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${POWERDIRECTOR_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${POWERDIRECTOR_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="powerdirector"
PACKAGE_NAME="${POWERDIRECTOR_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (POWERDIRECTOR_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${POWERDIRECTOR_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${POWERDIRECTOR_INSTALL_LATEST_OUT:-}"
fi
