...
ZIP=${1:?"Usage: $0 PowerDirector-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/powerdirector/powerdirector/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^PowerDirector-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/powerdirector/powerdirector/releases/download/v${VERSION}/"}
