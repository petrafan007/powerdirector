#!/usr/bin/env bash
# ClawDock - Docker helpers for PowerDirector
# Inspired by Simon Willison's "Running PowerDirector in Docker"
# https://til.simonwillison.net/llms/powerdirector-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/powerdirector/powerdirector/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for PowerDirector
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/powerdirector"
  "${HOME}/workspace/powerdirector"
  "${HOME}/projects/powerdirector"
  "${HOME}/dev/powerdirector"
  "${HOME}/code/powerdirector"
  "${HOME}/src/powerdirector"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found PowerDirector at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/powerdirector"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ PowerDirector not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/powerdirector/powerdirector.git ~/powerdirector"
    echo "  cd ~/powerdirector && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^POWERDIRECTOR_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d powerdirector-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart powerdirector-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f powerdirector-gateway
}
...
clawdock-workspace() {
  cd ~/.powerdirector/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec powerdirector-gateway \
    bash -c 'echo "alias powerdirector=\"./powerdirector.mjs\"" > /tmp/.bashrc_powerdirector && bash --rcfile /tmp/.bashrc_powerdirector'
}
...
clawdock-cli() {
  _clawdock_compose run --rm powerdirector-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build powerdirector-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "POWERDIRECTOR_GATEWAY_TOKEN=$token" powerdirector-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" powerdirector-gateway \
    bash -c './powerdirector.mjs config set gateway.remote.token "$TOKEN" && ./powerdirector.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec powerdirector-gateway \
    bash -c "./powerdirector.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart powerdirector-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm powerdirector-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec powerdirector-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'powerdirector config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for PowerDirector${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (powerdirector alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to powerdirector project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.powerdirector)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'powerdirector channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'powerdirector status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.powerdirector.ai${_CLR_RESET}"
  echo ""
}
