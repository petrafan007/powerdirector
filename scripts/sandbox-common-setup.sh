...
BASE_IMAGE="${BASE_IMAGE:-powerdirector-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-powerdirector-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=powerdirector.sandbox=1)
NOTE
