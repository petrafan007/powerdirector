...
IMAGE_NAME="powerdirector-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="powerdirector-net-e2e-$$"
GW_NAME="powerdirector-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "POWERDIRECTOR_GATEWAY_TOKEN=$TOKEN" \
	  -e "POWERDIRECTOR_SKIP_CHANNELS=1" \
	  -e "POWERDIRECTOR_SKIP_GMAIL_WATCHER=1" \
	  -e "POWERDIRECTOR_SKIP_CRON=1" \
	  -e "POWERDIRECTOR_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
