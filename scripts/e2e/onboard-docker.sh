...
IMAGE_NAME="powerdirector-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/powerdirector-bin:$PATH"
  mkdir -p /tmp/powerdirector-bin
  cat > /tmp/powerdirector-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/powerdirector-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$POWERDIRECTOR_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/powerdirector-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/powerdirector-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$POWERDIRECTOR_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/powerdirector-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    send "" 2.0
	  ...
	  run_case_local_basic() {
	    ...
	    export HOME="$home_dir"
	    mkdir -p "$HOME"
	    node "$POWERDIRECTOR_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$POWERDIRECTOR_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
