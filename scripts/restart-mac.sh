#!/usr/bin/env bash
# Reset PowerDirector like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${POWERDIRECTOR_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="PowerDirector.app/Contents/MacOS/PowerDirector"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/PowerDirector"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/PowerDirector"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/PowerDirector"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.powerdirector.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/powerdirector-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${POWERDIRECTOR_RESTART_LOG:-/tmp/powerdirector-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${POWERDIRECTOR_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.powerdirector/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  POWERDIRECTOR_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node powerdirector.mjs daemon install --force --runtime node"
      log "  node powerdirector.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.powerdirector/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.powerdirector.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing PowerDirector instances"
kill_all_powerdirector
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product PowerDirector"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/PowerDirector.app" ]]; then
    APP_BUNDLE="/Applications/PowerDirector.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/PowerDirector.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/PowerDirector.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/PowerDirector.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set POWERDIRECTOR_APP_BUNDLE to your installed PowerDirector.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node powerdirector.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node powerdirector.mjs daemon restart"
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: PowerDirector is running."
else
