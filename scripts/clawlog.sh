...
show_usage() {
    cat << EOF
clawlog - PowerDirector Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View PowerDirector logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    PowerDirector logs flow through the macOS unified log (subsystem: ai.powerdirector).
