...
# Checks both Claude Code and PowerDirector auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
POWERDIRECTOR_AUTH="$HOME/.powerdirector/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    powerdirector models status --json 2>/dev/null || true
}
...
check_powerdirector_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$POWERDIRECTOR_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    powerdirector_status=$(check_powerdirector_auth 2>/dev/null || true)

    claude_expires=0
    powerdirector_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        powerdirector_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        powerdirector_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$POWERDIRECTOR_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$powerdirector_status" \
        --arg be "$powerdirector_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            powerdirector: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$powerdirector_status" == EXPIRED* ]] || [[ "$powerdirector_status" == MISSING* ]]; then
        echo "POWERDIRECTOR_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$powerdirector_status" == EXPIRING* ]]; then
        echo "POWERDIRECTOR_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "PowerDirector Auth (~/.powerdirector/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$POWERDIRECTOR_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$POWERDIRECTOR_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'powerdirector doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active powerdirector >/dev/null 2>&1; then
    echo -e "PowerDirector service: ${GREEN}running${NC}"
else
    echo -e "PowerDirector service: ${RED}NOT running${NC}"
fi
