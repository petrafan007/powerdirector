...
# PowerDirector Auth Widget for Termux
...
STATUS=$(ssh "$SERVER" '$HOME/powerdirector/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/powerdirector/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|POWERDIRECTOR_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/powerdirector/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|POWERDIRECTOR_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: PowerDirector won't work until re-auth"
                ;;
        esac
        ;;
