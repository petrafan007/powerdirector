#!/bin/bash
# Setup PowerDirector Auth Management System
...
echo "=== PowerDirector Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'powerdirector-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  PowerDirector message: Send warning via PowerDirector itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/powerdirector-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/powerdirector-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/powerdirector-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now powerdirector-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status powerdirector-auth-monitor.timer"
echo ""
echo "Quick commands:"
