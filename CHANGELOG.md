...
### Changed / Fixed
- **[Hotfix 20] Doctor Command Crash**: Fixed a TypeError (`homedir is not a function`) in `powerdirector doctor` caused by inconsistent `node:os` default import behavior and thunk evaluation.
- **[Hotfix 20] Plugin SDK Resolution**: Fixed plugin loading failures for extensions using subpath imports (e.g. `powerdirector/plugin-sdk/core`) by correctly aliasing the SDK directory instead of a single file in the Jiti loader.
- **[Hotfix 19] Native Binary Support**: Resolved Webpack build failures in NextJS by excluding `@discordjs/voice` and other native binary packages from the UI bundle.
- **[Hotfix 19] Git Fetch Clobber**: Added the `--force` flag to git fetch commands in the update runner to prevent tag overwrite failures during hotfix repairs.
- **[Hotfix 21] Dirty Worktree Runtime Filtering**: Unified the git dirty-check logic used by the update status UI and the update runner so both paths share the same runtime-file filter set. Update checks now ignore repo-local runtime state and backup artifacts such as `.powerdirector/`, `.powerdirector-update-backups/`, `ui/.wwebjs_auth/`, runtime DB/env files, and config temp files, preventing false `dirty worktree` warnings from blocking legitimate upgrades.
- **[Hotfix 22] UI Native Package Externalization**: Hardened the Next.js server bundle config so native helper packages under `@napi-rs/canvas*` and `@snazzah/davey*` are always treated as server externals. This fixes clean-checkout `pnpm ui:build` failures where Webpack attempted to parse platform `.node` binaries during release QA.
- **[Hotfix 23] Config Section Registry Parity**: Restored the missing top-level config sections exposed by the Control UI (`terminal`, `commands`, `meta`, `diagnostics`, `audio`, `media`, `cron`, `talk`, `memory`, `plugins`) so per-section config APIs and schema generation match the actual PowerDirector config schema again. This eliminates false `400 Unknown section` errors across `/config/*` routes and repairs section-scoped reads/writes such as setup wizard terminal saves.
- **[Hotfix 23] UI Server `jiti` Externalization**: Marked `jiti` as a Next.js server external so the production UI server no longer bundles it into `.next` chunks. This fixes runtime `Cannot find module 'node:*'` failures triggered by plugin/runtime startup in clean QA builds.
- **[Hotfix 18] Dirty Worktree Checker**: Updated the update mechanism to ignore transient files in `ui/src-backend/`, `ui/next.config.ts`, and various lockfiles, preventing false "dirty worktree" reports during legitimate updates.
