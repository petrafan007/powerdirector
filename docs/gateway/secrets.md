...
PowerDirector supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `POWERDIRECTOR_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, PowerDirector runs preflight validation before saving:
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.powerdirector/secrets.json",
        mode: "json", // or "singleValue"
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/powerdirector-vault-resolver",
        args: ["--profile", "prod"],
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). PowerDirector validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/PowerDirector QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/powerdirector"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `powerdirector.json` refs)
...
When reload-time activation fails after a healthy state, PowerDirector enters degraded secrets state.
...
- Strict command paths (for example `powerdirector memory` remote-memory paths and `powerdirector qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `powerdirector status`, `powerdirector status --all`, `powerdirector channels status`, `powerdirector channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `powerdirector secrets reload`.
...
```bash
powerdirector secrets audit --check
powerdirector secrets configure
powerdirector secrets audit --check
```
...
- plaintext values at rest (`powerdirector.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `powerdirector.json` refs)
...
- lets you select supported secret-bearing fields in `powerdirector.json` plus `auth-profiles.json` for one agent scope
...
- `powerdirector secrets configure --providers-only`
- `powerdirector secrets configure --skip-provider-setup`
- `powerdirector secrets configure --agent <id>`
...
```bash
powerdirector secrets apply --from /tmp/powerdirector-secrets-plan.json
powerdirector secrets apply --from /tmp/powerdirector-secrets-plan.json --dry-run
```
...
PowerDirector intentionally does not write rollback backups containing historical plaintext secret values.
