...
- `--model` (optional): Specific Gemini image model to use. If not provided, it falls back to the `GEMINI_IMAGE_MODEL` environment variable, then to `gemini-2.5-flash-image`.

## Configuration

- `GEMINI_API_KEY`: (Required) Your Google AI API key.
- `GEMINI_IMAGE_MODEL`: (Optional) Default model to use for image generation (e.g., `gemini-2.5-flash-image`). PowerDirector automatically sets this from `agents.defaults.imageModel.primary`.
