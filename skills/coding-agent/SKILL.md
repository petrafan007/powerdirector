...
metadata:
  {
    "powerdirector": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in PowerDirector's own project folder!**
...
9. **NEVER checkout branches in ~/Projects/powerdirector/** - that's the LIVE PowerDirector instance!
...
For long-running background tasks, append a wake trigger to your prompt so PowerDirector gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
powerdirector system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: powerdirector system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
