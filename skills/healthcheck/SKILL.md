---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for PowerDirector deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, PowerDirector cron scheduling for periodic checks, or version status checks on a machine running PowerDirector (laptop, workstation, Pi, VPS).
---

# PowerDirector Host Hardening

## Overview

Assess and harden the host running PowerDirector, then align it to a user-defined risk tolerance without breaking access. Use PowerDirector security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim PowerDirector changes the host firewall, SSH, or OS updates; it does not.
...
5. PowerDirector gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if PowerDirector can access sensitive data.
...
### 2) Run PowerDirector security audits (read-only)

As part of the default read-only checks, run `powerdirector security audit --deep`. Only offer alternatives if the user requests them:

1. `powerdirector security audit` (faster, non-probing)
2. `powerdirector security audit --json` (structured output)

Offer to apply PowerDirector safe defaults (numbered):

1. `powerdirector security audit --fix`

Be explicit that `--fix` only tightens PowerDirector defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check PowerDirector version/update status (read-only)

As part of the default read-only checks, run `powerdirector update status`.
...
- Credential hygiene notes (location of PowerDirector creds, prefer disk encryption)
...
- PowerDirector security audit (re-run)
...
After PowerDirector install or first hardening pass, run at least one baseline audit and version check:

- `powerdirector security audit`
- `powerdirector security audit --deep`
- `powerdirector update status`

Ongoing monitoring is recommended. Use the PowerDirector cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `powerdirector cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `powerdirector update status`
...
Before creating, `powerdirector cron list` and match on exact `name`. If found, `powerdirector cron edit <id> ...`.
If not found, `powerdirector cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `powerdirector update status` (preferred for source checkouts and channels)
2. `npm view powerdirector version` (published npm version)

## PowerDirector command accuracy

Use only supported commands and flags:

- `powerdirector security audit [--deep] [--fix] [--json]`
- `powerdirector status` / `powerdirector status --deep`
- `powerdirector health --json`
- `powerdirector update status`
- `powerdirector cron add|list|runs|run`

Do not invent CLI flags or imply PowerDirector enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by PowerDirector compaction:
