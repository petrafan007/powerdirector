# PowerDirector ACP Bridge

This document describes how the PowerDirector ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`powerdirector acp` exposes an ACP agent over stdio and forwards prompts to a running
PowerDirector Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a PowerDirector Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `powerdirector acp` over stdio.

Example config:

```bash
powerdirector config set gateway.remote.url wss://gateway-host:18789
powerdirector config set gateway.remote.token <token>
```

Example run:

```bash
powerdirector acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
powerdirector acp --session agent:main:main
powerdirector acp --session agent:design:main
powerdirector acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “PowerDirector ACP” to start a thread.

## Execution Model

- ACP client spawns `powerdirector acp` and speaks ACP messages over stdio.
...
```bash
powerdirector acp --session agent:main:main
powerdirector acp --session-label "support inbox"
powerdirector acp --reset-session
```
...
`powerdirector acp` resolves the Gateway URL and auth from CLI flags or config:
