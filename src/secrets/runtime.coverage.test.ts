...
function buildConfigForPowerDirectorTarget(entry: SecretRegistryEntry, envId: string): PowerDirectorConfig {
  const config = {} as PowerDirectorConfig;
  const refTargetPath =
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every powerdirector.config.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "powerdirector.config.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `POWERDIRECTOR_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForPowerDirectorTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `POWERDIRECTOR_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
