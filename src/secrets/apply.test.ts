...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      POWERDIRECTOR_STATE_DIR: paths.stateDir,
      POWERDIRECTOR_CONFIG_PATH: paths.configPath,
