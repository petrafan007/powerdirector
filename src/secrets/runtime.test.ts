...
import { loadConfig, type PowerDirectorConfig, writeConfigFile } from "../config/config.js";
...
function createOpenAiFileModelsConfig(): NonNullable<PowerDirectorConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("resolves env refs for config and auth profiles", async () => {
    ...
    const snapshot = await prepareSecretsRuntimeSnapshot({
      ...
      agentDirs: ["/tmp/powerdirector-agent-main"],
      loadAuthStore: () =>
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: PowerDirectorConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    ...
    const store = ensureAuthProfileStore("/tmp/powerdirector-agent-main");
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("powerdirector-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("powerdirector-secrets-runtime-refresh-fail-", async (home) => {
      const configDir = path.join(home, ".powerdirector");
      const secretFile = path.join(configDir, "secrets.json");
      const agentDir = path.join(configDir, "agents", "main", "agent");
      const authStorePath = path.join(agentDir, "auth-profiles.json");
  ...
  it("keeps last-known-good web runtime snapshot when reload introduces unresolved active web refs", async () => {
    await withTempHome("powerdirector-secrets-runtime-web-reload-lkg-", async (home) => {
      const prepared = await prepareSecretsRuntimeSnapshot({
        config: asConfig({
          tools: {
            web: {
      ...
      const persistedConfig = JSON.parse(
        await fs.readFile(path.join(home, ".powerdirector", "powerdirector.json"), "utf8"),
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("powerdirector-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".powerdirector", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".powerdirector", "agents", "ops", "agent");
