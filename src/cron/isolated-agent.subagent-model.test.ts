...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "powerdirector-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".powerdirector", "sessions");
...
function makeCfg(
  ...
  const base: PowerDirectorConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<PowerDirectorConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<PowerDirectorConfig>,
