...
const { makeStorePath } = createCronStoreHarness({ prefix: "powerdirector-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses POWERDIRECTOR_HOME for tilde expansion", () => {
    vi.stubEnv("POWERDIRECTOR_HOME", "/srv/powerdirector-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/powerdirector-home", "cron", "jobs.json"));
  });
});
