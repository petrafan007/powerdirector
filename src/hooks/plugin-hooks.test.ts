...
describe("bundle plugin hooks", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.POWERDIRECTOR_BUNDLED_HOOKS_DIR;
    process.env.POWERDIRECTOR_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.POWERDIRECTOR_BUNDLED_HOOKS_DIR;
    } else {
      process.env.POWERDIRECTOR_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"powerdirector":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): PowerDirectorConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("powerdirector-plugin");
  ...
  it("does not treat Claude hooks.json bundles as PowerDirector hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".powerdirector", "extensions", "claude-bundle");
