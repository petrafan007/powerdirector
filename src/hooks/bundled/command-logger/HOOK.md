...
homepage: https://docs.powerdirector.ai/automation/hooks#command-logger
metadata:
  {
    "powerdirector":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.powerdirector/logs/commands.log`
...
`~/.powerdirector/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.powerdirector/logs/commands.log ~/.powerdirector/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/powerdirector`:
   ```
   /home/username/.powerdirector/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.powerdirector/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.powerdirector/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.powerdirector/logs/commands.log | jq .
```
