...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked POWERDIRECTOR_HOME.
  delete process.env.POWERDIRECTOR_HOME;
  process.env.POWERDIRECTOR_STATE_DIR = path.join(base, ".powerdirector");
...
export async function withTempHome<T>(
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".powerdirector", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
