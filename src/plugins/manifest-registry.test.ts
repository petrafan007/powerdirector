...
function makeTempDir() {
  return makeTrackedTempDir("powerdirector-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "powerdirector.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "powerdirector.plugin.json");
  const linkedManifest = path.join(rootDir, "powerdirector.plugin.json");
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        POWERDIRECTOR_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        POWERDIRECTOR_HOME: undefined,
        POWERDIRECTOR_STATE_DIR: path.join(homeB, ".state"),
      },
    });
