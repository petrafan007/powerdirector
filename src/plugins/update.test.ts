...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-powerdirector",
      targetDir: "/tmp/opik-powerdirector",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-powerdirector": {
              source: "npm",
              spec: "@opik/opik-powerdirector",
              integrity: "sha512-old",
              installPath: "/tmp/opik-powerdirector",
            },
          },
        },
      },
      pluginIds: ["opik-powerdirector"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-powerdirector": {
              source: "npm",
              spec: "@opik/opik-powerdirector@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-powerdirector",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @powerdirector/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @powerdirector/missing.",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@powerdirector/voice-call",
      targetDir: "/tmp/powerdirector-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@powerdirector/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@powerdirector/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@powerdirector/voice-call");
    expect(result.config.plugins?.entries?.["@powerdirector/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@powerdirector/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@powerdirector/voice-call",
      installPath: "/tmp/powerdirector-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/powerdirector-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
    ...
    try {
      const result = await syncPluginsForUpdateChannel({
        ...
        env: {
          ...process.env,
          POWERDIRECTOR_HOME: bundledHome,
