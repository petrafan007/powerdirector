...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "POWERDIRECTOR_HOME", "POWERDIRECTOR_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("powerdirector-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("powerdirector-bundle-mcp-workspace-");
      ...
      const config: PowerDirectorConfig = {
        plugins: {
          entries: {
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "POWERDIRECTOR_HOME", "POWERDIRECTOR_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("powerdirector-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("powerdirector-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.POWERDIRECTOR_HOME;
      delete process.env.POWERDIRECTOR_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".powerdirector", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".powerdirector", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "POWERDIRECTOR_HOME", "POWERDIRECTOR_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("powerdirector-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "powerdirector-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.POWERDIRECTOR_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".powerdirector", "extensions", "inline-claude");
