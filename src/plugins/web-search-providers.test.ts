...
const { loadPowerDirectorPluginsMock } = vi.hoisted(() => ({
  loadPowerDirectorPluginsMock: vi.fn((params?: { config?: { plugins?: Record<string, unknown> } }) => {
    ...
    const webSearchProviders = BUNDLED_WEB_SEARCH_PROVIDERS.filter((provider) => {
      ...
      provider: {
        ...
        applySelectionConfig:
          provider.id === "firecrawl" ? (config: PowerDirectorConfig) => config : undefined,
...
vi.mock("./loader.js", () => ({
  loadPowerDirectorPlugins: loadPowerDirectorPluginsMock,
}));
