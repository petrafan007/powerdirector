...
import { resolvePowerDirectorPackageRootSync } from "../infra/powerdirector-root.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolvePowerDirectorPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`powerdirector/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PowerDirectorPluginDefinition;
  register?: PowerDirectorPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PowerDirectorPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PowerDirectorPluginDefinition;
...
export function loadPowerDirectorPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getJiti()(safeSource) as PowerDirectorPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
