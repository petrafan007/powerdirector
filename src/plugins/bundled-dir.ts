...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolvePowerDirectorPackageRootSync({ cwd: process.cwd() }),
      resolvePowerDirectorPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
