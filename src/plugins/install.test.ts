...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      powerdirector: { extensions: ["./dist/index.js"] },
...
function setupDualFormatInstallFixture(params: { bundleFormat: "codex" | "claude" }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@powerdirector/native-dual",
      version: "0.0.1",
      powerdirector: { extensions: ["./dist/index.js"] },
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@powerdirector/test-plugin",
      version: "0.0.1",
      powerdirector: { extensions: ["./dist/index.js"] },
      dependencies: { "left-pad": "1.3.0" },
    }),
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@powerdirector/cognee-powerdirector",
      version: "0.0.1",
      powerdirector: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "powerdirector.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.powerdirector/extensions and preserves scoped package ids", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@powerdirector/voice-call" });
  });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@powerdirector/zipper" });
  });
  ...
  it("rejects packages without powerdirector.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@powerdirector/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("powerdirector.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_POWERDIRECTOR_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when powerdirector.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@powerdirector/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing powerdirector.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_POWERDIRECTOR_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without powerdirector.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.powerdirector).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses powerdirector.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@powerdirector/cognee-powerdirector"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("preserves scoped package names when no plugin manifest id is present", async () => {
    ...
    expectInstalledWithPluginId(res, extensionsDir, "@powerdirector/test-plugin");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@powerdirector/voice-call@0.0.1",
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@powerdirector/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@powerdirector/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@powerdirector/voice-call@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@powerdirector/voice-call@0.0.2-beta.1",
      name: "@powerdirector/voice-call",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@powerdirector/voice-call@beta"');
    }
  });
  ...
  it("allows explicit prerelease npm tags", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@powerdirector/voice-call@beta",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@powerdirector/voice-call@0.0.2-beta.1");
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@powerdirector/voice-call@beta",
    });
    expect(packTmpDir).not.toBe("");
  });
});
