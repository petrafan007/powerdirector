...
describe("installPluginFromArchive", () => {
  it("installs into ~/.powerdirector/extensions and uses unscoped id", async () => {
  ...
  it("uses powerdirector.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-powerdirector"',
        ),
      ),
    ).toBe(true);
  });
