...
function createSinglePluginWithEmptySlotsConfig(): PowerDirectorConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): PowerDirectorConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-powerdirector-extensions/my-plugin",
      },
      extensionsDir,
    });
