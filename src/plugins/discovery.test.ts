...
import { clearPluginDiscoveryCache, discoverPowerDirectorPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("powerdirector-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverPowerDirectorPlugins>[0],
) {
  return discoverPowerDirectorPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      powerdirector: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverPowerDirectorPlugins", () => {
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".powerdirector", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverPowerDirectorPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@powerdirector/voice-call",
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@powerdirector/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@powerdirector/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@powerdirector/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@powerdirector/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("powerdirector");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@powerdirector/escape-pack",
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@powerdirector/pack",
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        powerdirector: { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverPowerDirectorPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        POWERDIRECTOR_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverPowerDirectorPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        POWERDIRECTOR_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverPowerDirectorPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        POWERDIRECTOR_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
