...
type PluginLoadConfig = NonNullable<Parameters<typeof loadPowerDirectorPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "powerdirector.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadPowerDirectorPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          powerdirector: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadPowerDirectorPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadPowerDirectorPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadPowerDirectorPlugins>[0], "cache" | "config">,
) {
  return loadPowerDirectorPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "powerdirector.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function createPluginSdkAliasFixture(params?: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "powerdirector", type: "module" }, null, 2),
    "utf-8",
  );
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".powerdirector", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ POWERDIRECTOR_STATE_DIR: stateDir, ...params.env }, () =>
    loadPowerDirectorPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadPowerDirectorPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadPowerDirectorPlugins>;
  loadSecond: () => ReturnType<typeof loadPowerDirectorPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadPowerDirectorPlugins>;
  loadVariant: () => ReturnType<typeof loadPowerDirectorPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const powerdirectorHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(powerdirectorHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    POWERDIRECTOR_HOME: powerdirectorHome,
    HOME: ignoredHome,
    POWERDIRECTOR_STATE_DIR: stateDir,
    CLAWDBOT_STATE_DIR: undefined,
    POWERDIRECTOR_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".powerdirector", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ POWERDIRECTOR_STATE_DIR: stateDir }, () =>
      loadPowerDirectorPlugins({
        workspaceDir,
...
describe("loadPowerDirectorPlugins", () => {
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadPowerDirectorPlugins>) => {
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    const registry = loadBundledMemoryPluginRegistry({
      packageMeta: {
        name: "@powerdirector/memory-core",
        version: "1.2.3",
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadPowerDirectorPlugins(options);
          const scoped = loadPowerDirectorPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadPowerDirectorPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadPowerDirectorPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadPowerDirectorPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadPowerDirectorPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadPowerDirectorPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadPowerDirectorPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadPowerDirectorPlugins({
              ...
              env: {
                ...process.env,
                POWERDIRECTOR_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadPowerDirectorPlugins({
              ...
              env: {
                ...process.env,
                POWERDIRECTOR_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadPowerDirectorPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                POWERDIRECTOR_HOME: undefined,
                POWERDIRECTOR_STATE_DIR: stateDir,
                POWERDIRECTOR_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadPowerDirectorPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(powerdirectorHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadPowerDirectorPlugins(options),
          loadVariant: () =>
            loadPowerDirectorPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadPowerDirectorPlugins({
      env: {
        ...
        HOME: homeDir,
        POWERDIRECTOR_HOME: undefined,
        POWERDIRECTOR_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers POWERDIRECTOR_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const powerdirectorHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "powerdirector-home-demo",
      dir: path.join(powerdirectorHome, "plugins", "powerdirector-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "powerdirector-home-demo", register() {} };`,
    });
    ...
    const registry = loadPowerDirectorPlugins({
      ...
      config: {
        plugins: {
          allow: ["powerdirector-home-demo"],
          entries: {
            "powerdirector-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/powerdirector-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "powerdirector-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadPowerDirectorPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadPowerDirectorPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "powerdirector.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadPowerDirectorPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@powerdirector/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@powerdirector/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@powerdirector/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@powerdirector/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "powerdirector.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "powerdirector.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ POWERDIRECTOR_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("warns about open allowlists for discoverable plugins once per plugin set", () => {
    ...
    for (const scenario of scenarios) {
      ...
      for (let index = 0; index < scenario.loads; index += 1) {
        loadPowerDirectorPlugins(options);
      }
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".powerdirector", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("powerdirector/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadPowerDirectorPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadPowerDirectorPlugins({
        cache: false,
  ...
  it.each([
    ...
    },
    {
      ...
      buildFixture: () => createPluginSdkAliasFixture(),
      modulePath: () => "/tmp/tsx-cache/powerdirector-loader.js",
      argv1: (root: string) => path.join(root, "powerdirector.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
