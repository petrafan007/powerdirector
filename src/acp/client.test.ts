...
const createTempDir = () => tempDirs.make("powerdirector-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets POWERDIRECTOR_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "powerdirector",
    });

    expect(env.POWERDIRECTOR_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("powerdirector");
  });

  it("overrides pre-existing POWERDIRECTOR_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      POWERDIRECTOR_SHELL: "wrong",
    });
    expect(env.POWERDIRECTOR_SHELL).toBe("acp-client");
  });
  ...
  it("preserves POWERDIRECTOR_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        POWERDIRECTOR_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["POWERDIRECTOR_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default PowerDirector bridge", () => {
    ...
    expect(env.POWERDIRECTOR_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.POWERDIRECTOR_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "powerdirector",
        serverArgs: ["acp"],
        defaultServerCommand: "powerdirector",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("POWERDIRECTOR_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "powerdirector", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "powerdirector", "dist", "entry.js");
    const shimPath = path.join(dir, "powerdirector.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\powerdirector\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/powerdirector-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/powerdirector-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/powerdirector-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
