...
type ConfiguredBinding = NonNullable<PowerDirectorConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: PowerDirectorConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/powerdirector" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
