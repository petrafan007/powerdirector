...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "powerdirector-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-lid-b-"));
...
describe("resolveHomeDir", () => {
  it("prefers POWERDIRECTOR_HOME over HOME", () => {
    vi.stubEnv("POWERDIRECTOR_HOME", "/srv/powerdirector-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/powerdirector-home"));
...
describe("shortenHomePath", () => {
  it("uses $POWERDIRECTOR_HOME prefix when POWERDIRECTOR_HOME is set", () => {
    vi.stubEnv("POWERDIRECTOR_HOME", "/srv/powerdirector-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $POWERDIRECTOR_HOME replacement when POWERDIRECTOR_HOME is set", () => {
    vi.stubEnv("POWERDIRECTOR_HOME", "/srv/powerdirector-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("prefers POWERDIRECTOR_HOME for tilde expansion", () => {
    vi.stubEnv("POWERDIRECTOR_HOME", "/srv/powerdirector-home");
    vi.stubEnv("HOME", "/home/other");
