...
function resolveMode(cfg: PowerDirectorConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: PowerDirectorConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("powerdirector configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("powerdirector config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("powerdirector setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
