...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: PowerDirectorConfig;
  let lastUpdatedConfig: PowerDirectorConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/powerdirector/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/powerdirector/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/powerdirector/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: PowerDirectorConfig) => PowerDirectorConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/powerdirector/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/powerdirector/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `powerdirector plugins list --json`.',
    );
  });
