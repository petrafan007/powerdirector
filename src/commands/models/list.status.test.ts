...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePowerDirectorAgentDir: vi.fn().mockReturnValue("/tmp/powerdirector-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/powerdirector-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePowerDirectorAgentDir: mocks.resolvePowerDirectorAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/powerdirector-agent");
    }
  }
}
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/powerdirector-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/powerdirector-agent-custom");
