...
export function requirePowerDirectorAgentDir(): string {
  const agentDir = process.env.POWERDIRECTOR_AGENT_DIR;
  if (!agentDir) {
    throw new Error("POWERDIRECTOR_AGENT_DIR not set");
  }
  return agentDir;
}
