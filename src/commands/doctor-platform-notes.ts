...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".powerdirector", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: PowerDirectorConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`POWERDIRECTOR_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["POWERDIRECTOR_GATEWAY_TOKEN", await getenv("POWERDIRECTOR_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["POWERDIRECTOR_GATEWAY_PASSWORD", await getenv("POWERDIRECTOR_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "POWERDIRECTOR_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use POWERDIRECTOR_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> POWERDIRECTOR_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.POWERDIRECTOR_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- POWERDIRECTOR_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/powerdirector-compile-cache",
    "  mkdir -p /var/tmp/powerdirector-compile-cache",
    "  export POWERDIRECTOR_NO_RESPAWN=1",
