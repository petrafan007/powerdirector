...
describe("doctor config flow safe bins", () => {
  ...
  it("warns when interpreter/custom safeBins entries are missing profiles in non-repair mode", async () => {
    ...
    expect(noteSpy).toHaveBeenCalledWith(
      expect.stringContaining("powerdirector doctor --fix"),
      "Doctor warnings",
    );
  });
  ...
  it("hints safeBinTrustedDirs when safeBins resolve outside default trusted dirs", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-doctor-safe-bins-"));
