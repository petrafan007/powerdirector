...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under POWERDIRECTOR_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("powerdirector-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.POWERDIRECTOR_AGENT_DIR = kidAgentDir;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to POWERDIRECTOR_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("powerdirector-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
