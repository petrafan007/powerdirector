...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: PowerDirectorConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: PowerDirectorConfig) => { changed: boolean; next: PowerDirectorConfig };
  defaultModel: string;
  overrideConfig: PowerDirectorConfig;
  alreadyDefaultConfig: PowerDirectorConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: PowerDirectorConfig) => config,
      applyDefaultConfig: (config: PowerDirectorConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: PowerDirectorConfig = { agents: { defaults: {} } };
