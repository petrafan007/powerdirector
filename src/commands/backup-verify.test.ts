...
describe("backupVerifyCommand", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("powerdirector-backup-verify-test-");
  });
    ...
    try {
      ...
      const manifest = {
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.powerdirector",
            archivePath: `${rootName}/payload/posix/tmp/.powerdirector`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-backup-traversal-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
  ...
  it("fails when archive paths contain backslashes", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-backup-backslash-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    const payloadPath = path.join(tempDir, "payload.txt");
    try {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-backup-duplicate-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    const payloadPath = path.join(tempDir, "payload.txt");
    try {
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.powerdirector",
            archivePath: `${rootName}/payload/posix/tmp/.powerdirector/payload.txt`,
          },
        ],
      };
      await fs.writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
      ...
      await tar.c(
        {
          ...
          onWriteEntry: (entry) => {
            ...
            if (entry.path === payloadPath) {
              entry.path = `${rootName}/payload/posix/tmp/.powerdirector/payload.txt`;
            }
          },
        },
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-backup-duplicate-payload-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    ...
    try {
      const rootName = "2026-03-09T00-00-00.000Z-powerdirector-backup";
      const payloadArchivePath = `${rootName}/payload/posix/tmp/.powerdirector/payload.txt`;
      const manifest = {
        schemaVersion: 1,
        createdAt: "2026-03-09T00:00:00.000Z",
        archiveRoot: rootName,
