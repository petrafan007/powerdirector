...
type EnvSnapshot = {
  HOME?: string;
  POWERDIRECTOR_HOME?: string;
  POWERDIRECTOR_STATE_DIR?: string;
  POWERDIRECTOR_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    POWERDIRECTOR_HOME: process.env.POWERDIRECTOR_HOME,
    POWERDIRECTOR_STATE_DIR: process.env.POWERDIRECTOR_STATE_DIR,
    POWERDIRECTOR_OAUTH_DIR: process.env.POWERDIRECTOR_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: PowerDirectorConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: PowerDirectorConfig) {
...
async function runStateIntegrityText(cfg: PowerDirectorConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.POWERDIRECTOR_OAUTH_DIR;
    fs.mkdirSync(process.env.POWERDIRECTOR_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when POWERDIRECTOR_OAUTH_DIR is explicitly configured", async () => {
    process.env.POWERDIRECTOR_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: PowerDirectorConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints powerdirector-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: PowerDirectorConfig = {};
    ...
    expect(text).toMatch(/powerdirector sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/powerdirector sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /powerdirector sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
