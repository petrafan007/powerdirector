...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.POWERDIRECTOR_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("powerdirector gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
