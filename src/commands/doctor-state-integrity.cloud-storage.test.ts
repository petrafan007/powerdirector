...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "PowerDirector", ".powerdirector");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/powerdirector-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".powerdirector");
  ...
  it("anchors cloud detection to OS homedir when POWERDIRECTOR_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".powerdirector");
    const originalPowerDirectorHome = process.env.POWERDIRECTOR_HOME;
    process.env.POWERDIRECTOR_HOME = "/tmp/powerdirector-home-override";
