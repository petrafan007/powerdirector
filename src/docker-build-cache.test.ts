...
describe("docker build cache layout", () => {
  ...
  it("uses pnpm cache mounts in Dockerfiles that install repo dependencies", async () => {
    for (const path of [
      ...
      expect(dockerfile, `${path} should use a shared pnpm store cache`).toContain(
        "--mount=type=cache,id=powerdirector-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked",
      );
    }
  });
  ...
  it("copies only install inputs before pnpm install in the e2e image", async () => {
    ...
    expect(
      dockerfile.indexOf(
        "COPY tsconfig.json tsconfig.plugin-sdk.dts.json tsdown.config.ts vitest.config.ts vitest.e2e.config.ts powerdirector.mjs ./",
      ),
