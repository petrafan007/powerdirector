...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("powerdirector-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __powerdirector?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__powerdirector?.kind).toBe("compaction");
    expect(marker.__powerdirector?.id).toBe("comp-1");
  ...
  test("reads cross-agent absolute sessionFile across store-root layouts", () => {
    ...
    for (const testCase of cases) {
      ...
      expect((out[0] as { __powerdirector?: { seq?: number } }).__powerdirector?.seq).toBe(1);
    }
  });
});
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("powerdirector-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  registerTempSessionStore("powerdirector-session-usage-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses POWERDIRECTOR_HOME instead of ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof os.homedir === "function") ? os.homedir : (() => ""))() : "") : "")", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/powerdirector-home"), ".powerdirector", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile for standard and custom store roots", () => {
    const cases = [
      {
        storePath: "/tmp/powerdirector/agents/main/sessions/sessions.json",
        sessionFile: "/tmp/powerdirector/agents/ops/sessions/sess-safe.jsonl",
      },
      {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/powerdirector/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/powerdirector/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("powerdirector-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });

  beforeAll(() => {
    vi.stubEnv("POWERDIRECTOR_HOME", tmpDir);
  });
