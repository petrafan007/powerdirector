...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("powerdirector doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with POWERDIRECTOR_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with POWERDIRECTOR_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.POWERDIRECTOR_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://powerdirector-gateway.ai:18789",
      onConnectError,
    });
