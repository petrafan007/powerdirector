...
let lastCreatePowerDirectorToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/powerdirector-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreatePowerDirectorToolsContext?.agentTo,
          agentThreadId: lastCreatePowerDirectorToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createPowerDirectorTools: (ctx: Record<string, unknown>) => {
      lastCreatePowerDirectorToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreatePowerDirectorToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreatePowerDirectorToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreatePowerDirectorToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-powerdirector-message-to": "channel:24514",
        "x-powerdirector-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
