...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.POWERDIRECTOR_STATE_DIR;
  previousConfigPath = process.env.POWERDIRECTOR_CONFIG_PATH;
  previousSkipBrowserControl = process.env.POWERDIRECTOR_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.POWERDIRECTOR_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.POWERDIRECTOR_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.POWERDIRECTOR_SKIP_CHANNELS;
  previousSkipProviders = process.env.POWERDIRECTOR_SKIP_PROVIDERS;
  previousSkipCron = process.env.POWERDIRECTOR_SKIP_CRON;
  previousMinimalGateway = process.env.POWERDIRECTOR_TEST_MINIMAL_GATEWAY;
...
function applyGatewaySkipEnv() {
  ...
  process.env.POWERDIRECTOR_SKIP_CANVAS_HOST = "1";
  process.env.POWERDIRECTOR_SKIP_CHANNELS = "1";
  process.env.POWERDIRECTOR_SKIP_PROVIDERS = "1";
  process.env.POWERDIRECTOR_SKIP_CRON = "1";
  process.env.POWERDIRECTOR_TEST_MINIMAL_GATEWAY = "1";
  process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "powerdirector-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "powerdirector-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".powerdirector-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.POWERDIRECTOR_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.POWERDIRECTOR_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.POWERDIRECTOR_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.POWERDIRECTOR_SKIP_GMAIL_WATCHER;
    } else {
      process.env.POWERDIRECTOR_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.POWERDIRECTOR_SKIP_CANVAS_HOST;
    } else {
      process.env.POWERDIRECTOR_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.POWERDIRECTOR_SKIP_CHANNELS;
    } else {
      process.env.POWERDIRECTOR_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.POWERDIRECTOR_SKIP_PROVIDERS;
    } else {
      process.env.POWERDIRECTOR_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.POWERDIRECTOR_SKIP_CRON;
    } else {
      process.env.POWERDIRECTOR_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.POWERDIRECTOR_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.POWERDIRECTOR_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["POWERDIRECTOR_GATEWAY_TOKEN"]);
  const prev = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = fallbackToken;
  }
