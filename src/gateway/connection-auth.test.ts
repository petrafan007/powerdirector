...
function cfg(input: Partial<PowerDirectorConfig>): PowerDirectorConfig {
  return input as PowerDirectorConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: PowerDirectorConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  POWERDIRECTOR_GATEWAY_TOKEN: "env-token",
  POWERDIRECTOR_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves config-first token SecretRef even when POWERDIRECTOR env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when POWERDIRECTOR env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
