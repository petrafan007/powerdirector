...
async function createSubagentRuntime(
  ...
  loadPowerDirectorPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadPowerDirectorPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(async () => {
  loadPowerDirectorPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadPowerDirectorPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.powerdirector.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
