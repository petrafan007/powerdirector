...
describe("ensureGatewayStartupAuth", () => {
  ...
  async function expectNoTokenGeneration(cfg: PowerDirectorConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): PowerDirectorConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
    ...
    await expectResolvedToken({
      cfg: {
        gateway: {
          auth: {
            mode: "token",
            token: "${POWERDIRECTOR_GATEWAY_TOKEN}",
          },
        },
      },
      env: {
        POWERDIRECTOR_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${POWERDIRECTOR_GATEWAY_TOKEN}",
    });
  });

  it("uses POWERDIRECTOR_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
  ...
  it("uses POWERDIRECTOR_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
      ...
      env: {
        POWERDIRECTOR_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
