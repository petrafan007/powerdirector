...
type SessionSendTool = ReturnType<typeof createPowerDirectorTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createPowerDirectorTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["POWERDIRECTOR_GATEWAY_PORT", "POWERDIRECTOR_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.POWERDIRECTOR_GATEWAY_PORT = String(gatewayPort);
  process.env.POWERDIRECTOR_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("POWERDIRECTOR_CONFIG_PATH missing in gateway test environment");
      }
