  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set POWERDIRECTOR_GATEWAY_TOKEN/POWERDIRECTOR_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
export function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.POWERDIRECTOR_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.POWERDIRECTOR_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.POWERDIRECTOR_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
