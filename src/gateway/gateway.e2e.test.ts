...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.POWERDIRECTOR_SKIP_BROWSER_CONTROL_SERVER,
      };
      ...
      const workspaceDir = path.join(tempHome, "powerdirector");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.powerdirector-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.POWERDIRECTOR_CONFIG_PATH = prev.configPath;
        process.env.POWERDIRECTOR_GATEWAY_TOKEN = prev.token;
        process.env.POWERDIRECTOR_SKIP_CHANNELS = prev.skipChannels;
        process.env.POWERDIRECTOR_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.POWERDIRECTOR_SKIP_CRON = prev.skipCron;
        process.env.POWERDIRECTOR_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.POWERDIRECTOR_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.POWERDIRECTOR_STATE_DIR = prev.stateDir;
      process.env.POWERDIRECTOR_CONFIG_PATH = prev.configPath;
      process.env.POWERDIRECTOR_GATEWAY_TOKEN = prev.token;
      process.env.POWERDIRECTOR_SKIP_CHANNELS = prev.skipChannels;
      process.env.POWERDIRECTOR_SKIP_GMAIL_WATCHER = prev.skipGmail;
