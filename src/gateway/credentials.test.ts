...
type GatewayConfig = NonNullable<PowerDirectorConfig["gateway"]>;
  ...
  it("uses config-first local token precedence inside gateway service runtime", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      ...
      env: {
        POWERDIRECTOR_GATEWAY_TOKEN: "env-token",
        POWERDIRECTOR_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
        POWERDIRECTOR_SERVICE_KIND: "gateway",
      } as NodeJS.ProcessEnv,
    });
    expect(resolved).toEqual({
      token: "config-token",
...
describe("resolveGatewayCredentialsFromValues", () => {
  ...
  it("rejects unresolved env var placeholders in config credentials", () => {
    const resolved = resolveGatewayCredentialsFromValues({
      configToken: "${POWERDIRECTOR_GATEWAY_TOKEN}",
      configPassword: "${POWERDIRECTOR_GATEWAY_PASSWORD}",
      env: {} as NodeJS.ProcessEnv,
      tokenPrecedence: "config-first",
      passwordPrecedence: "config-first", // pragma: allowlist secret
    });
