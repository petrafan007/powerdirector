    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "powerdirector", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-powerdirector-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-powerdirector-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "powerdirector", messages: [{ role: "user", content: "hi" }] },
          {
            "x-powerdirector-agent-id": "beta",
            "x-powerdirector-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from PowerDirector.");
      }
