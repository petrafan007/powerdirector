...
describe("gateway server chat", () => {
  ...
  test("sessions.send forwards dashboard messages into existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.steer forwards dashboard messages into existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-gw-"));
