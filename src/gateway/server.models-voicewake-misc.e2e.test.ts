...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.POWERDIRECTOR_STATE_DIR = path.join(homeDir, ".powerdirector");
  ...
  test(
    ...
    async () => {
      ...
      expect(initial.payload?.triggers).toEqual(["powerdirector", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".powerdirector", "settings", "voicewake.json"), "utf8"),
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    const setRes = await rpcReq<{ triggers: string[] }>(ws, "voicewake.set", {
      triggers: ["powerdirector", "computer"],
    });
    expect(setRes.ok).toBe(true);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["POWERDIRECTOR_CANVAS_HOST_PORT", "POWERDIRECTOR_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.POWERDIRECTOR_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    ...
    if (!configPath) {
      throw new Error("Missing POWERDIRECTOR_CONFIG_PATH");
    }
