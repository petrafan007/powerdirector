...
describe("handleControlUiHttpRequest", () => {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: `/powerdirector${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/powerdirector",
            root: { kind: "resolved", path: tmp },
            config: {
              agents: { defaults: { workspace: tmp } },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/powerdirector");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/powerdirector/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = handleControlUiHttpRequest(
          ...
          res,
          { basePath: "/powerdirector", root: { kind: "resolved", path: tmp } },
        );
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/powerdirector", "/powerdirector/", "/powerdirector/some-page"]) {
          const { handled, end } = runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,
  ...
  it("rejects absolute-path escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: `/powerdirector/${absolutePathUrl}`,
          method: "GET",
          rootPath: root,
          basePath: "/powerdirector",
        });
  ...
  it("rejects symlink escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: "/powerdirector/assets/leak.txt",
          method: "GET",
          rootPath: root,
          basePath: "/powerdirector",
        });
