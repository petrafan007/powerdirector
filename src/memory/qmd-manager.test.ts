...
describe("QmdMemoryManager", () => {
  ...
  let stateDir: string;
  let cfg: PowerDirectorConfig;
  const agentId = "main";

  async function createManager(params?: { mode?: "full" | "status"; cfg?: PowerDirectorConfig }) {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "qmd-manager-test-fixtures-"));
  });
  ...
  afterEach(() => {
    ...
    delete (globalThis as Record<string, unknown>).__powerdirectorMcporterDaemonStart;
    delete (globalThis as Record<string, unknown>).__powerdirectorMcporterColdStartWarned;
  });
