...
describe("usage-format", () => {
  const originalAgentDir = process.env.POWERDIRECTOR_AGENT_DIR;
  let agentDir: string;
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.POWERDIRECTOR_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over powerdirector config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          openai: {
  ...
  it("falls back to powerdirector config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
          anthropic: {
