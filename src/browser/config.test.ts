...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("powerdirector");
    expect(profile?.driver).toBe("powerdirector");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const powerdirector = resolveProfile(resolved, "powerdirector");
    expect(powerdirector?.driver).toBe("powerdirector");
    expect(powerdirector?.cdpPort).toBe(18800);
    expect(powerdirector?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from POWERDIRECTOR_GATEWAY_PORT when unset", () => {
    withEnv({ POWERDIRECTOR_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const powerdirector = resolveProfile(resolved, "powerdirector");
      expect(powerdirector?.cdpPort).toBe(19012);
      expect(powerdirector?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ POWERDIRECTOR_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const powerdirector = resolveProfile(resolved, "powerdirector");
      expect(powerdirector?.cdpPort).toBe(19022);
      expect(powerdirector?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(powerdirector?.cdpPort).toBe(19000);
    expect(powerdirector?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "powerdirector");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "powerdirector")!;
  ...
  describe("default profile preference", () => {
    it("defaults to powerdirector profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("powerdirector");
    });

    it("keeps powerdirector default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("powerdirector");
    ...
    it("keeps powerdirector default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("powerdirector");
    ...
    it("keeps powerdirector default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
