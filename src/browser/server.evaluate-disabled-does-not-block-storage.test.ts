  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          powerdirector: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.POWERDIRECTOR_GATEWAY_PORT;
    process.env.POWERDIRECTOR_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.POWERDIRECTOR_GATEWAY_PORT = prevGatewayPort;
    }
