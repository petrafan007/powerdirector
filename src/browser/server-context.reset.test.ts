...
function localPowerDirectorProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "powerdirector",
...
function createLocalPowerDirectorResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localPowerDirectorProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolvePowerDirectorUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    ...
    const ops = createLocalPowerDirectorResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolvePowerDirectorUserDataDir: () => profileDir,
    });
