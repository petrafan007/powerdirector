...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-powerdirector-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    try {
      ...
      expect(headers["x-powerdirector-relay-token"]).toBeTruthy();
      expect(headers["x-powerdirector-relay-token"]).not.toBe("test-gateway-token");
    } finally {
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!powerdirector) {
      throw new Error("expected powerdirector profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...powerdirector, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
