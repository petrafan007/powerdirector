...
import {
  probeAuthenticatedPowerDirectorRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedPowerDirectorRelay({
    baseUrl,
    relayAuthHeader: "x-powerdirector-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
  ...
  it("accepts authenticated powerdirector relay probe responses", async () => {
