...
const POWERDIRECTOR_EXTENSION_RELAY_BROWSER = "PowerDirector/extension-relay";
...
export async function isExtensionRelayCdpEndpoint(cdpUrl: string): Promise<boolean> {
  ...
  try {
    ...
    const isRelay = String(version?.Browser ?? "").trim() === POWERDIRECTOR_EXTENSION_RELAY_BROWSER;
