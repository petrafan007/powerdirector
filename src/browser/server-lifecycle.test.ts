...
const { stopPowerDirectorChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopPowerDirectorChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopPowerDirectorChrome: stopPowerDirectorChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "powerdirector", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopPowerDirectorChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["powerdirector", "chrome"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      powerdirector: vi.fn(async () => {}),
      chrome: vi.fn(async () => {
    ...
    expect(stopMap.powerdirector).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopPowerDirectorChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("powerdirector browser stop failed: Error: oops");
  });
});
