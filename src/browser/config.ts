...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "powerdirector" | "extension";
  attachOnly: boolean;
};
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_POWERDIRECTOR_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "powerdirector" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an PowerDirector browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for powerdirector/powerdirector CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PowerDirectorConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_POWERDIRECTOR_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME]
        ? DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "powerdirector";
