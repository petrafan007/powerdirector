...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "POWERDIRECTOR_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "POWERDIRECTOR_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.POWERDIRECTOR_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.POWERDIRECTOR_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-powerdirector-relay-token");
    expect(headers["x-powerdirector-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${sharedUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-powerdirector-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.POWERDIRECTOR_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("stops advertising websocket endpoint after reconnect grace expires", async () => {
    process.env.POWERDIRECTOR_EXTENSION_RELAY_RECONNECT_GRACE_MS = "120";
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(sharedUrl)["x-powerdirector-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${sharedUrl}/json/version`, {
      headers: { "x-powerdirector-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-powerdirector-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "PowerDirector/extension-relay" }));
        return;
      }
  ...
  it(
    ...
    async () => {
      process.env.POWERDIRECTOR_EXTENSION_RELAY_RECONNECT_GRACE_MS = "200";
  ...
  it(
    ...
    async () => {
      process.env.POWERDIRECTOR_EXTENSION_RELAY_RECONNECT_GRACE_MS = "2000";
  ...
  it("does not swallow EADDRINUSE when occupied port is not an powerdirector relay", async () => {
    const port = await getFreePort();
