...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredPowerDirectorTmpDir.mockReturnValue("/tmp/powerdirector");
  });
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".powerdirector-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredPowerDirectorTmpDir.mockReturnValue("/tmp/powerdirector-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "powerdirector-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "powerdirector-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredPowerDirectorTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "powerdirector-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "powerdirector-preferred", "downloads")}${path.sep}`),
    );
  });
