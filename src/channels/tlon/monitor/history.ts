...
export async function fetchChannelHistory(
  ...
  try {
    ...
    logger?.log(`[tlon] Fetching history: ${scryPath}`);
    ...
    logger?.log(`[tlon] Extracted ${messages.length} messages from history`);
    return messages;
  } catch (error) {
    logger?.log(
...
export async function getChannelHistory(
  ...
  if (cache.length >= count) {
    logger?.log(`[tlon] Using cached messages (${cache.length} available)`);
    return cache.slice(0, count);
  }

  logger?.log(`[tlon] Cache has ${cache.length} messages, need ${count}, fetching from scry...`);
  return await fetchChannelHistory(api, channelNest, count, logger);
}
