...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@powerdirector/msteams");
  ...
  it("includes external catalog entries", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@powerdirector/demo-channel",
            powerdirector: {
              channel: {
                id: "demo-channel",
                label: "Demo Channel",
              ...
              install: {
                npmSpec: "@powerdirector/demo-channel",
              },
            },
          },
        ],
  ...
  it("uses the provided env for external catalog path resolution", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@powerdirector/env-demo-channel",
            powerdirector: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@powerdirector/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        POWERDIRECTOR_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@powerdirector/default-env-demo",
            powerdirector: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@powerdirector/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "powerdirector" }),
      "utf8",
    );
    fs.writeFileSync(
      path.join(bundledDir, "package.json"),
      JSON.stringify({
        name: "@powerdirector/whatsapp",
        powerdirector: {
          extensions: ["./index.js"],
          channel: {
            id: "whatsapp",
          ...
          install: {
            npmSpec: "@powerdirector/whatsapp",
            defaultChoice: "npm",
          },
        },
      }),
    ...
    fs.writeFileSync(
      path.join(bundledDir, "powerdirector.plugin.json"),
    ...
    const entry = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        POWERDIRECTOR_BUNDLED_PLUGINS_DIR: path.join(packageRoot, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");

    expect(entry?.install.npmSpec).toBe("@powerdirector/whatsapp");
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: PowerDirectorConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
