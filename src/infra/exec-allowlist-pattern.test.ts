...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    ...
    process.env.POWERDIRECTOR_HOME = "/srv/powerdirector-home";
    process.env.HOME = "/home/other";
    const powerDirectorHome = path.join(path.resolve("/srv/powerdirector-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", powerDirectorHome)).toBe(true);
