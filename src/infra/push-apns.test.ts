...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-push-apns-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs registration store", () => {
  it("stores and reloads node APNs registration", async () => {
    ...
    expect(loaded?.topic).toBe("ai.powerdirector.ios");
...
describe("push APNs env config", () => {
  ...
  it("returns an error when required APNs auth vars are missing", async () => {
    ...
    expect(resolved.error).toContain("POWERDIRECTOR_APNS_TEAM_ID");
  });
});
