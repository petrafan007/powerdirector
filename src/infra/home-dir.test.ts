...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers POWERDIRECTOR_HOME over HOME and USERPROFILE",
      env: {
      ...
      homedir: () => "/fallback",
      expected: "/srv/powerdirector-home",
    },
    {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for POWERDIRECTOR_HOME", () => {
    const result = resolveRequiredHomeDir(
      { POWERDIRECTOR_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when POWERDIRECTOR_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ POWERDIRECTOR_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { POWERDIRECTOR_HOME: "/srv/powerdirector-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/powerdirector-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/powerdirector-home " },
      expected: "/srv/powerdirector-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/powerdirector-home/docs"));
  });
