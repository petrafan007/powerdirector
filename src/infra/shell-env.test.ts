...
describe("shell env fallback", () => {
  ...
  function expectSanitizedStartupEnv(receivedEnv: NodeJS.ProcessEnv | undefined) {
    ...
    expect(receivedEnv?.HOME).toBe(((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof os.homedir === "function") ? os.homedir : (() => ""))() : "") : ""));
  }
  ...
  it("is disabled by default", () => {
    ...
    expect(shouldEnableShellEnvFallback({ POWERDIRECTOR_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ POWERDIRECTOR_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  ...
  it("uses the same truthy env parsing for deferred fallback", () => {
    ...
    expect(shouldDeferShellEnvFallback({ POWERDIRECTOR_DEFER_SHELL_ENV_FALLBACK: "false" })).toBe(false);
    expect(shouldDeferShellEnvFallback({ POWERDIRECTOR_DEFER_SHELL_ENV_FALLBACK: "yes" })).toBe(true);
  });
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(resolveShellEnvFallbackTimeoutMs({ POWERDIRECTOR_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        POWERDIRECTOR_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
  });
