...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__powerdirector_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "powerdirector-root");
...
describe("resolvePowerDirectorPackageRoot", () => {
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "powerdirector");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "powerdirector.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "powerdirector" }));

    expect(resolvePowerDirectorPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolvePowerDirectorPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolvePowerDirectorPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-powerdirector package roots", async () => {
    const pkgRoot = fx("not-powerdirector");
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolvePowerDirectorPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
