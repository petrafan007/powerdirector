...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-push-apns-auth-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs auth and helper coverage", () => {
  ...
  it("prefers inline APNs private key values and unescapes newlines", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      POWERDIRECTOR_APNS_TEAM_ID: "TEAM123",
      POWERDIRECTOR_APNS_KEY_ID: "KEY123",
      POWERDIRECTOR_APNS_PRIVATE_KEY_P8:
        "-----BEGIN PRIVATE KEY-----\\nline-a\\nline-b\\n-----END PRIVATE KEY-----", // pragma: allowlist secret
      POWERDIRECTOR_APNS_PRIVATE_KEY: "ignored",
  ...
  it("falls back to POWERDIRECTOR_APNS_PRIVATE_KEY when POWERDIRECTOR_APNS_PRIVATE_KEY_P8 is blank", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      POWERDIRECTOR_APNS_TEAM_ID: "TEAM123",
      POWERDIRECTOR_APNS_KEY_ID: "KEY123",
      POWERDIRECTOR_APNS_PRIVATE_KEY_P8: "   ",
      POWERDIRECTOR_APNS_PRIVATE_KEY:
        "-----BEGIN PRIVATE KEY-----\\nline-c\\nline-d\\n-----END PRIVATE KEY-----", // pragma: allowlist secret
    } as NodeJS.ProcessEnv);
  ...
  it("reads APNs private keys from POWERDIRECTOR_APNS_PRIVATE_KEY_PATH", async () => {
    ...
    const resolved = await resolveApnsAuthConfigFromEnv({
      POWERDIRECTOR_APNS_TEAM_ID: "TEAM123",
      POWERDIRECTOR_APNS_KEY_ID: "KEY123",
      POWERDIRECTOR_APNS_PRIVATE_KEY_PATH: keyPath,
    } as NodeJS.ProcessEnv);
  ...
  it("reports missing auth fields and path read failures", async () => {
    ...
    await expect(resolveApnsAuthConfigFromEnv({} as NodeJS.ProcessEnv)).resolves.toEqual({
      ok: false,
      error: "APNs auth missing: set POWERDIRECTOR_APNS_TEAM_ID and POWERDIRECTOR_APNS_KEY_ID",
    });

    const missingKey = await resolveApnsAuthConfigFromEnv({
      POWERDIRECTOR_APNS_TEAM_ID: "TEAM123",
      POWERDIRECTOR_APNS_KEY_ID: "KEY123",
      POWERDIRECTOR_APNS_PRIVATE_KEY_PATH: missingPath,
    } as NodeJS.ProcessEnv);

    expect(missingKey.ok).toBe(false);
    if (!missingKey.ok) {
      expect(missingKey.error).toContain(
        `failed reading POWERDIRECTOR_APNS_PRIVATE_KEY_PATH (${missingPath})`,
      );
    }
  });
