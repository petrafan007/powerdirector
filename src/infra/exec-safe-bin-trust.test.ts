...
describe("exec safe bin trust", () => {
  ...
  it("does not trust PATH entries by default", () => {
    const injected = `/tmp/powerdirector-path-injected-${Date.now()}`;
  ...
  it("flags explicitly trusted dirs that are group/world writable", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-safe-bin-trust-"));
    try {
