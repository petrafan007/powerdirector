...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.POWERDIRECTOR_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("powerdirector-gw");
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.cliPath).toBe(
      "/opt/homebrew/bin/powerdirector",
    );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("powerdirector (PowerDirector)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("powerdirector");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("powerdirector.local");
