...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-powerdirector/current processes", () => {
...
describe.runIf(process.platform !== "win32")("cleanStaleGatewayProcessesSync", () => {
  it("kills stale gateway pids discovered on the gateway port", () => {
    spawnSyncMock.mockReturnValue({
      error: undefined,
      status: 0,
      stdout: ["p6001", "cpowerdirector", "p6002", "cpowerdirector-gateway"].join("\n"),
    });
  ...
  it("uses explicit port override when provided", () => {
    spawnSyncMock.mockReturnValue({
      error: undefined,
      status: 0,
      stdout: ["p7001", "cpowerdirector"].join("\n"),
    });
    const killSpy = vi.spyOn(process, "kill").mockImplementation(() => true);
