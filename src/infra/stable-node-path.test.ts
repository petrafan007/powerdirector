...
describe("resolveStableNodePath", () => {
  ...
  it("prefers the Homebrew opt symlink for default and versioned formulas", async () => {
    const prefix = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-stable-node-"));
