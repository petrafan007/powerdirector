  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "POWERDIRECTOR_WINDOWS_TASK_NAME",
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.powerdirector.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ POWERDIRECTOR_WINDOWS_TASK_NAME: "PowerDirector Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
          POWERDIRECTOR_SERVICE_MARKER: "powerdirector",
    ...
    expect(
      detectRespawnSupervisor(
        {
          POWERDIRECTOR_SERVICE_MARKER: "powerdirector",
          POWERDIRECTOR_SERVICE_KIND: "worker",
        },
        "win32",
      ),
    ).toBeNull();
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.powerdirector.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
