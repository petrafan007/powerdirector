...
function createPowerDirectorBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "powerdirector-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createPowerDirectorBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses powerdirector-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "powerdirector-gateway" },
          { pid: process.pid, cmd: "powerdirector-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'powerdirector'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple powerdirector pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "powerdirector-gateway" },
          { pid: pid2, cmd: "powerdirector-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-powerdirector pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks powerdirector processes — non-powerdirector listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty powerdirector stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty powerdirector stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + powerdirector pid in stdout — container-restricted lsof reports partial results
          return createPowerDirectorBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "powerdirector-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'powerdirector'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("powerdirector")
      // inside the mid-loop flush: a non-powerdirector cmd between two entries must not
      // be pushed, but the following powerdirector entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-powerdirector entry first, then powerdirector entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid powerdirector entry must still be found.
      const stdout = `p0\ncpowerdirector-gateway\np${stalePid}\ncpowerdirector-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero powerdirector pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-powerdirector stdout (line 145)", () => {
    it("treats status 1 + non-powerdirector stdout as port-free (not an powerdirector process)", () => {
      // status 1 + non-empty stdout where no powerdirector pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill powerdirector pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-powerdirector output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no powerdirector pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
