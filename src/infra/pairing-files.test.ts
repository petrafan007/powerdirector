...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/powerdirector-state", "devices")).toEqual({
      dir: path.join("/tmp/powerdirector-state", "devices"),
      pendingPath: path.join("/tmp/powerdirector-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/powerdirector-state", "devices", "paired.json"),
    });
  });
