...
vi.mock("./tmp-powerdirector-dir.js", () => ({
  resolvePreferredPowerDirectorTmpDir: () => resolvePreferredPowerDirectorTmpDirMock(),
}));
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ POWERDIRECTOR_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "PowerDirector Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "PowerDirector Gateway (work)" >nul 2>&1');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers POWERDIRECTOR_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    relaunchGatewayScheduledTask({
      POWERDIRECTOR_PROFILE: "work",
      POWERDIRECTOR_WINDOWS_TASK_NAME: "PowerDirector Gateway (custom)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "PowerDirector Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    ...
    relaunchGatewayScheduledTask({ POWERDIRECTOR_PROFILE: "work" });
