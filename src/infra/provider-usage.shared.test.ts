...
describe("provider-usage.shared", () => {
  ...
  it("reads legacy pi auth tokens for known provider aliases", async () => {
    const home = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-provider-usage-"));
