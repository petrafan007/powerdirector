...
function resolveShellExecEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  const home = ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof os.homedir === "function") ? os.homedir : (() => ""))() : "") : "").trim();
  if (home) {
    execEnv.HOME = home;
  } else {
    delete execEnv.HOME;
...
export function loadShellEnvFallback(opts: ShellEnvFallbackOptions): ShellEnvFallbackResult {
  ...
  if (!probe.ok) {
    logger.warn(`[powerdirector] shell env fallback failed: ${probe.error}`);
    lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env: NodeJS.ProcessEnv): boolean {
  return isTruthyEnvValue(env.POWERDIRECTOR_LOAD_SHELL_ENV);
}

export function shouldDeferShellEnvFallback(env: NodeJS.ProcessEnv): boolean {
  return isTruthyEnvValue(env.POWERDIRECTOR_DEFER_SHELL_ENV_FALLBACK);
}

export function resolveShellEnvFallbackTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.POWERDIRECTOR_SHELL_ENV_TIMEOUT_MS?.trim();
  if (!raw) {
    return DEFAULT_TIMEOUT_MS;
  }
