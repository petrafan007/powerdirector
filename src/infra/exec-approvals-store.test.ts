...
const originalPowerDirectorHome = process.env.POWERDIRECTOR_HOME;
...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalPowerDirectorHome === undefined) {
    delete process.env.POWERDIRECTOR_HOME;
  } else {
    process.env.POWERDIRECTOR_HOME = originalPowerDirectorHome;
  }
...
function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".powerdirector", "exec-approvals.json");
}
