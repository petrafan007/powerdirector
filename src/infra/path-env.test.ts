...
describe("ensurePowerDirectorCliOnPath", () => {
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/powerdirector-path/${name}`);
  ...
  function bootstrapPath(params: {
    ...
    ensurePowerDirectorCliOnPath(params);
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.POWERDIRECTOR_PATH_BOOTSTRAPPED;
      delete process.env.POWERDIRECTOR_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.POWERDIRECTOR_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
