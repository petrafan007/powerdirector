...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: PowerDirectorConfig | null = null;
  ...
  get(key: string, cfg: PowerDirectorConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: PowerDirectorConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: PowerDirectorConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: PowerDirectorConfig): void {
