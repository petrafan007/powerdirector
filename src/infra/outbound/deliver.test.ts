...
import { resolvePreferredPowerDirectorTmpDir } from "../tmp-powerdirector-dir.js";
...
const whatsappChunkConfig: PowerDirectorConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes PowerDirector tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredPowerDirectorTmpDir()]),
      }),
    );
  });

  it("includes PowerDirector tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes PowerDirector tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes PowerDirector tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: PowerDirectorConfig = { channels: { signal: { mediaMaxMb: 2 } } };
