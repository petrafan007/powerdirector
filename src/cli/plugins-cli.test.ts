...
const loadConfig = vi.fn<() => PowerDirectorConfig>(() => ({}) as PowerDirectorConfig);
const writeConfigFile = vi.fn<(config: PowerDirectorConfig) => Promise<void>>(async () => undefined);
const resolveStateDir = vi.fn(() => "/tmp/powerdirector-state");
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (config: PowerDirectorConfig) => writeConfigFile(config),
  };
});
...
describe("plugins cli", () => {
  ...
  beforeEach(() => {
    ...
    loadConfig.mockReturnValue({} as PowerDirectorConfig);
    writeConfigFile.mockResolvedValue(undefined);
    resolveStateDir.mockReturnValue("/tmp/powerdirector-state");
    resolveMarketplaceInstallShortcut.mockResolvedValue(null);
    installPluginFromMarketplace.mockResolvedValue({
      ok: false,
      error: "marketplace install failed",
    });
    enablePluginInConfig.mockImplementation((cfg: PowerDirectorConfig) => ({ config: cfg }));
    recordPluginInstall.mockImplementation((cfg: PowerDirectorConfig) => cfg);
    buildPluginStatusReport.mockReturnValue({
      plugins: [],
      diagnostics: [],
    });
    applyExclusiveSlotSelection.mockImplementation(({ config }: { config: PowerDirectorConfig }) => ({
      config,
      warnings: [],
    }));
  ...
  it("installs marketplace plugins and persists config", async () => {
    ...
    const installedCfg = {
      ...
      plugins: {
        ...
        installs: {
          alpha: {
            source: "marketplace",
            installPath: "/tmp/powerdirector-state/extensions/alpha",
          },
        },
      },
    } as PowerDirectorConfig;
    ...
    installPluginFromMarketplace.mockResolvedValue({
      ok: true,
      pluginId: "alpha",
      targetDir: "/tmp/powerdirector-state/extensions/alpha",
      version: "1.2.3",
    ...
    loadConfig.mockReturnValue({
      plugins: {
        ...
        installs: {
          alpha: {
            source: "path",
            sourcePath: "/tmp/powerdirector-state/extensions/alpha",
            installPath: "/tmp/powerdirector-state/extensions/alpha",
          },
        },
      },
    ...
    const cfg = {
      plugins: {
        installs: {
          alpha: {
            source: "npm",
            spec: "@powerdirector/alpha@1.0.0",
          },
        },
      },
    } as PowerDirectorConfig;
    const nextConfig = {
      plugins: {
        installs: {
          alpha: {
            source: "npm",
            spec: "@powerdirector/alpha@1.1.0",
          },
        },
      },
    } as PowerDirectorConfig;
