...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/powerdirector-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  it("reads gateway password from --password-file", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-gateway-run-"));
    try {
