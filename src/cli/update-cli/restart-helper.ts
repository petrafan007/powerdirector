...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.POWERDIRECTOR_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.POWERDIRECTOR_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.POWERDIRECTOR_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.POWERDIRECTOR_PROFILE);
}
...
function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  ...
  return resolveGatewayWindowsTaskName(env.POWERDIRECTOR_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  const tmpDir = ((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp");
...
`;
      ...
      const home = env.HOME?.trim() || process.env.HOME || ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function") ? ((typeof os.homedir === "function") ? os.homedir : (() => ""))() : "") : "");
...
`;
      ...
      filename = `powerdirector-restart-${timestamp}.bat`;
