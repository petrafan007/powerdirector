...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@powerdirector/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@powerdirector/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@powerdirector/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@powerdirector/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @powerdirector/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      mapNpmResolutionMetadata({
        name: "@powerdirector/plugin-alpha",
        version: "1.2.3",
        resolvedSpec: "@powerdirector/plugin-alpha@1.2.3",
      ...
      resolvedName: "@powerdirector/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@powerdirector/plugin-alpha@1.2.3",
        installPath: "/tmp/powerdirector/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@powerdirector/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @powerdirector/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@powerdirector/plugin-alpha@latest",
      true,
      "/tmp/powerdirector/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@powerdirector/plugin-alpha@latest",
      installPath: "/tmp/powerdirector/extensions/alpha",
      version: "1.2.3",
