...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    POWERDIRECTOR_STATE_DIR: "/tmp/powerdirector-daemon",
    POWERDIRECTOR_CONFIG_PATH: "/tmp/powerdirector-daemon/powerdirector.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.POWERDIRECTOR_STATE_DIR ?? "/tmp/powerdirector-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.POWERDIRECTOR_CONFIG_PATH ?? `${stateDir}/powerdirector.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/powerdirector-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.POWERDIRECTOR_STATE_DIR = "/tmp/powerdirector-cli";
    process.env.POWERDIRECTOR_CONFIG_PATH = "/tmp/powerdirector-cli/powerdirector.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.POWERDIRECTOR_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.POWERDIRECTOR_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.POWERDIRECTOR_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "powerdirector-gateway" }],
        hints: [],
      },
      healthy: false,
