...
const SANDBOX_EXAMPLES = {
  main: [
    ["powerdirector sandbox list", "List all sandbox containers."],
    ["powerdirector sandbox list --browser", "List only browser containers."],
    ["powerdirector sandbox recreate --all", "Recreate all containers."],
    ["powerdirector sandbox recreate --session main", "Recreate a specific session."],
    ["powerdirector sandbox recreate --agent mybot", "Recreate agent containers."],
    ["powerdirector sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["powerdirector sandbox list", "List all sandbox containers."],
    ["powerdirector sandbox list --browser", "List only browser containers."],
    ["powerdirector sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["powerdirector sandbox recreate --all", "Recreate all containers."],
    ["powerdirector sandbox recreate --session main", "Recreate a specific session."],
    ["powerdirector sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["powerdirector sandbox recreate --browser --all", "Recreate only browser containers."],
    ["powerdirector sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["powerdirector sandbox explain", "Show effective sandbox config."],
    ["powerdirector sandbox explain --session agent:main:main", "Explain a specific session."],
    ["powerdirector sandbox explain --agent work", "Explain an agent sandbox."],
    ["powerdirector sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.powerdirector.ai/cli/sandbox")}\n`,
    )
    .action(() => {
