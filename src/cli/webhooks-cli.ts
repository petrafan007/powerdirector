...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.powerdirector.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + PowerDirector hooks")
    ...
    .option("--hook-url <url>", "PowerDirector hook URL")
    .option("--hook-token <token>", "PowerDirector hook token")
