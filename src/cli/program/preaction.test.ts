...
beforeEach(() => {
  ...
  originalHideBanner = process.env.POWERDIRECTOR_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.POWERDIRECTOR_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.POWERDIRECTOR_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("powerdirector");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "powerdirector", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("powerdirector-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "powerdirector", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "powerdirector", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "powerdirector", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "powerdirector", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.POWERDIRECTOR_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "powerdirector", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "powerdirector", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "powerdirector", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "powerdirector", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "powerdirector", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "powerdirector", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "powerdirector", "backup", "create", "--json"],
    });
