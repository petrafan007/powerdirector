...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "powerdirector", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "powerdirector", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "powerdirector", "status"]);
  });
