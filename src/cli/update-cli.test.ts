...
describe("update-cli", () => {
  ...
  const baseConfig = {} as PowerDirectorConfig;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolvePowerDirectorPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("powerdirector-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("powerdirector-updated-root");
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolvePowerDirectorPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("powerdirector");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/powerdirector-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "powerdirector-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("PowerDirector update status");
        },
      },
      {
        name: "json output",
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as PowerDirectorConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "powerdirector@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("powerdirector-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "powerdirector@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:powerdirector/powerdirector#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("powerdirector-update"));
          await updateCommand({ yes: true, tag: "github:powerdirector/powerdirector#main" });
        },
        expectedSpec: "github:powerdirector/powerdirector#main",
      },
      {
        name: "POWERDIRECTOR_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("powerdirector-update"));
          await withEnvAsync(
            { POWERDIRECTOR_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/powerdirector-next.tgz" },
            async () => {
              await updateCommand({ yes: true, tag: "latest" });
            },
          );
        },
        expectedSpec: "http://10.211.55.2:8138/powerdirector-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("powerdirector");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("powerdirector-update");
    const localAppData = createCaseDir("powerdirector-localappdata");
        ...
        await withEnvAsync(
          {
            ...
            POWERDIRECTOR_CONFIG_PATH: "./config/powerdirector.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            POWERDIRECTOR_STATE_DIR: path.resolve("./state"),
            POWERDIRECTOR_CONFIG_PATH: path.resolve("./config/powerdirector.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            POWERDIRECTOR_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ POWERDIRECTOR_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.POWERDIRECTOR_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("powerdirector-update-wizard");
    await withEnvAsync({ POWERDIRECTOR_GIT_DIR: tempDir }, async () => {
      setTty(true);
