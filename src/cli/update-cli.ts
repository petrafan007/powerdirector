...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["powerdirector update", "Update a source checkout (git)"],
        ...
        ["powerdirector update wizard", "Interactive update wizard"],
        ["powerdirector --update", "Shorthand for powerdirector update"],
      ] as const;
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.powerdirector.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["powerdirector update status", "Show channel + version status."],
          ["powerdirector update status --json", "JSON output."],
          ["powerdirector update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.powerdirector.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
