...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# PowerDirector Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(powerdirector completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# PowerDirector Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(powerdirector completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.powerdirector.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $POWERDIRECTOR_STATE_DIR/completions (no stdout)",
    )
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c powerdirector -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'powerdirector gateway' and not 'powerdirector other gateway', we need to check the sequence.
