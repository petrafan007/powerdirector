...
vi.mock("../infra/path-env.js", () => ({
  ensurePowerDirectorCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    await runCli(["node", "powerdirector", "status"]);

    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "powerdirector", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "powerdirector", "--help"]);
