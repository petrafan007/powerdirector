...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: PowerDirectorConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: PowerDirectorConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: PowerDirectorConfig) => string[];
  isConfigured?: (account: unknown, cfg: PowerDirectorConfig) => boolean;
  isEnabled?: (account: unknown, cfg: PowerDirectorConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ POWERDIRECTOR_STATE_DIR: sharedChannelSecurityStateDir }, () =>
      fn(sharedChannelSecurityStateDir),
    );
  };

  const runSharedExtensionsAudit = async (config: PowerDirectorConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "powerdirector.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        powerdirector: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
    ...
    const cases = [
              ...
              gateway: {
                ...
                auth: {
                  password: {
                    source: "env",
                    provider: "default",
                    id: "POWERDIRECTOR_GATEWAY_PASSWORD",
                  },
                },
              },
            },
      ...
      },
      {
        ...
        run: async () => {
            ...
            gateway: {
              ...
              auth: {
                token: {
                  source: "env",
                  provider: "default",
                  id: "POWERDIRECTOR_GATEWAY_TOKEN",
                },
              },
            },
            secrets: {
          ...
          const resolvedConfig: PowerDirectorConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/powerdirector-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "powerdirector-sbx-browser-old\npowerdirector-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "powerdirector-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "powerdirector-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("powerdirector-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("powerdirector-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "powerdirector-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "powerdirector-sbx-browser-exposed") {
                return {
  ...
  it("evaluates workspace skill path escape findings", async () => {
    ...
    for (const testCase of cases) {
      ...
      const configPath = path.join(fixture.stateDir, "powerdirector.json");
      await fs.writeFile(configPath, "{}\n", "utf-8");
      if (!isWindows) {
        await fs.chmod(configPath, 0o600);
      }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): PowerDirectorConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: PowerDirectorConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.powerdirector/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: PowerDirectorConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<PowerDirectorConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          POWERDIRECTOR_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies PowerDirectorConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies PowerDirectorConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies PowerDirectorConfig,
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: PowerDirectorConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
          ...
          runInstallMetadataAudit(
            {
              plugins: {
                installs: {
                  "voice-call": {
                    source: "npm",
                    spec: "@powerdirector/voice-call@1.2.3",
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@powerdirector/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@powerdirector/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@powerdirector/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
    ...
    const cases = [
      {
        ...
        cfg: {} satisfies PowerDirectorConfig,
        assert: (res: SecurityAuditReport) => {
          expect(res.findings).toEqual(
            expect.arrayContaining([
              expect.objectContaining({
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.POWERDIRECTOR_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.POWERDIRECTOR_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
