...
`,
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
    const prevPassword = process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.POWERDIRECTOR_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
      } else {
        process.env.POWERDIRECTOR_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("powerdirector-sbx-browser-old\npowerdirector-sbx-browser-missing-hash\n"),
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: PowerDirectorConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: PowerDirectorConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: PowerDirectorConfig = { session: { dmScope: "main" } };
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: PowerDirectorConfig = {
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "powerdirector.config.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
