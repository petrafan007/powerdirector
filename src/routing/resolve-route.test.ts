...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: PowerDirectorConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: PowerDirectorConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/powerdirector-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<PowerDirectorConfig["bindings"]>[number];
