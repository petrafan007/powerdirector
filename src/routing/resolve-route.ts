...
function listAgents(cfg: PowerDirectorConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: PowerDirectorConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<PowerDirectorConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: PowerDirectorConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: PowerDirectorConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: PowerDirectorConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<PowerDirectorConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  PowerDirectorConfig,
  {
    bindingsRef: PowerDirectorConfig["bindings"];
    agentsRef: PowerDirectorConfig["agents"];
    sessionRef: PowerDirectorConfig["session"];
...
function resolveRouteCacheForConfig(cfg: PowerDirectorConfig): Map<string, ResolvedAgentRoute> {
