...
function createOpenAiAudioCfg(extra?: Partial<PowerDirectorConfig>): PowerDirectorConfig {
  return {
    models: {
      providers: {
        openai: {
...
async function runAutoAudioCase(params: {
  transcribeAudio: (req: { model?: string }) => Promise<{ text: string; model: string }>;
  cfgExtra?: Partial<PowerDirectorConfig>;
}) {
  let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
  await withAudioFixture("powerdirector-auto-audio", async ({ ctx, media, cache }) => {
  ...
  it("uses mistral when only mistral key is configured", async () => {
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          await withAudioFixture("powerdirector-auto-audio-mistral", async ({ ctx, media, cache }) => {
