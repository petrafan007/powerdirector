...
const ensurePowerDirectorModelsJsonMock = vi.fn(async () => {});
...
describe("describeImageWithModel", () => {
  beforeEach(async () => {
    ...
    vi.doMock("../../agents/models-config.js", () => ({
      ensurePowerDirectorModelsJson: ensurePowerDirectorModelsJsonMock,
    }));
  ...
  it("routes minimax-portal image models through the MiniMax VLM endpoint", async () => {
    const result = await describeImageWithModel({
      cfg: {},
      agentDir: "/tmp/powerdirector-agent",
      provider: "minimax-portal",
      model: "MiniMax-VL-01",
    ...
    expect(ensurePowerDirectorModelsJsonMock).toHaveBeenCalled();
