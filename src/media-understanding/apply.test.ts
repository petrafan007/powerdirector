...
const TEMP_MEDIA_PREFIX = "powerdirector-media-";
...
function createMediaDisabledConfig(): PowerDirectorConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): PowerDirectorConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      POWERDIRECTOR_AGENT_DIR: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: PowerDirectorConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredPowerDirectorTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        POWERDIRECTOR_AGENT_DIR: isolatedAgentDir,
