...
function getTestConfigPath() {
  return path.join(process.env.HOME ?? "", ".powerdirector", "powerdirector.json");
}

async function writePowerDirectorConfig(config: Record<string, unknown>, pretty = false) {
...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".powerdirector", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  it("returns an unavailable approval message instead of a local /approve prompt when discord exec approvals are disabled", async () => {
    await writePowerDirectorConfig({
      channels: {
        discord: {
          enabled: true,
