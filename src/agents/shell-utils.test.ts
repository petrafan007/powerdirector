...
describe("resolvePowerShellPath", () => {
  ...
  it("prefers PowerShell 7 in ProgramFiles", () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-pfiles-"));
    tempDirs.push(base);
  ...
  it("prefers ProgramW6432 PowerShell 7 when ProgramFiles lacks pwsh", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-pfiles-"));
    const programW6432 = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-pw6432-"));
  ...
  it("finds pwsh on PATH when not in standard install locations", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-pfiles-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-bin-"));
  ...
  it("falls back to Windows PowerShell 5.1 path when pwsh is unavailable", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-pfiles-"));
    const sysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "powerdirector-sysroot-"));
