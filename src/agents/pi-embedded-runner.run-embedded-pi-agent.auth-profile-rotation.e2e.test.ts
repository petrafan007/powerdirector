...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensurePowerDirectorModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): PowerDirectorConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): PowerDirectorConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): PowerDirectorConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
