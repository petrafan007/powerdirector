...
metadata: ${JSON.stringify({ powerdirector: { install: installSpecs } })}
---
...
describe("skills-install fallback edge cases", () => {
  ...
  beforeAll(async () => {
    workspaceDir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-fallback-test-"));
