...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.POWERDIRECTOR_TEST_FAST;
...
describe("powerdirector-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.POWERDIRECTOR_TEST_FAST = fastModeEnv.previous;
  });
