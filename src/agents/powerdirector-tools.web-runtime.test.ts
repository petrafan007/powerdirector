...
function findTool(name: string, config: PowerDirectorConfig) {
  const allTools = createPowerDirectorTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: PowerDirectorConfig; env?: NodeJS.ProcessEnv }) {
...
describe("powerdirector tools runtime web metadata wiring", () => {
