...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("powerdirector-bundled-");
  const workspaceDir = await createTempDir("powerdirector-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: PowerDirectorConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.POWERDIRECTOR_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
