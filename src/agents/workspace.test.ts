...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses POWERDIRECTOR_HOME for default workspace resolution", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/powerdirector-home"), ".powerdirector", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".powerdirector", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("powerdirector-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("powerdirector-workspace-");
    await fs.mkdir(path.join(tempDir, ".powerdirector"), { recursive: true });
    await fs.writeFile(
