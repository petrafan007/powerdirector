...
describe("buildAgentSystemPrompt", () => {
  it("formats owner section for plain, hash, and missing owner lists", () => {
    const cases = typedCases<{
    ...
    }>([
      {
        ...
        params: {
          workspaceDir: "/tmp/powerdirector",
  ...
  it("omits extended sections in minimal prompt mode", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      docsPath: "/tmp/powerdirector/docs",
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## PowerDirector CLI Quick Reference");
    expect(prompt).toContain("powerdirector gateway restart");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List PowerDirector agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("PowerDirector docs: /tmp/powerdirector/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## PowerDirector Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/powerdirector. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`powerdirector ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for PowerDirector subagents");
