...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-pi-tools-"));
    ...
    try {
      ...
      const tools = createPowerDirectorCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: PowerDirectorConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<PowerDirectorConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<PowerDirectorConfig["agents"]>["list"]>[number]["tools"];
  }): PowerDirectorConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createPowerDirectorCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createPowerDirectorCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createPowerDirectorCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createPowerDirectorCodingTools({
      config: cfg,
    ...
    const bobTools = createPowerDirectorCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createPowerDirectorCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createPowerDirectorCodingTools({
      config: cfg,
