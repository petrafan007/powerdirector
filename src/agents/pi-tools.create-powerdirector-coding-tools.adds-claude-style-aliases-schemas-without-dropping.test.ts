...
describe("createPowerDirectorCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createPowerDirectorCodingTools({ senderIsOwner: true }).find(
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createPowerDirectorCodingTools({ messageProvider: "discord" });
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createPowerDirectorCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createPowerDirectorCodingTools({
      modelProvider: "xai",
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createPowerDirectorReadTool(
      baseRead as unknown as Parameters<typeof createPowerDirectorReadTool>[0],
    );
