...
let previousConfigPath = process.env.POWERDIRECTOR_CONFIG_PATH;
...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(() => {
    ...
    process.env.POWERDIRECTOR_CONFIG_PATH = configPathOverride;
  });
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".powerdirector", "attachments");
