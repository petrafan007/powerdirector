...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("powerdirector-ws-", async (workspaceDir) => {
      await withTempDir("powerdirector-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createPowerDirectorCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("powerdirector-ws-", async (workspaceDir) => {
      await withTempDir("powerdirector-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("powerdirector-ws-", async (workspaceDir) => {
      const cfg: PowerDirectorConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createPowerDirectorCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-powerdirector.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("powerdirector-sandbox-", async (sandboxDir) => {
      await withTempDir("powerdirector-workspace-", async (workspaceDir) => {
        ...
        const tools = createPowerDirectorCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
