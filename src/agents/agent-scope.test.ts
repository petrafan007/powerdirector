...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: PowerDirectorConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: PowerDirectorConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: PowerDirectorConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: PowerDirectorConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/powerdirector-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/powerdirector-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });
