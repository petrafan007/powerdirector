...
describe("resolvePowerDirectorAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolvePowerDirectorAgentDir();
      ...
      withEnv(
        {
          ...
          POWERDIRECTOR_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolvePowerDirectorAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when POWERDIRECTOR_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers POWERDIRECTOR_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          POWERDIRECTOR_STATE_DIR: undefined,
          POWERDIRECTOR_AGENT_DIR: primaryOverride,
