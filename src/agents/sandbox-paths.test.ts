...
async function withOutsideHardlinkInPowerDirectorTmp<T>(
  params: {
    powerDirectorTmpDir: string;
  ...
  const hardlinkPath = path.join(params.powerDirectorTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.powerDirectorTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.powerDirectorTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.powerDirectorTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const powerDirectorTmpDir = resolvePreferredPowerDirectorTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred PowerDirector tmp root",
      media: path.join(powerDirectorTmpDir, "image.png"),
      expected: path.join(powerDirectorTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred PowerDirector tmp root",
      media: pathToFileURL(path.join(powerDirectorTmpDir, "photo.png")).href,
      expected: path.join(powerDirectorTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred PowerDirector tmp root",
      media: path.join(powerDirectorTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(powerDirectorTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(powerDirectorTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked PowerDirector tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(powerDirectorTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(powerDirectorTmpDir, { recursive: true });
      const symlinkPath = path.join(powerDirectorTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked PowerDirector tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInPowerDirectorTmp(
      {
        powerDirectorTmpDir,
  ...
  it("rejects symlinked PowerDirector tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInPowerDirectorTmp(
