...
type CoreToolDefinition = {
  ...
  includeInPowerDirectorGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "web",
    profiles: [],
    includeInPowerDirectorGroup: true,
  },
  {
    id: "web_fetch",
    label: "web_fetch",
...
function buildCoreToolGroupMap() {
  ...
  const powerdirectorTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInPowerDirectorGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:powerdirector": powerdirectorTools,
