...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parsePowerDirectorManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyPowerDirectorManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): PowerDirectorSkillMetadata | undefined {
