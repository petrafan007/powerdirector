    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "powerdirector.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "powerdirector.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "powerdirector.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("powerdirector-");
  const acpxRoot = await tempDirs.make("powerdirector-acpx-plugin-");
  const helperRoot = await tempDirs.make("powerdirector-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("powerdirector-");
  const pluginRoot = await tempDirs.make("powerdirector-plugin-");
  const outsideDir = await tempDirs.make("powerdirector-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("powerdirector-");
    const pluginRoot = await tempDirs.make("powerdirector-claude-bundle-");
