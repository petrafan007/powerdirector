...
function makeFallbacksOnlyCfg(): PowerDirectorConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): PowerDirectorConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
// https://github.com/powerdirector/powerdirector/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal PowerDirector compatibility marker, not a provider API contract.
