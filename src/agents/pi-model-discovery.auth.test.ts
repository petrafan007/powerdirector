...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.POWERDIRECTOR_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.POWERDIRECTOR_AUTH_STORE_READONLY;
        } else {
          process.env.POWERDIRECTOR_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
