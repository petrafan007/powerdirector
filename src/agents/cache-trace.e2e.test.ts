...
describe("createCacheTrace", () => {
  ...
  it("honors diagnostics cache trace config and expands file paths", () => {
    ...
    const trace = createCacheTrace({
      cfg: {
        diagnostics: {
          cacheTrace: {
            enabled: true,
            filePath: "~/.powerdirector/logs/cache-trace.jsonl",
          },
        },
      },
      env: {},
    ...
    expect(trace?.filePath).toBe(resolveUserPath("~/.powerdirector/logs/cache-trace.jsonl"));
