...
describe("exec PATH login shell merge", () => {
  ...
  it("sets POWERDIRECTOR_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-powerdirector-shell", {
      command: 'printf "%s" "${POWERDIRECTOR_SHELL:-}"',
    });
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/powerdirector-ssl-keys.log";
    try {
      const { createExecTool } = await import("./bash-tools.exec.js");
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      ...
      expect(output).not.toContain("/tmp/powerdirector-ssl-keys.log");
    } finally {
