...
function restorePowerDirectorStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.POWERDIRECTOR_STATE_DIR;
    return;
  }
  process.env.POWERDIRECTOR_STATE_DIR = originalValue;
}

const originalStateDir = process.env.POWERDIRECTOR_STATE_DIR;

afterEach(() => {
  restorePowerDirectorStateDir(originalStateDir);
});
