...
function createConfig(): PowerDirectorConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/powerdirector",
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "powerdirector-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "powerdirector-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/powerdirector/powerdirector-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        scope: "session",
        workspaceAccess: "rw",
        workspaceRoot: "~/.powerdirector/sandboxes",
        docker: {
        ...
        browser: {
          enabled: false,
          image: "powerdirector-browser",
          containerPrefix: "powerdirector-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/powerdirector/powerdirector-ssh-agent-worker");
