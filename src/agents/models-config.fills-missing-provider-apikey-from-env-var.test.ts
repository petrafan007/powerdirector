...
async function runCustomProviderMergeTest(params: {
  ...
  await ensurePowerDirectorModelsJson({
    models: {
      mode: "merge",
      providers: {
...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): PowerDirectorConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensurePowerDirectorModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensurePowerDirectorModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
