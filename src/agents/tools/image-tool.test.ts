...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".powerdirector-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): PowerDirectorConfig {
  return {
    agents: {
      defaults: {
  ...
  it("allows workspace images via createPowerDirectorCodingTools default workspace root", async () => {
    await withTempWorkspacePng(async ({ imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const tools = createPowerDirectorCodingTools({ config: cfg, agentDir });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.powerdirector/media/inbound/photo.png",
      });
