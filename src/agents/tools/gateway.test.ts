...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    powerdirector: process.env.POWERDIRECTOR_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.powerdirector === undefined) {
      delete process.env.POWERDIRECTOR_GATEWAY_TOKEN;
    } else {
      process.env.POWERDIRECTOR_GATEWAY_TOKEN = envSnapshot.powerdirector;
    }
  ...
  it("uses POWERDIRECTOR_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = "local-env-token";
