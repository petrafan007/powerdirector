...
const defaultTools = createPowerDirectorCodingTools({ senderIsOwner: true });
...
describe("createPowerDirectorCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: PowerDirectorConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createPowerDirectorCodingTools({
    ...
    const anthropicTools = createPowerDirectorCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: PowerDirectorConfig = {
      tools: {
        exec: {
    ...
    const allowed = createPowerDirectorCodingTools({
    ...
    const denied = createPowerDirectorCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createPowerDirectorCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
