...
const fixtureSuite = createFixtureSuite("powerdirector-skills-suite-");
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("powerdirector-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".powerdirector", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("applySkillEnvOverrides", () => {
  it("sets and restores env vars", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"powerdirector":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
    });
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"powerdirector":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"powerdirector":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"powerdirector":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
