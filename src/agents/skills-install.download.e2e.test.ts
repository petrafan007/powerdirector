...
const originalPowerDirectorStateDir = process.env.POWERDIRECTOR_STATE_DIR;

afterEach(() => {
  if (originalPowerDirectorStateDir === undefined) {
    delete process.env.POWERDIRECTOR_STATE_DIR;
  } else {
    process.env.POWERDIRECTOR_STATE_DIR = originalPowerDirectorStateDir;
  }
});
