...
type EnvSnapshot = {
  powerdirector?: string;
  legacy?: string;
};

function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    powerdirector: process.env.POWERDIRECTOR_STATE_DIR,
    legacy: process.env.CLAWDBOT_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.POWERDIRECTOR_STATE_DIR).toBe(snapshot.powerdirector);
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips POWERDIRECTOR_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/powerdirector-state-dir-test");
    expect(process.env.POWERDIRECTOR_STATE_DIR).toBe("/tmp/powerdirector-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("powerdirector-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.POWERDIRECTOR_STATE_DIR).toBe(stateDir);
      expect(process.env.CLAWDBOT_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("powerdirector-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.POWERDIRECTOR_STATE_DIR = "/tmp/original-powerdirector";
