...
const mainSchemaHints = mapSensitivePaths(PowerDirectorSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.powerdirector/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/powerdirector/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/powerdirector/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/powerdirector/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/powerdirector/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = PowerDirectorSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "PowerDirectorConfig";
    const hints = mainSchemaHints;
