...
function createSourceConfig(): PowerDirectorConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): PowerDirectorConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("powerdirector-config-runtime-source-", async () => {
