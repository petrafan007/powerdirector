...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.POWERDIRECTOR_NIX_MODE === "1" || env.OPENCLAW_NIX_MODE === "1";
}
...
export function resolveConfigPath(
  ...
  const override = env.POWERDIRECTOR_CONFIG_PATH?.trim() || env.OPENCLAW_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override, env, homedir);
  }
  const stateOverride = env.POWERDIRECTOR_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim();
  const candidates = [
...
export function resolveOAuthDir(
  ...
  const override = env.POWERDIRECTOR_OAUTH_DIR?.trim() || env.OPENCLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
