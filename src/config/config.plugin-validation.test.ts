...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "powerdirector.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS: process.env.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS,
  };
  ...
  beforeAll(async () => {
    ...
    process.env.POWERDIRECTOR_STATE_DIR = path.join(suiteHome, ".powerdirector");
    process.env.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.POWERDIRECTOR_STATE_DIR === undefined) {
      delete process.env.POWERDIRECTOR_STATE_DIR;
    } else {
      process.env.POWERDIRECTOR_STATE_DIR = envSnapshot.POWERDIRECTOR_STATE_DIR;
    }
    if (envSnapshot.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
