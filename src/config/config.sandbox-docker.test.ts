...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("powerdirector-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "powerdirector-sandbox-browser-global" },
      agentBrowser: { network: "powerdirector-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("powerdirector-sandbox-browser-agent");
  });
