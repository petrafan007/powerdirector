...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): PowerDirectorConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: PowerDirectorConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveSessionStoreTargets", () => {
  it("resolves all configured agent stores", () => {
    const cfg: PowerDirectorConfig = {
      session: {
        store: "~/.powerdirector/agents/{agentId}/sessions/sessions.json",
      },
      agents: {
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".powerdirector/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".powerdirector/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        POWERDIRECTOR_STATE_DIR: envStateDir,
      };
      const cfg: PowerDirectorConfig = {};
