...
const workspaceHarness = createCommandWorkspaceHarness("powerdirector-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".powerdirector", "extensions", params.pluginId);
...
function buildCfg(): PowerDirectorConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("powerdirector-command-plugins-home-", async () => {
