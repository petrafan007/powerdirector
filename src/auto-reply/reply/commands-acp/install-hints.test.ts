...
function withAcpConfig(acp: PowerDirectorConfig["acp"]): PowerDirectorConfig {
  return { acp } as PowerDirectorConfig;
}
...
describe("ACP install hints", () => {
  it("prefers explicit runtime install command", () => {
    const cfg = withAcpConfig({
      runtime: { installCommand: "pnpm powerdirector plugins install acpx" },
    });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("pnpm powerdirector plugins install acpx");
  });
  ...
  it("uses local acpx extension path when present", () => {
    ...
    expect(hint).toContain("powerdirector plugins install ");
  ...
  it("falls back to npm install hint for acpx when local extension is absent", () => {
    ...
    const cfg = withAcpConfig({ backend: "acpx" });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("powerdirector plugins install acpx");
  });
