...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop powerdirector!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@powerdirector_bot", { botUsername: "powerdirector_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@powerdirector_bot", { botUsername: "powerdirector_bot" })).toBe(true);
