...
function createDiscordParams(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

function createTelegramTopicParams(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
...
function createTelegramDmParams(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
    OriginatingChannel: "telegram",
...
async function runDiscordAcpCommand(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  return handleAcpCommand(createTelegramTopicParams(commandBody, cfg), true);
}

async function runTelegramDmAcpCommand(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  return handleAcpCommand(createTelegramDmParams(commandBody, cfg), true);
}

function createFeishuDmParams(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "feishu",
    Surface: "feishu",
...
async function runFeishuDmAcpCommand(commandBody: string, cfg: PowerDirectorConfig = baseCfg) {
