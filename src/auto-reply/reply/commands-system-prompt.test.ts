...
const { createPowerDirectorCodingToolsMock } = vi.hoisted(() => ({
  createPowerDirectorCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createPowerDirectorCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createPowerDirectorCodingToolsMock).toHaveBeenCalledWith(
