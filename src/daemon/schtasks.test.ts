...
describe("schtasks runtime parsing", () => {
  it.each(["Ready", "Running"])("parses %s status", (status) => {
    const output = [
      "TaskName: \\PowerDirector Gateway",
      `Status: ${status}`,
...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when POWERDIRECTOR_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".powerdirector", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when POWERDIRECTOR_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", POWERDIRECTOR_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".powerdirector-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers POWERDIRECTOR_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        POWERDIRECTOR_PROFILE: "rescue",
        POWERDIRECTOR_STATE_DIR: "C:\\State\\powerdirector",
      },
      expected: path.join("C:\\State\\powerdirector", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", POWERDIRECTOR_PROFILE: "default" },
      expected: path.join("/home/test", ".powerdirector", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem PowerDirector Gateway",
          "cd /d C:\\Projects\\powerdirector",
          "set NODE_ENV=production",
          "set POWERDIRECTOR_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\powerdirector",
          environment: {
            NODE_ENV: "production",
            POWERDIRECTOR_PORT: "18789",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\powerdirector\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\powerdirector\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\PowerDirector Share\\node.exe" "\\\\fileserver\\PowerDirector Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\PowerDirector Share\\node.exe",
            "\\\\fileserver\\PowerDirector Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from POWERDIRECTOR_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ POWERDIRECTOR_STATE_DIR: path.join(tmpDir, "custom-state") }),
