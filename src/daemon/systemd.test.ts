...
const TEST_MANAGED_HOME = "/tmp/powerdirector-test-home";
const GATEWAY_SERVICE = "powerdirector-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/powerdirector gateway run", envFileLine]);
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads POWERDIRECTOR_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/powerdirector gateway run", "EnvironmentFile=%h/.powerdirector/.env"],
      { [`${TEST_SERVICE_HOME}/.powerdirector/.env`]: "POWERDIRECTOR_GATEWAY_TOKEN=env-file-token\n" },
    );
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.powerdirector/missing.env");
  });
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "powerdirector-gateway-work.service");
        cb(null, "", "");
      });
