...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.POWERDIRECTOR_GATEWAY_PORT).toBe("18789");
    expect(env.POWERDIRECTOR_GATEWAY_TOKEN).toBeUndefined();
    expect(env.POWERDIRECTOR_SERVICE_MARKER).toBe("powerdirector");
    expect(env.POWERDIRECTOR_SERVICE_KIND).toBe("gateway");
    expect(typeof env.POWERDIRECTOR_SERVICE_VERSION).toBe("string");
    expect(env.POWERDIRECTOR_SYSTEMD_UNIT).toBe("powerdirector-gateway.service");
    expect(env.POWERDIRECTOR_WINDOWS_TASK_NAME).toBe("PowerDirector Gateway");
    if (process.platform === "darwin") {
      expect(env.POWERDIRECTOR_LAUNCHD_LABEL).toBe("ai.powerdirector.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", POWERDIRECTOR_PROFILE: "work" },
      port: 18789,
    });
    expect(env.POWERDIRECTOR_SYSTEMD_UNIT).toBe("powerdirector-gateway-work.service");
    expect(env.POWERDIRECTOR_WINDOWS_TASK_NAME).toBe("PowerDirector Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.POWERDIRECTOR_LAUNCHD_LABEL).toBe("ai.powerdirector.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through POWERDIRECTOR_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", POWERDIRECTOR_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.POWERDIRECTOR_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to POWERDIRECTOR_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.POWERDIRECTOR_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers POWERDIRECTOR_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        POWERDIRECTOR_GATEWAY_TOKEN: "powerdirector-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.POWERDIRECTOR_GATEWAY_TOKEN).toBe("powerdirector-token");
  });

  it("omits POWERDIRECTOR_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".powerdirector"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", POWERDIRECTOR_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".powerdirector-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", POWERDIRECTOR_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".powerdirector"));
  });

  it("uses POWERDIRECTOR_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", POWERDIRECTOR_STATE_DIR: "/var/lib/powerdirector" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/powerdirector"));
  });

  it("expands ~ in POWERDIRECTOR_STATE_DIR", () => {
    const env = { HOME: "/Users/test", POWERDIRECTOR_STATE_DIR: "~/powerdirector-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/powerdirector-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { POWERDIRECTOR_STATE_DIR: "C:\\State\\powerdirector" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\powerdirector");
  });
});
