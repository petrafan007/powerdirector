...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `powerdirector-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["POWERDIRECTOR_GATEWAY_TOKEN", "POWERDIRECTOR_GATEWAY_PASSWORD"]);
  ...
  it.each([
    ...
    const result = await withEnvAsync({ POWERDIRECTOR_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-tui-file-secret-"));
