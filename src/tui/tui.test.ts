...
describe("resolveInitialTuiAgentId", () => {
  const cfg: PowerDirectorConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/powerdirector" },
        { id: "ops", workspace: "/tmp/powerdirector/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/powerdirector/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run powerdirector devices list");
    expect(state.pairingHint).toContain("powerdirector devices list");
  });
