...
POWERDIRECTOR_BIN="$FAKE_POWERDIRECTOR_BIN"
resolve_powerdirector_version`,
    ],
    {
      cwd: process.cwd(),
      encoding: "utf-8",
      env: {
        ...process.env,
        FAKE_POWERDIRECTOR_BIN: cliPath,
        POWERDIRECTOR_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("PowerDirector 2026.3.8 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("PowerDirector dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("PowerDirector dev's build");
    },
  );
