...
import Foundation
import Testing
@testable import PowerDirectorChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> PowerDirectorChatHistoryPayload
{
    PowerDirectorChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> PowerDirectorChatSessionEntry {
    PowerDirectorChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> PowerDirectorChatSessionEntry
{
    PowerDirectorChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> PowerDirectorChatModelChoice {
    PowerDirectorChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [PowerDirectorChatHistoryPayload],
    sessionsResponses: [PowerDirectorChatSessionsListResponse] = [],
    modelResponses: [[PowerDirectorChatModelChoice]] = [],
    ...
    -> (TestChatTransport, PowerDirectorChatViewModel)
{
...
private func sendUserMessage(_ vm: PowerDirectorChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
    ...
    seq: Int = 1)
{
    transport.emit(
        .agent(
            PowerDirectorAgentEventPayload(
                runId: runId,
                seq: seq,
                stream: "assistant",
...
private final class TestChatTransport: @unchecked Sendable, PowerDirectorChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PowerDirectorChatHistoryPayload]
    private let sessionsResponses: [PowerDirectorChatSessionsListResponse]
    private let modelResponses: [[PowerDirectorChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<PowerDirectorChatTransportEvent>
    private let continuation: AsyncStream<PowerDirectorChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<PowerDirectorChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PowerDirectorChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PowerDirectorChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [PowerDirectorChatAttachmentPayload]) async throws -> PowerDirectorChatSendResponse
    {
        ...
        return PowerDirectorChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PowerDirectorChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PowerDirectorChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PowerDirectorChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = PowerDirectorChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = PowerDirectorChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: PowerDirectorChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = PowerDirectorChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: PowerDirectorChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(PowerDirectorChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == PowerDirectorChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = PowerDirectorChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: PowerDirectorChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = PowerDirectorChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = PowerDirectorChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = PowerDirectorChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { PowerDirectorChatViewModel(sessionKey: "main", transport: transport) }
