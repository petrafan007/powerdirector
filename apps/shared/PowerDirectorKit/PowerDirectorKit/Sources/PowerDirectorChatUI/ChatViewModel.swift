...
private let chatUILogger = Logger(subsystem: "ai.powerdirector", category: "PowerDirectorChatUI")

@MainActor
@Observable
public final class PowerDirectorChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [PowerDirectorChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [PowerDirectorChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [PowerDirectorPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PowerDirectorChatPendingToolCall] = []
    public private(set) var sessions: [PowerDirectorChatSessionEntry] = []
    private let transport: any PowerDirectorChatTransport
    private var sessionDefaults: PowerDirectorChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: PowerDirectorChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any PowerDirectorChatTransport,
    ...
    public var sessionChoices: [PowerDirectorChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [PowerDirectorChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PowerDirectorPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PowerDirectorChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PowerDirectorChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: PowerDirectorChatMessage) -> PowerDirectorChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> PowerDirectorChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return PowerDirectorChatMessageContent(
                type: content.type,
                text: cleaned,
        ...
        return PowerDirectorChatMessage(
            id: message.id,
            role: message.role,
    ...
    private static func messageIdentityKey(for message: PowerDirectorChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [PowerDirectorChatMessage],
        incoming: [PowerDirectorChatMessage]) -> [PowerDirectorChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [PowerDirectorChatMessage]) -> [PowerDirectorChatMessage] {
        var result: [PowerDirectorChatMessage] = []
    ...
    private static func dedupeKey(for message: PowerDirectorChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PowerDirectorChatMessageContent] = [
            PowerDirectorChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> PowerDirectorChatAttachmentPayload in
            PowerDirectorChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PowerDirectorChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = PowerDirectorChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: PowerDirectorChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PowerDirectorChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PowerDirectorAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PowerDirectorChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            PowerDirectorPendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> PowerDirectorPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
