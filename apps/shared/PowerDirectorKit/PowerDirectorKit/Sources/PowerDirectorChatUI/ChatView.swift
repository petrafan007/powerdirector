...
@MainActor
public struct PowerDirectorChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    public init(
        viewModel: PowerDirectorChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                PowerDirectorChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [PowerDirectorChatMessage] {
        let base: [PowerDirectorChatMessage]
    ...
    private func mergeToolResults(in messages: [PowerDirectorChatMessage]) -> [PowerDirectorChatMessage] {
        ...
        for message in messages {
            ...
            let merged = PowerDirectorChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: PowerDirectorChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: PowerDirectorChatMessage) -> Bool {
    ...
    private func primaryText(in message: PowerDirectorChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: PowerDirectorChatMessage) -> Bool {
    ...
    private func toolCalls(in message: PowerDirectorChatMessage) -> [PowerDirectorChatMessageContent] {
    ...
    private func inlineToolResults(in message: PowerDirectorChatMessage) -> [PowerDirectorChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: PowerDirectorChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: PowerDirectorChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(PowerDirectorChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
