...
struct ChatMessageBubble: View {
    let message: PowerDirectorChatMessage
    let style: PowerDirectorChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? PowerDirectorChatTheme.userText : PowerDirectorChatTheme.assistantText
    ...
    private var inlineAttachments: [PowerDirectorChatMessageContent] {
    ...
    private var toolCalls: [PowerDirectorChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [PowerDirectorChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? PowerDirectorChatTheme.userBubble
        }
        if self.style == .onboarding {
            return PowerDirectorChatTheme.onboardingAssistantBubble
        }
        return PowerDirectorChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return PowerDirectorChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: PowerDirectorChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? PowerDirectorChatTheme.userText : PowerDirectorChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: PowerDirectorChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(PowerDirectorChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [PowerDirectorChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: PowerDirectorChatTheme.assistantText)
            }
        }
    }
}
