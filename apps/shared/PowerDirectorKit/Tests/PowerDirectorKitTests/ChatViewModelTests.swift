...
private func makeViewModel(
    ...
    sessionsResponses: [PowerDirectorChatSessionsListResponse] = []) async -> (TestChatTransport, PowerDirectorChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { PowerDirectorChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}
