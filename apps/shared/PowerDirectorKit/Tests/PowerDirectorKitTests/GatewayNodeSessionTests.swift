import Foundation
import Testing
@testable import PowerDirectorKit
...
struct GatewayNodeSessionTests {
    ...
    func normalizeCanvasHostUrlPreservesExplicitSecureCanvasPort() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "https://canvas.example.com:9443/__powerdirector__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com")!)

        #expect(normalized == "https://canvas.example.com:9443/__powerdirector__/cap/token")
    }

    @Test
    func normalizeCanvasHostUrlBackfillsGatewayHostForLoopbackCanvas() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "http://127.0.0.1:18789/__powerdirector__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com:7443")!)

        #expect(normalized == "https://gateway.example.com:7443/__powerdirector__/cap/token")
    }
    ...
    func emitsSyntheticSeqGapAfterReconnectSnapshot() async throws {
        ...
        let options = GatewayConnectOptions(
            ...
            commands: [],
            permissions: [:],
            clientId: "powerdirector-ios-test",
            clientMode: "ui",
