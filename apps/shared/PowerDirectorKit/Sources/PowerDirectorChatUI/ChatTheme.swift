...
enum PowerDirectorChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("PowerDirectorChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("PowerDirectorChatTheme.onboardingAssistantBubble"),
...
enum PowerDirectorPlatformImageFactory {
    static func image(_ image: PowerDirectorPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
