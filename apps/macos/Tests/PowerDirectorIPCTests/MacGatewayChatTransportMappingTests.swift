...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: PowerDirectorProtocol.AnyCodable(["ok": PowerDirectorProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: PowerDirectorProtocol.AnyCodable(["ok": PowerDirectorProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = PowerDirectorProtocol.AnyCodable([
            "runId": PowerDirectorProtocol.AnyCodable("run-1"),
            "sessionKey": PowerDirectorProtocol.AnyCodable("main"),
            "state": PowerDirectorProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: PowerDirectorProtocol.AnyCodable(["a": PowerDirectorProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
