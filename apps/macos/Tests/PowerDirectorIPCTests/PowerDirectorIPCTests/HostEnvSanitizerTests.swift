...
struct HostEnvSanitizerTests {
    @Test func `sanitize blocks shell trace variables`() {
        let env = HostEnvSanitizer.sanitize(overrides: [
            "SHELLOPTS": "xtrace",
            "PS4": "$(touch /tmp/pwned)",
            "POWERDIRECTOR_TEST": "1",
        ])
        #expect(env["SHELLOPTS"] == nil)
        #expect(env["PS4"] == nil)
        #expect(env["POWERDIRECTOR_TEST"] == "1")
    }
    ...
    @Test func `sanitize shell wrapper allows only explicit override keys`() {
        ...
        #expect(env["LANG"] == "C")
        #expect(env["LC_ALL"] == "C")
        #expect(env["POWERDIRECTOR_TOKEN"] == nil)
        #expect(env["PS4"] == nil)
    }

    @Test func `sanitize non shell wrapper keeps regular overrides`() {
        let env = HostEnvSanitizer.sanitize(overrides: ["POWERDIRECTOR_TOKEN": "secret"])
        #expect(env["POWERDIRECTOR_TOKEN"] == "secret")
    }
}
