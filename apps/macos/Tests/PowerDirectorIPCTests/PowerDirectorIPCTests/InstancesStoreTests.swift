...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: PowerDirectorProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PowerDirectorProtocol.AnyCodable] = [
            "presence": .init([PowerDirectorProtocol.AnyCodable(entry)]),
        ]
        let payload = PowerDirectorProtocol.AnyCodable(payloadMap)
