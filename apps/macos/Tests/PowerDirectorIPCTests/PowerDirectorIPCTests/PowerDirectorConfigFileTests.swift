...
@Suite(.serialized)
struct PowerDirectorConfigFileTests {
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["POWERDIRECTOR_CONFIG_PATH": override]) {
            #expect(PowerDirectorConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["POWERDIRECTOR_CONFIG_PATH": override]) {
            PowerDirectorConfigFile.saveDict([
                "gateway": [
                    "remote": [
            ...
            #expect(PowerDirectorConfigFile.remoteGatewayPort() == 19999)
            #expect(PowerDirectorConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(PowerDirectorConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(PowerDirectorConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["POWERDIRECTOR_CONFIG_PATH": override]) {
            ...
            PowerDirectorConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = PowerDirectorConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["POWERDIRECTOR_CONFIG_PATH": override]) {
            ...
            PowerDirectorConfigFile.clearRemoteGatewayUrl()
            let root = PowerDirectorConfigFile.loadDict()
    ...
    func `save dict appends config audit log`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            "POWERDIRECTOR_STATE_DIR": stateDir.path,
            "POWERDIRECTOR_CONFIG_PATH": configPath.path,
        ]) {
            PowerDirectorConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-powerdirector-config-file")
