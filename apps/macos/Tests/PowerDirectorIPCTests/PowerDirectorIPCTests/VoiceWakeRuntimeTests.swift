...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "powerdirector"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["powerdirector"]
        let text = "hey powerdirector"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey powerdirector do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["powerdirector"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "powerdirector", start: 0.2, duration: 0.1, range: transcript.range(of: "powerdirector")),
