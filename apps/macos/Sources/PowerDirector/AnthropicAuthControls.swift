...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: PowerDirectorOAuthStore.AnthropicOAuthStatus = PowerDirectorOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                Spacer()
                Button("Reveal") {
                    NSWorkspace.shared.activateFileViewerSelecting([PowerDirectorOAuthStore.oauthURL()])
                }
                .buttonStyle(.bordered)
                .disabled(!FileManager().fileExists(atPath: PowerDirectorOAuthStore.oauthURL().path))
            ...
            Text(PowerDirectorOAuthStore.oauthURL().path)
    ...
    private func refresh() {
        let imported = PowerDirectorOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = PowerDirectorOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try PowerDirectorOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. PowerDirector can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: PowerDirectorOAuthStore.AnthropicOAuthStatus,
