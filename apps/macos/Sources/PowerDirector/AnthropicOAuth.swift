...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (PowerDirector token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: PowerDirectorOAuthStore.AnthropicOAuthStatus = PowerDirectorOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.powerdirector", category: "anthropic-oauth")
...
enum PowerDirectorOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let powerdirectorOAuthDirEnv = "POWERDIRECTOR_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "PowerDirector OAuth token file not found"
            case .unreadableFile: "PowerDirector OAuth token file not readable"
            case .invalidJSON: "PowerDirector OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in PowerDirector OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "PowerDirector OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.powerdirectorOAuthDirEnv]?
