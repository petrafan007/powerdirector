...
enum PowerDirectorLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = PowerDirectorOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PowerDirectorFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PowerDirectorLogging.bootstrapIfNeeded()
        let label = PowerDirectorLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PowerDirectorOSLogHandler: AppLogLevelBackedHandler {
...
struct PowerDirectorFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PowerDirectorLogging.parseLabel(self.label)
