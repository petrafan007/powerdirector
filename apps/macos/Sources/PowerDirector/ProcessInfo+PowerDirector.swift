...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.powerdirector.mac`) even if the shipped
    ...
    {
        if environment["POWERDIRECTOR_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "powerdirector.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "powerdirector.nixMode") { return true }
