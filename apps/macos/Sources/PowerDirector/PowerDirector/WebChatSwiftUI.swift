...
private let webChatSwiftLogger = Logger(subsystem: "ai.powerdirector", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "powerdirector.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: PowerDirectorChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> PowerDirectorChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(PowerDirectorChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            PowerDirectorChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return PowerDirectorChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PowerDirectorChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PowerDirectorGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    PowerDirectorChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    PowerDirectorAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: PowerDirectorProtocol.ModelChoice) -> PowerDirectorChatModelChoice {
        PowerDirectorChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PowerDirectorChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PowerDirectorChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PowerDirectorChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: PowerDirectorChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "PowerDirector Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PowerDirectorChatView>) -> NSViewController
    {
