...
enum DebugActions {
    private static let verboseDefaultsKey = "powerdirector.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "powerdirector.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = PowerDirectorPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "PowerDirector", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = PowerDirectorPaths.configURL
        guard
