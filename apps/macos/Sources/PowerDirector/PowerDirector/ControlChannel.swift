...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PowerDirectorProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.powerdirector", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PowerDirectorKit.AnyCodable]()) {
                $0[$1.key] = PowerDirectorKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PowerDirectorProtocol.AnyCodable?) -> [String: PowerDirectorProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PowerDirectorProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PowerDirectorKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PowerDirectorProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("powerdirector.control.heartbeat")
    static let controlAgentEvent = Notification.Name("powerdirector.control.agent")
}
