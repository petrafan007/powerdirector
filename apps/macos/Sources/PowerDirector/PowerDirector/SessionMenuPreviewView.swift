...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.powerdirector", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PowerDirectorSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PowerDirectorSessionPreviewEntry,
    ...
    private static func cache(payload: PowerDirectorSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PowerDirectorChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PowerDirectorKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PowerDirectorKit.AnyCodable]) -> [PowerDirectorChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PowerDirectorChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PowerDirectorChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PowerDirectorChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PowerDirectorChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PowerDirectorChatMessage) -> String? {
