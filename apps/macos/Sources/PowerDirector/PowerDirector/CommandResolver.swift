...
enum CommandResolver {
    private static let projectRootDefaultsKey = "powerdirector.gatewayProjectRootPath"
    private static let helperName = "powerdirector"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let powerdirectorEntry = root.appendingPathComponent("powerdirector.mjs").path
        if FileManager().isReadableFile(atPath: powerdirectorEntry) { return powerdirectorEntry }
        let binEntry = root.appendingPathComponent("bin/powerdirector.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__POWERDIRECTOR_ERR__' >&2
        \(message)
        __POWERDIRECTOR_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/powerdirector")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let powerdirectorPaths = self.powerdirectorManagedPaths(home: home)
        if !powerdirectorPaths.isEmpty {
            extras.insert(contentsOf: powerdirectorPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + powerdirectorPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func powerdirectorManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".powerdirector"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func powerdirectorExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectPowerDirectorExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("powerdirector.mjs").path,
            root.appendingPathComponent("bin/powerdirector.js").path,
        ]
    ...
    static func hasAnyPowerDirectorInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.powerdirectorExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func powerdirectorNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let powerdirectorPath = self.projectPowerDirectorExecutable(projectRoot: root) {
            return [powerdirectorPath, subcommand] + extraArgs
        }
        if let powerdirectorPath = self.powerdirectorExecutable(searchPaths: searchPaths) {
            return [powerdirectorPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "powerdirector", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            powerdirector entrypoint missing (looked for dist/index.js or powerdirector.mjs); run pnpm build.
            """
    ...
    static func powerdirectorCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real powerdirector CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/powerdirector"
        ...
        \(cliSection)
        \(projectSection)
        if command -v powerdirector >/dev/null 2>&1; then
          CLI="$(command -v powerdirector)"
          powerdirector \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/powerdirector.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/powerdirector.mjs"
            node "$PRJ/powerdirector.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/powerdirector.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/powerdirector.js"
            node "$PRJ/bin/powerdirector.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent powerdirector"
          pnpm --silent powerdirector \(quotedArgs);
        else
          echo "powerdirector CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? PowerDirectorConfigFile.loadDict()
