...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["POWERDIRECTOR_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/powerdirector")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("powerdirector-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("powerdirector-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/powerdirector/ (rolling or stdout), or nil if none exist.
