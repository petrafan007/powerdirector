...
private const val NOTIFICATION_CHANNEL_BASE_ID = "powerdirector.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "PowerDirector Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "PowerDirector Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "PowerDirector Active")
      }
