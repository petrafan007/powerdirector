...
val androidStoreFile = providers.gradleProperty("POWERDIRECTOR_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("POWERDIRECTOR_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("POWERDIRECTOR_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("POWERDIRECTOR_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set POWERDIRECTOR_ANDROID_STORE_FILE, " +
            "POWERDIRECTOR_ANDROID_STORE_PASSWORD, POWERDIRECTOR_ANDROID_KEY_ALIAS, and " +
            "POWERDIRECTOR_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    namespace = "ai.powerdirector.app"
    compileSdk = 36
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/PowerDirectorKit/Sources/PowerDirectorKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.powerdirector.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "powerdirector-$versionName-$buildType.apk"
