...
```bash
pnpm powerdirector gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__powerdirector__/canvas/` (see `docs/platforms/android.md`).
...
```bash
powerdirector devices list
powerdirector devices approve <requestId>
```
...
Current PowerDirector Android implication:
...
2) Android app is connected to that gateway and `powerdirector nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device (do not run gateway with `POWERDIRECTOR_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__powerdirector__/`).
...
```bash
powerdirector devices list
powerdirector devices approve --latest
```
...
- `POWERDIRECTOR_ANDROID_GATEWAY_URL=ws://...` (default: from your local PowerDirector config)
- `POWERDIRECTOR_ANDROID_GATEWAY_TOKEN=...`
- `POWERDIRECTOR_ANDROID_GATEWAY_PASSWORD=...`
- `POWERDIRECTOR_ANDROID_NODE_ID=...` or `POWERDIRECTOR_ANDROID_NODE_NAME=...`
...
- `pairing required` before tests start:
  - approve pending device pairing (`powerdirector devices approve --latest`) and rerun.
