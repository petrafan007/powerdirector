...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__POWERDIRECTOR_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<PowerDirectorTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.powerdirector", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = PowerDirectorTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> PowerDirectorTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return PowerDirectorTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> PowerDirectorTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> PowerDirectorTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return PowerDirectorTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> PowerDirectorTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = PowerDirectorTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: PowerDirectorTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: PowerDirectorAgentEventPayload.self
            ) else {
                continue
            }
