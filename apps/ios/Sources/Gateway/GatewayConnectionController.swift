...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [PowerDirectorCapability.canvas.rawValue, PowerDirectorCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(PowerDirectorCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(PowerDirectorCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = PowerDirectorLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(PowerDirectorCapability.location.rawValue) }

        caps.append(PowerDirectorCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(PowerDirectorCapability.watch.rawValue)
        }
        caps.append(PowerDirectorCapability.photos.rawValue)
        caps.append(PowerDirectorCapability.contacts.rawValue)
        caps.append(PowerDirectorCapability.calendar.rawValue)
        caps.append(PowerDirectorCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(PowerDirectorCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            PowerDirectorScreenCommand.record.rawValue,
            PowerDirectorSystemCommand.notify.rawValue,
            PowerDirectorChatCommand.push.rawValue,
        ...
        if caps.contains(PowerDirectorCapability.camera.rawValue) {
        ...
        if caps.contains(PowerDirectorCapability.location.rawValue) {
            commands.append(PowerDirectorLocationCommand.get.rawValue)
        }
        if caps.contains(PowerDirectorCapability.device.rawValue) {
            commands.append(PowerDirectorDeviceCommand.status.rawValue)
            commands.append(PowerDirectorDeviceCommand.info.rawValue)
        }
        if caps.contains(PowerDirectorCapability.watch.rawValue) {
            commands.append(PowerDirectorWatchCommand.status.rawValue)
            commands.append(PowerDirectorWatchCommand.notify.rawValue)
        }
        if caps.contains(PowerDirectorCapability.photos.rawValue) {
            commands.append(PowerDirectorPhotosCommand.latest.rawValue)
        }
        if caps.contains(PowerDirectorCapability.contacts.rawValue) {
            commands.append(PowerDirectorContactsCommand.search.rawValue)
            commands.append(PowerDirectorContactsCommand.add.rawValue)
        }
        if caps.contains(PowerDirectorCapability.calendar.rawValue) {
            commands.append(PowerDirectorCalendarCommand.events.rawValue)
            commands.append(PowerDirectorCalendarCommand.add.rawValue)
        }
        if caps.contains(PowerDirectorCapability.reminders.rawValue) {
            commands.append(PowerDirectorRemindersCommand.list.rawValue)
            commands.append(PowerDirectorRemindersCommand.add.rawValue)
        }
        if caps.contains(PowerDirectorCapability.motion.rawValue) {
            commands.append(PowerDirectorMotionCommand.activity.rawValue)
            commands.append(PowerDirectorMotionCommand.pedometer.rawValue)
        }
