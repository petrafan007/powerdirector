...
struct IOSGatewayChatTransport: PowerDirectorChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.powerdirector", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> PowerDirectorChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(PowerDirectorChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> PowerDirectorChatHistoryPayload {
        ...
        return try JSONDecoder().decode(PowerDirectorChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [PowerDirectorChatAttachmentPayload]) async throws -> PowerDirectorChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [PowerDirectorChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(PowerDirectorChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(PowerDirectorGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PowerDirectorGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PowerDirectorChatEventPayload.self)
                        {
