...
@MainActor
final class PowerDirectorAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.powerdirector.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.powerdirector.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.powerdirector.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "powerdirector.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "powerdirector.watch.promptId"
    static let sessionKeyKey = "powerdirector.watch.sessionKey"
    static let actionPrimaryIDKey = "powerdirector.watch.action.primary.id"
    static let actionPrimaryLabelKey = "powerdirector.watch.action.primary.label"
    static let actionSecondaryIDKey = "powerdirector.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "powerdirector.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "powerdirector.watch.action.primary"
    static let actionSecondaryIdentifier = "powerdirector.watch.action.secondary"
    static let actionIdentifierPrefix = "powerdirector.watch.action."
    static let actionIDKeyPrefix = "powerdirector.watch.action.id."
    static let actionLabelKeyPrefix = "powerdirector.watch.action.label."
    static let categoryPrefix = "powerdirector.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: PowerDirectorWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> PowerDirectorWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return PowerDirectorWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "PowerDirector" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [PowerDirectorWatchAction]) -> [UNNotificationAction] {
...
@main
struct PowerDirectorApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(PowerDirectorAppDelegate.self) private var appDelegate
...
extension PowerDirectorApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("PowerDirector: installing uncaught exception handler")
