...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: PowerDirectorWatchNotifyParams) -> PowerDirectorWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [PowerDirectorWatchAction]?,
        kind: String?,
        promptId: String?) -> [PowerDirectorWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> PowerDirectorWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return PowerDirectorWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                PowerDirectorWatchAction(id: "approve", label: "Approve"),
                PowerDirectorWatchAction(id: "decline", label: "Decline", style: "destructive"),
                PowerDirectorWatchAction(id: "open_phone", label: "Open iPhone"),
                PowerDirectorWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            PowerDirectorWatchAction(id: "done", label: "Done"),
            PowerDirectorWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            PowerDirectorWatchAction(id: "open_phone", label: "Open iPhone"),
            PowerDirectorWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: PowerDirectorWatchRisk?,
        priority: PowerDirectorNotificationPriority?) -> PowerDirectorWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: PowerDirectorNotificationPriority?,
        risk: PowerDirectorWatchRisk?) -> PowerDirectorNotificationPriority?
    {
