...
final class MotionService: MotionServicing {
    func activities(params: PowerDirectorMotionActivityParams) async throws -> PowerDirectorMotionActivityPayload {
        ...
        let mapped: [PowerDirectorMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        PowerDirectorMotionActivityEntry(
        ...
        return PowerDirectorMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: PowerDirectorPedometerParams) async throws -> PowerDirectorPedometerPayload {
        ...
        let payload: PowerDirectorPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = PowerDirectorPedometerPayload(
