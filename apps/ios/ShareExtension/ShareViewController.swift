...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.powerdirector.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to PowerDirector", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to PowerDirector gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to PowerDirector.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "PowerDirectorShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "PowerDirector is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "PowerDirector Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("powerdirector-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
