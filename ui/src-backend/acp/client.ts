...
import { isKnownCoreToolId } from "../agents/tool-catalog";
...
export function resolveAcpClientSpawnEnv(
  ...
  env.POWERDIRECTOR_SHELL = "acp-client";
  return env;
}
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "powerdirector";
  ...
  const { getActiveSkillEnvKeys } = await import("../agents/skills/env-overrides.runtime");
  ...
  await client.initialize({
    ...
    clientInfo: { name: "powerdirector-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("PowerDirector ACP client");
