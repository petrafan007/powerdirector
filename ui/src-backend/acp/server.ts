...
import { normalizeAcpProvenanceMode, type AcpServerOptions } from "./types";
...
export async function serveAcpGateway(opts: AcpServerOptions = {}): Promise<void> {
  ...
  const gatewayUrlOverrideSource =
    connection.urlSource === "cli --url"
      ? "cli"
      : connection.urlSource === "env POWERDIRECTOR_GATEWAY_URL"
        ? "env"
        : undefined;
...
function printHelp(): void {
  console.log(`Usage: powerdirector acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or POWERDIRECTOR_GATEWAY_TOKEN.",
    );
  }
