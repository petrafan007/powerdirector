...
import { expandHomePrefix } from "../infra/home-dir";
...
import { getActivePluginRegistry } from "./runtime";
import type {
...
const APPROVALS_PATH = "~/.powerdirector/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "powerdirector-app-server:thread:",
  "powerdirector-codex-app-server:thread:",
] as const;
...
const pluginBindingGlobalStateKey = Symbol.for("powerdirector.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to PowerDirector instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
