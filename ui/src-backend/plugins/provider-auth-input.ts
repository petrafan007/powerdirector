...
import { encodeJsonPointerToken } from "../secrets/json-pointer";
import { PROVIDER_ENV_VARS } from "../secrets/provider-env-vars";
import {
...
import { resolveSecretRefString } from "../secrets/resolve";
    ...
    if (source === "env") {
      ...
      await params.prompter.note(
        params.copy?.envValidatedMessage?.(envVar) ??
          `Validated environment variable ${envVar}. PowerDirector will store a reference, not the key value.`,
        "Reference validated",
      );
    ...
    try {
      ...
      await params.prompter.note(
        params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ??
          `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. PowerDirector will store a reference, not the key value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    } catch (error) {
    ...
    options: [
      {
        ...
        hint: params.copy?.plaintextHint ?? "Stores the key directly in PowerDirector config",
      },
      {
        value: "ref",
