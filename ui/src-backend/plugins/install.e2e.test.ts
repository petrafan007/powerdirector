...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `powerdirector-plugin-install-${randomUUID()}`);
...
async function installFromDirWithWarnings(params: { pluginDir: string; extensionsDir: string }) {
  const { installPluginFromDir } = await import('./install');
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  const { installPluginFromArchive } = await import('./install');
...
describe("installPluginFromArchive", () => {
  ...
  it("rejects packages without powerdirector.extensions", async () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@powerdirector/nope", version: "0.0.1" }),
      "utf-8",
    );
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const { installPluginFromNpmSpec } = await import('./install');
