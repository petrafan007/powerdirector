...
import { resolvePluginCacheInputs, resolvePluginSourceRoots } from "./roots";
import type { PluginBundleFormat, PluginDiagnostic, PluginFormat, PluginOrigin } from "./types";
...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.POWERDIRECTOR_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.POWERDIRECTOR_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @powerdirector/voice-call -> voice-call).
...
export function discoverPowerDirectorPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
