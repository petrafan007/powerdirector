...
import { enablePluginInConfig } from "./enable";
import {
  applyDefaultModel,
  mergeConfigPatch,
  pickAuthMethod,
  resolveProviderMatch,
} from "./provider-auth-choice-helpers";
import { applyAuthProfileConfig } from "./provider-auth-helpers";
import { createVpsAwareOAuthHandlers } from "./provider-oauth-flow";
import { isRemoteEnvironment, openUrl } from "./setup-browser";
import type { ProviderAuthMethod, ProviderAuthOptionBag } from "./types";
...
function restoreConfiguredPrimaryModel(
  ...
  originalConfig: PowerDirectorConfig,
): PowerDirectorConfig {
...
async function loadPluginProviderRuntime() {
  return import("./provider-auth-choice.runtime");
}
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: PowerDirectorConfig; defaultModel?: string }> {
  const agentId = params.agentId ?? resolveDefaultAgentId(params.config);
  const defaultAgentId = resolveDefaultAgentId(params.config);
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId
      ? resolvePowerDirectorAgentDir()
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolvePowerDirectorAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
