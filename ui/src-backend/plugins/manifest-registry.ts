...
import { loadBundleManifest } from "./bundle-manifest";
...
import { isPathInside, safeRealpathSync } from "./path-safety";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.POWERDIRECTOR_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.POWERDIRECTOR_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "powerdirector",
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "powerdirector") === "bundle";
    const manifestRes =
