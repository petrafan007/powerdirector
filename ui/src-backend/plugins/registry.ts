...
import { registerContextEngineForOwner } from "../context-engine/registry";
import type {
...
import { registerPluginCommand, validatePluginCommandDefinition } from "./commands";
import { normalizePluginHttpPath } from "./http-path";
import { findOverlappingPluginHttpRoute } from "./http-route-overlap";
import { registerPluginInteractiveHandler } from "./interactive";
import { normalizeRegisteredProvider } from "./provider-validation";
import { createEmptyPluginRegistry } from "./registry-empty";
import { withPluginRuntimePluginIdScope } from "./runtime/gateway-request-scope";
...
import type {
  ...
  PowerDirectorPluginCliRegistrar,
  PowerDirectorPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  PowerDirectorPluginHttpRouteAuth,
  PowerDirectorPluginHttpRouteMatch,
  PowerDirectorPluginHttpRouteHandler,
  PowerDirectorPluginHttpRouteParams,
  PowerDirectorPluginHookOptions,
...
export type PluginToolRegistration = {
  pluginId: string;
  pluginName?: string;
  factory: PowerDirectorPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
  rootDir?: string;
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: PowerDirectorPluginCliRegistrar;
  commands: string[];
  source: string;
  rootDir?: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  pluginName?: string;
  service: PowerDirectorPluginService;
  source: string;
  rootDir?: string;
};
...
export type PluginCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: PowerDirectorPluginCommandDefinition;
  source: string;
  rootDir?: string;
};
...
export { createEmptyPluginRegistry } from "./registry-empty";
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: PowerDirectorPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: PowerDirectorPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: PowerDirectorPluginHookOptions | undefined,
    config: PowerDirectorPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: PowerDirectorPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: PowerDirectorPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as PowerDirectorPluginChannelRegistration).plugin === "object"
        ? (registration as PowerDirectorPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: PowerDirectorPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: PowerDirectorPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: PowerDirectorPluginCommandDefinition) => {
