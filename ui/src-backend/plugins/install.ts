...
} & Partial<Record<typeof POWERDIRECTOR_MANIFEST_KEY, { extensions?: string[] }>>;
...
async function ensurePowerDirectorExtensions(manifest: PackageManifest) {
  const extensions = manifest[POWERDIRECTOR_MANIFEST_KEY]?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing powerdirector.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json powerdirector.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensurePowerDirectorExtensions(manifest);
  } catch (err) {
...
export async function installPluginFromArchive(params: {
  ...
  return await withTempDir("powerdirector-plugin-", async (tmpDir) => {
