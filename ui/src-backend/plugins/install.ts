...
import { fileExists, readJsonFile, resolveArchiveKind } from "../infra/archive";
import { writeFileFromPathWithinRoot } from "../infra/fs-safe";
import { resolveExistingInstallPath, withExtractedArchiveRoot } from "../infra/install-flow";
import {
  resolveInstallModeOptions,
  resolveTimedInstallModeOptions,
} from "../infra/install-mode-options";
...
} from "../infra/install-target";
import {
  finalizeNpmSpecArchiveInstall,
  installFromNpmSpecArchiveWithInstaller,
} from "../infra/npm-pack-install";
...
import { detectBundleManifestFormat, loadBundleManifest } from "./bundle-manifest";
import {
  loadPluginManifest,
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing powerdirector.extensions; update the plugin package to include powerdirector.extensions (for example ["./dist/index"]). See https://docs.powerdirector.ai/help/troubleshooting#plugin-install-fails-with-missing-powerdirector-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_POWERDIRECTOR_EXTENSIONS: "missing_powerdirector_extensions",
  EMPTY_POWERDIRECTOR_EXTENSIONS: "empty_powerdirector_extensions",
...
function ensurePowerDirectorExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_POWERDIRECTOR_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json powerdirector.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_POWERDIRECTOR_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Bundle "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "powerdirector security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "powerdirector security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensurePowerDirectorExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensurePowerDirectorExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from powerdirector.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-powerdirector"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "powerdirector security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "powerdirector security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "powerdirector-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "powerdirector-npm-pack-",
    spec,
    timeoutMs,
