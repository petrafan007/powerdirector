...
import { modelKey, normalizeModelRef, normalizeProviderId } from "../agents/model-selection";
import type { NormalizedUsage } from "../agents/usage";
import type { PowerDirectorConfig } from "../config/config";
import type { ModelProviderConfig } from "../config/types.models";
import { getCachedGatewayModelPricing } from "../gateway/model-pricing-cache";
...
function loadModelsJsonCostIndex(): Map<string, ModelCostConfig> {
  const modelsPath = path.join(resolvePowerDirectorAgentDir(), "models.json");
  try {
