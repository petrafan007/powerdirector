...
import { normalizeResolvedSecretInputString } from "../config/types.secrets";
import type {
  TtsConfig,
  TtsAutoMode,
  TtsMode,
...
import type { SpeechVoiceOption } from "./provider-types";
import {
...
export function resolveTtsConfig(cfg: PowerDirectorConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.POWERDIRECTOR_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: PowerDirectorConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: PowerDirectorConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      const tempRoot = resolvePreferredPowerDirectorTmpDir();
