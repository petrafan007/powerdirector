...
import { applyGroupGating, type GroupHistoryEntry } from './monitor/group-gating';
import { buildInboundLine, formatReplyContext } from './monitor/message-line';

let sessionDir: string | undefined;
let sessionStorePath: string;

beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-group-gating-"));
...
describe("applyGroupGating", () => {
  ...
  it("allows group messages when whatsapp groups default disables mention gating", () => {
    const cfg = makeConfig({
      ...
      messages: { groupChat: { mentionPatterns: ["@powerdirector"] } },
    });
...
describe("buildInboundLine", () => {
  it("prefixes group messages with sender", () => {
    const line = buildInboundLine({
      cfg: {
        agents: { defaults: { workspace: "/tmp/powerdirector" } },
