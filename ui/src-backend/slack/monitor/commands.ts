import type { SlackSlashCommandConfig } from '../../config/config';
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "powerdirector");
  const name = normalizedName || "powerdirector";
  return {
