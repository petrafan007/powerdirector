...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${POWERDIRECTOR_ACTION_PREFIX}`);

  // Handle PowerDirector modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
