...
import { loadAuthProfileStoreForSecretsRuntime } from "../agents/auth-profiles";
...
import { resolveStateDir, type PowerDirectorConfig } from "../config/config";
import type { ConfigWriteOptions } from "../config/io";
import type { SecretProviderConfig } from "../config/types.secrets";
import { normalizeAgentId } from "../routing/session-key";
import { resolveConfigDir, resolveUserPath } from "../utils";
import { iterateAuthProfileCredentials } from "./auth-profiles-scan";
import { createSecretsConfigIO } from "./config-io";
import { getSkippedExecRefStaticError } from "./exec-resolution-policy";
import { deletePathStrict, getPath, setPathCreateStrict } from "./path-utils";
import {
...
import { listKnownSecretEnvVarNames } from "./provider-env-vars";
import { resolveSecretRefValue } from "./resolve";
import { prepareSecretsRuntimeSnapshot } from "./runtime";
import { assertExpectedResolvedSecretValue } from "./secret-value";
import { isNonEmptyString, isRecord, writeTextFileAtomic } from "./shared";
import {
...
async function projectPlanState(params: {
  ...
  const stateDir = resolveStateDir(params.env, safeHomedir);
...
function asConfigPathRoot(store: MutableAuthProfileStore): PowerDirectorConfig {
  return store as unknown as PowerDirectorConfig;
}
...
function scrubEnvFiles(params: {
  ...
  const envPath = path.join(resolveConfigDir(params.env, safeHomedir), ".env");
