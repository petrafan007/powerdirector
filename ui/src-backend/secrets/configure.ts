...
import { listAgentIds, resolveAgentDir, resolveDefaultAgentId } from "../agents/agent-scope";
import type { AuthProfileStore } from "../agents/auth-profiles";
import { AUTH_STORE_VERSION } from "../agents/auth-profiles/constants";
import { resolveAuthStorePath } from "../agents/auth-profiles/paths";
import type { PowerDirectorConfig } from "../config/config";
import type { SecretProviderConfig, SecretRef, SecretRefSource } from "../config/types.secrets";
import { isSafeExecutableValue } from "../infra/exec-safety";
import { normalizeAgentId } from "../routing/session-key";
import { runSecretsApply, type SecretsApplyResult } from "./apply";
import { createSecretsConfigIO } from "./config-io";
import {
...
import { getSkippedExecRefStaticError } from "./exec-resolution-policy";
import type { SecretsApplyPlan } from "./plan";
import { PROVIDER_ENV_VARS } from "./provider-env-vars";
import {
...
import { resolveSecretRefValue } from "./resolve";
import { assertExpectedResolvedSecretValue } from "./secret-value";
import { isRecord } from "./shared";
import { readJsonObjectIfExists } from "./storage-scan";
...
function getSecretProviders(config: PowerDirectorConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: PowerDirectorConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: PowerDirectorConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "powerdirector.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
  ...
  return `powerdirector:${candidate.path}`;
}
...
function resolveConfigureAgentId(config: PowerDirectorConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: PowerDirectorConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredPowerDirectorConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "powerdirector.json",
