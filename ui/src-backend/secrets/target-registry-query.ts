...
import { SECRET_TARGET_REGISTRY } from "./target-registry-data";
import {
...
} from "./target-registry-pattern";
import type {
  DiscoveredConfigSecretTarget,
  ResolvedPlanTarget,
  SecretTargetRegistryEntry,
} from "./target-registry-types";
...
const POWERDIRECTOR_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: POWERDIRECTOR_COMPILED_SECRET_TARGETS,
    entriesById: POWERDIRECTOR_TARGETS_BY_ID,
  });
