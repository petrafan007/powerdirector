...
import { coerceSecretRef } from "../config/types.secrets";
import { resolveSecretInputRef, type SecretRef } from "../config/types.secrets";
...
import { getSkippedExecRefStaticError, selectRefsForExecPolicy } from "./exec-resolution-policy";
import { listKnownSecretEnvVarNames } from "./provider-env-vars";
import { secretRefKey } from "./ref-contract";
import {
...
import { isNonEmptyString, isRecord } from "./shared";
import { describeUnknownError } from "./shared";
import {
...
import { discoverConfigSecretTargets } from "./target-registry";
...
export async function runSecretsAudit(
  ...
  const envPath = path.join(resolveConfigDir(env, safeHomedir), ".env");
