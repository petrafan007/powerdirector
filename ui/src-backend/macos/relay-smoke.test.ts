...
import { parseRelaySmokeTest, runRelaySmokeTest } from './relay-smoke';
...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { POWERDIRECTOR_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { POWERDIRECTOR_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports POWERDIRECTOR_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { POWERDIRECTOR_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { POWERDIRECTOR_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { POWERDIRECTOR_SMOKE: "nope" })).toBe("qr");
  });
});
...
describe("runRelaySmokeTest", () => {
  it("runs qr smoke test", async () => {
    ...
    const mod = await import('../web/qr-image');
