...
const BUNDLED_VERSION =
  (typeof __POWERDIRECTOR_VERSION__ === "string" && __POWERDIRECTOR_VERSION__) ||
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `powerdirector --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.POWERDIRECTOR_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.POWERDIRECTOR_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (POWERDIRECTOR_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
...
void main().catch((err) => {
  console.error(
    "[powerdirector] Gateway daemon failed:",
