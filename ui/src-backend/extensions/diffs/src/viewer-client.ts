...
import type { DiffViewerPayload, DiffLayout, DiffTheme } from "./types";
import { parseViewerPayloadJson } from "./viewer-payload";
...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-powerdirector-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-powerdirector-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.powerdirectorDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.powerdirectorDiffsError = "true";
