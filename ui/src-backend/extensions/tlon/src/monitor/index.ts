import type { RuntimeEnv, ReplyPayload, PowerDirectorConfig } from "../../api";
import { createLoggerBackedRuntime } from "../../api";
import { getTlonRuntime } from "../runtime";
import { createSettingsManager, type TlonSettingsStore } from "../settings";
import { normalizeShip, parseChannelNest } from "../targets";
import { resolveTlonAccount } from "../types";
import { authenticate } from "../urbit/auth";
import { ssrfPolicyFromAllowPrivateNetwork } from "../urbit/context";
import type { Foreigns, DmInvite } from "../urbit/foreigns";
import { sendDm, sendGroupMessage } from "../urbit/send";
import { UrbitSSEClient } from "../urbit/sse-client";
import { createTlonApprovalRuntime } from "./approval-runtime";
import {
...
import { resolveChannelAuthorization } from "./authorization";
import { createTlonCitationResolver } from "./cites";
import { fetchAllChannels, fetchInitData } from "./discovery";
import { cacheMessage, getChannelHistory, fetchThreadHistory } from "./history";
import { downloadMessageImages } from "./media";
import { createProcessedMessageTracker } from "./processed-messages";
import {
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as PowerDirectorConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in PowerDirector config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your PowerDirector config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.powerdirector.ai/concepts/session#secure-dm-mode`;
