...
import { parseBlueBubblesAllowTarget } from "./targets";
import { normalizeBlueBubblesServerUrl } from "./types";
import { DEFAULT_WEBHOOK_PATH } from "./webhook-shared";
...
function resolveBlueBubblesServerUrl(cfg: PowerDirectorConfig, accountId: string): string | undefined {
  return resolveBlueBubblesAccount({ cfg, accountId }).config.serverUrl?.trim() || undefined;
}

function resolveBlueBubblesWebhookPath(cfg: PowerDirectorConfig, accountId: string): string | undefined {
...
export const blueBubblesSetupWizard: ChannelSetupWizard = {
  ...
  completionNote: {
    ...
    lines: [
      ...
      "2. Add your PowerDirector gateway URL + webhook path",
