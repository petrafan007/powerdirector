...
import { registerAcpRuntimeBackend, unregisterAcpRuntimeBackend } from "../runtime-api";
import { resolveAcpxPluginConfig, type ResolvedAcpxPluginConfig } from "./config";
import { ensureAcpx } from "./ensure";
import { ACPX_BACKEND_ID, AcpxRuntime } from "./runtime";
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: PowerDirectorPluginServiceContext): Promise<void> {
    ...
    async stop(_ctx: PowerDirectorPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
