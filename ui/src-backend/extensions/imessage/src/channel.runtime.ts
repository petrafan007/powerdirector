...
import { getIMessageRuntime } from "./runtime";
import { imessageSetupWizard } from "./setup-surface";
...
export async function sendIMessageOutbound(params: {
  cfg: Parameters<typeof import("./accounts").resolveIMessageAccount>[0]["cfg"];
  to: string;
  text: string;
  mediaUrl?: string;
...
export async function startIMessageGatewayAccount(
  ctx: Parameters<
    NonNullable<
      NonNullable<
        import("../runtime-api").ChannelPlugin<ResolvedIMessageAccount>["gateway"]
      >["startAccount"]
    >
  >[0],
) {
