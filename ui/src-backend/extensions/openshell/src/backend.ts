...
import { resolveOpenShellPluginConfig, type ResolvedOpenShellPluginConfig } from "./config";
import { createOpenShellFsBridge } from "./fs-bridge";
import { replaceDirectoryContents } from "./mirror";
...
class OpenShellSandboxBackendImpl {
  ...
  private async syncWorkspaceFromRemote(): Promise<void> {
    const tmpDir = await fs.mkdtemp(
      path.join(resolveOpenShellTmpRoot(), "powerdirector-openshell-sync-"),
    );
    try {
      const result = await runOpenShellCli({
        context: this.params.execContext,
...
function buildOpenShellSandboxName(scopeKey: string): string {
  ...
  return `powerdirector-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
