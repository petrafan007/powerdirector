...
import { writeQrDataUrlToTempFile } from "./qr-temp-file";
...
function ensureZalouserPluginEnabled(cfg: PowerDirectorConfig): PowerDirectorConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("powerdirector directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
  ...
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ...
        : resolveDefaultZalouserAccountId(cfg as PowerDirectorConfig);
    return await promptZalouserAllowFrom({
      cfg: cfg as PowerDirectorConfig,
      prompter,
      accountId: id,
    });
  },
...
export { zalouserSetupAdapter } from "./setup-core";
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as PowerDirectorConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("powerdirector directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as PowerDirectorConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as PowerDirectorConfig, accountId, resolved as string[]),
  },
