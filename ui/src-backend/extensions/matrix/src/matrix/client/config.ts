...
import { loadMatrixSdk } from "../sdk-runtime";
import { ensureMatrixSdkLoggingConfigured } from "./logging";
import type { MatrixAuth, MatrixResolvedConfig } from "./types";
  ...
  const { response: loginResponse, release: releaseLoginResponse } = await fetchWithSsrFGuard({
    ...
    init: {
      ...
      body: JSON.stringify({
        ...
        initial_device_display_name: resolved.deviceName ?? "PowerDirector Gateway",
      }),
    },
