...
import { buildMatrixConfigUpdate, matrixSetupAdapter } from "./setup-core";
...
export { matrixSetupAdapter } from "./setup-core";
  ...
  finalize: async ({ cfg, runtime, prompter, forceAllowFrom }) => {
    ...
    const deviceName = String(
      await prompter.text({
        ...
        initialValue: existing.deviceName ?? "PowerDirector Gateway",
      }),
    ).trim();
