...
import type { ResolvedMattermostAccount } from "./accounts";
import { resolveSlashCommandConfig, type MattermostRegisteredCommand } from "./slash-commands";
import { createSlashCommandHttpHandler } from "./slash-http";
...
export function activateSlashCommands(params: {
  ...
  triggerMap?: Map<string, string>;
  api: {
    cfg: import("../runtime-api").PowerDirectorConfig;
    runtime: import("../runtime-api").RuntimeEnv;
  };
  log?: (msg: string) => void;
}) {
 ...
 */
export function registerSlashCommandRoute(api: PowerDirectorPluginApi) {
  ...
  const addCallbackPaths = (
    raw: Partial<import("./slash-commands").MattermostSlashCommandConfig> | undefined,
  ) => {
  ...
  const commandsRaw = mmConfig?.commands as
    | Partial<import("./slash-commands").MattermostSlashCommandConfig>
    | undefined;
