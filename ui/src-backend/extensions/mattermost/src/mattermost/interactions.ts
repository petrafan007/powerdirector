...
import { isTrustedProxyAddress, resolveClientIp, type PowerDirectorConfig } from "../runtime-api";
import { getMattermostRuntime } from "../runtime";
import { updateMattermostPost, type MattermostClient, type MattermostPost } from "./client";

const INTERACTION_MAX_BODY_BYTES = 64 * 1024;
const INTERACTION_BODY_TIMEOUT_MS = 10_000;
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__powerdirector_channel_id";
...
type InteractionCallbackConfig = Pick<PowerDirectorConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "powerdirector-mattermost-interactions").update(botToken).digest("hex");
}
