...
import type { CallManagerContext } from "./manager/context";
import { processEvent as processManagerEvent } from "./manager/events";
import { getCallByProviderCallId as getCallByProviderCallIdFromMaps } from "./manager/lookup";
import {
...
} from "./manager/outbound";
import { getCallHistoryFromStore, loadActiveCallsFromStore } from "./manager/store";
import { startMaxDurationTimer } from "./manager/timers";
...
function resolveDefaultStoreBase(config: VoiceCallConfig, storePath?: string): string {
  ...
  const preferred = path.join(os.homedir(), ".powerdirector", "voice-calls");
