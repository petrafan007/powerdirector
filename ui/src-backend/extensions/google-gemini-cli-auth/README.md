# Google Gemini CLI Auth (PowerDirector plugin)
...
```bash
powerdirector plugins enable google-gemini-cli-auth
```
...
```bash
powerdirector models auth login --provider google-gemini-cli --set-default
```
