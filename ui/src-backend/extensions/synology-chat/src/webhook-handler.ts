...
import { sendMessage, resolveChatUserId } from "./client";
import { validateToken, authorizeUserForDm, sanitizeInput, RateLimiter } from "./security";
import type { SynologyWebhookPayload, ResolvedSynologyChatAccount } from "./types";
...
function extractTokenFromHeaders(req: IncomingMessage): string | undefined {
  const explicit =
    ...
    headerValue(req.headers["x-powerdirector-token"]);
