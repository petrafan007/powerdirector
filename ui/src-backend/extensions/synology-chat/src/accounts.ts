...
import type { SynologyChatChannelConfig, ResolvedSynologyChatAccount } from "./types";

/** Extract the channel config from the full PowerDirector config object. */
...
export function resolveAccount(cfg: any, accountId?: string | null): ResolvedSynologyChatAccount {
  ...
  const envBotName = process.env.POWERDIRECTOR_BOT_NAME ?? "PowerDirector";
