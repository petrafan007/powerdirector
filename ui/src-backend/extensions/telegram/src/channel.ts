...
import { auditTelegramGroupMembership, collectTelegramUnmentionedGroupIds } from "./audit";
import { buildTelegramGroupPeerId } from "./bot/helpers";
import {
...
import { looksLikeTelegramTargetId, normalizeTelegramMessagingTarget } from "./normalize";
import { sendTelegramPayloadMessages } from "./outbound-adapter";
import { parseTelegramReplyToMessageId, parseTelegramThreadId } from "./outbound-params";
import { probeTelegram, type TelegramProbe } from "./probe";
import { getTelegramRuntime } from "./runtime";
import { sendTypingTelegram } from "./send";
...
import { collectTelegramStatusIssues } from "./status-issues";
import { parseTelegramTarget } from "./targets";
...
function hasTelegramExecApprovalDmRoute(cfg: PowerDirectorConfig): boolean {
...
export const telegramPlugin: ChannelPlugin<ResolvedTelegramAccount, TelegramProbe> = {
  ...
  lifecycle: {
    onAccountConfigChanged: async ({ prevCfg, nextCfg, accountId }) => {
      ...
      if (previousToken !== nextToken) {
        const { deleteTelegramUpdateOffset } = await import("./update-offset-store");
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      const nextCfg = { ...cfg } as PowerDirectorConfig;
