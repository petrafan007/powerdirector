...
let createTelegramBotRef: typeof import("./bot").createTelegramBot;
...
beforeAll(async () => {
  const harness = await import("./bot.media.e2e-harness");
...
vi.mock("./sticker-cache", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./sticker-cache")>();
  return {
    ...actual,
