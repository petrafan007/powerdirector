...
import type { ResolvedNextcloudTalkAccount } from "./accounts";
import {
...
import { resolveNextcloudTalkRoomKind } from "./room-info";
import { getNextcloudTalkRuntime } from "./runtime";
import { sendMessageNextcloudTalk } from "./send";
import type { CoreConfig, GroupPolicy, NextcloudTalkInboundMessage } from "./types";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as PowerDirectorConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as PowerDirectorConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as PowerDirectorConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as PowerDirectorConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as PowerDirectorConfig);
