...
import { NextcloudTalkConfigSchema } from "./config-schema";
import { monitorNextcloudTalkProvider } from "./monitor";
import {
  looksLikeNextcloudTalkTargetId,
  normalizeNextcloudTalkMessagingTarget,
} from "./normalize";
import { resolveNextcloudTalkGroupToolPolicy } from "./policy";
import { getNextcloudTalkRuntime } from "./runtime";
import { sendMessageNextcloudTalk } from "./send";
import { resolveNextcloudTalkOutboundSessionRoute } from "./session-route";
import { nextcloudTalkSetupAdapter } from "./setup-core";
import { nextcloudTalkSetupWizard } from "./setup-surface";
      ...
      if (changed) {
        if (nextSection && Object.keys(nextSection).length > 0) {
          ...
          if (Object.keys(nextChannels).length > 0) {
            nextCfg.channels = nextChannels as PowerDirectorConfig["channels"];
          } else {
