...
import { NostrConfigSchema } from "./config-schema";
import type { MetricEvent, MetricsSnapshot } from "./metrics";
import { normalizePubkey, startNostrBus, type NostrBusHandle } from "./nostr-bus";
import type { ProfilePublishResult } from "./nostr-profile";
import { getNostrRuntime } from "./runtime";
import { resolveNostrOutboundSessionRoute } from "./session-route";
import { nostrSetupAdapter, nostrSetupWizard } from "./setup-surface";
import {
  listNostrAccountIds,
    ...
    startAccount: async (ctx) => {
      ...
      const bus = await startNostrBus({
        ...
        onMessage: async (senderPubkey, text, reply) => {
          ...
          // Forward to PowerDirector's message pipeline
          await (
