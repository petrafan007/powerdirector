...
function resolveGatewayPort(cfg: PowerDirectorPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.POWERDIRECTOR_GATEWAY_PORT?.trim()) ??
...
function resolveScheme(
  cfg: PowerDirectorPluginApi["config"],
...
function resolveAuthLabel(cfg: PowerDirectorPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: PowerDirectorPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for PowerDirector devices",
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (channel === "telegram" && target) {
            try {
              ...
              if (send) {
                await send(
                  target,
                  ["Scan this QR code with the PowerDirector iOS app:", "", "```", qrAscii, "```"].join(
                    "\n",
                  ),
                  {
          ...
          return {
            text: [
              "Scan this QR code with the PowerDirector iOS app:",
              "",
              "```",
              qrAscii,
              "```",
