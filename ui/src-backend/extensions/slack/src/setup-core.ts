...
function enableSlackAccount(cfg: PowerDirectorConfig, accountId: string): PowerDirectorConfig {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: PowerDirectorConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: PowerDirectorConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: PowerDirectorConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: PowerDirectorConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.allow !== false && value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: PowerDirectorConfig; accountId: string }) =>
    ...
    disable: (cfg: PowerDirectorConfig) => setSetupChannelEnabled(cfg, channel, false),
