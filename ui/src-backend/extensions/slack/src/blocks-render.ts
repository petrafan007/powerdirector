...
export const SLACK_REPLY_SELECT_ACTION_ID = "powerdirector:reply_select";
...
export function buildSlackInteractiveBlocks(interactive?: InteractiveReply): SlackBlock[] {
  ...
  return reduceInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `powerdirector_reply_buttons_${++state.buttonIndex}`,
    ...
    state.blocks.push({
      type: "actions",
      block_id: `powerdirector_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
