...
import { agentCommandFromIngress } from "powerdirector/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "powerdirector/plugin-sdk/agent-runtime";
...
import type { DiscordAccountConfig, TtsConfig } from "powerdirector/plugin-sdk/config-runtime";
...
import { transcribeAudioFile } from "powerdirector/plugin-sdk/media-understanding-runtime";
...
import { parseTtsDirectives } from "powerdirector/plugin-sdk/speech";
import { textToSpeech } from "powerdirector/plugin-sdk/speech-runtime";
import { formatMention } from "../mentions";
import { resolveDiscordOwnerAccess } from "../monitor/allow-list";
import { formatDiscordUserTag } from "../monitor/format";
import { loadDiscordVoiceSdk } from "./sdk-runtime";
...
function resolveVoiceTtsConfig(params: { cfg: PowerDirectorConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredPowerDirectorTmpDir(), "discord-voice-"));
