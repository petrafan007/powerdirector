...
import { isThreadArchived } from "./thread-bindings.discord-api";
...
type DiscordReactionIngressAuthorizationParams = {
  ...
  groupPolicy: "open" | "allowlist" | "disabled";
  allowNameMatching: boolean;
  guildInfo: import("./allow-list").DiscordGuildEntryResolved | null;
  channelConfig?: import("./allow-list").DiscordChannelConfigResolved | null;
};
...
export class DiscordThreadUpdateListener extends ThreadUpdateListener {
  constructor(
    private cfg: PowerDirectorConfig,
    private accountId: string,
    private logger?: Logger,
  ) {
    super();
