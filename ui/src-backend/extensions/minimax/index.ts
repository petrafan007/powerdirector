...
} from "./media-understanding-provider";
import type { MiniMaxRegion } from "./oauth";
import { applyMinimaxApiConfig, applyMinimaxApiConfigCn } from "./onboard";
import { buildMinimaxPortalProvider, buildMinimaxProvider } from "./provider-catalog";
...
function createOAuthHandler(region: MiniMaxRegion) {
  ...
  return async (ctx: ProviderAuthContext): Promise<ProviderAuthResult> => {
    ...
    try {
      const { loginMiniMaxPortalOAuth } = await import("./oauth.runtime");
