...
import { applyGoogleChatInboundAccessPolicy, isSenderAllowed } from "./monitor-access";
import type {
...
import { createGoogleChatWebhookRequestHandler } from "./monitor-webhook";
import { getGoogleChatRuntime } from "./runtime";
import type { GoogleChatAttachment, GoogleChatEvent } from "./types";
export type { GoogleChatMonitorOptions, GoogleChatRuntimeEnv } from "./monitor-types";
 ...
 * 3. "PowerDirector" as generic fallback
 */
...
async function uploadAttachmentForReply(params: {
  ...
  const { uploadGoogleChatAttachment } = await import("./api");
