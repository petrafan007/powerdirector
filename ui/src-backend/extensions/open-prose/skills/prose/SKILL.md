...
metadata: { "powerdirector": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## PowerDirector Runtime Mapping

- **Task tool** in the upstream spec == PowerDirector `sessions_spawn`
- **File I/O** == PowerDirector `read`/`write`
- **Remote fetch** == PowerDirector `web_fetch` (or `exec` with curl when POST is required)
