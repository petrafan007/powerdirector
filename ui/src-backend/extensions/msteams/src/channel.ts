...
import { resolveMSTeamsGroupToolPolicy } from "./policy";
import type { ProbeMSTeamsResult } from "./probe";
import {
...
} from "./resolve-allowlist";
import { getMSTeamsRuntime } from "./runtime";
import { resolveMSTeamsOutboundSessionRoute } from "./session-route";
import { msteamsSetupAdapter } from "./setup-core";
import { msteamsSetupWizard } from "./setup-surface";
import { resolveMSTeamsCredentials } from "./token";
...
const resolveMSTeamsChannelConfig = (cfg: PowerDirectorConfig) => ({
...
export const msteamsPlugin: ChannelPlugin<ResolvedMSTeamsAccount> = {
  ...
  security: {
    collectWarnings: projectWarningCollector(
      ({ cfg }: { cfg: PowerDirectorConfig }) => ({ cfg }),
  ...
  gateway: {
    startAccount: async (ctx) => {
      const { monitorMSTeamsProvider } = await import("./index");
