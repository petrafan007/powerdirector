...
import { resolveMSTeamsStorePath } from "./storage";
import { readJsonFile, withFileLock, writeJsonFile } from "./store-fs";
...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["powerdirectorPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["powerdirector", "pollId"]) ??
    readNestedString(value, ["powerdirector", "poll", "id"]) ??
    readNestedString(value, ["data", "powerdirectorPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "powerdirector", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          powerdirectorPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "powerdirector poll vote",
          displayText: "Vote recorded",
          value: { powerdirectorPollId: pollId, pollId },
        },
      },
    ],
  };
