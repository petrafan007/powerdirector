...
import type { ChannelLogSink, TwitchAccountConfig, TwitchChatMessage } from "./types";
import { normalizeToken } from "./utils/twitch";
...
export class TwitchClientManager {
  ...
  async getClient(
    ...
    if (!tokenResolution.token) {
      this.logger.error(
        `Missing Twitch token for account ${account.username} (set channels.twitch.accounts.${account.username}.token or POWERDIRECTOR_TWITCH_ACCESS_TOKEN for default)`,
      );
