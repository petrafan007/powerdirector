/**
 * Twitch channel plugin for PowerDirector.
 *
...
import { removeClientManager } from "./client-manager-registry";
import { TwitchConfigSchema } from "./config-schema";
import { DEFAULT_ACCOUNT_ID, getAccountConfig, listAccountIds } from "./config";
import { twitchOutbound } from "./outbound";
import { probeTwitch } from "./probe";
import { resolveTwitchTargets } from "./resolver";
import { twitchSetupAdapter, twitchSetupWizard } from "./setup-surface";
import { collectTwitchStatusIssues } from "./status";
import { resolveTwitchToken } from "./token";
import type {
 ...
 * for PowerDirector. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: PowerDirectorConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: PowerDirectorConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: PowerDirectorConfig): boolean => {
