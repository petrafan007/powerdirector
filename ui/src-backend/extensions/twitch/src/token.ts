 ...
 * The POWERDIRECTOR_TWITCH_ACCESS_TOKEN env var is only used for the default account.
 *
...
import { DEFAULT_ACCOUNT_ID, normalizeAccountId, type PowerDirectorConfig } from "../runtime-api";
 ...
 *
 * @param cfg - PowerDirector config
...
export function resolveTwitchToken(
  ...
  const envToken = allowEnv
    ? normalizeTwitchToken(opts.envToken ?? process.env.POWERDIRECTOR_TWITCH_ACCESS_TOKEN)
    : undefined;
  if (envToken) {
