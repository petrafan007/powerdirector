...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: POWERDIRECTOR_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: PowerDirectorConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var POWERDIRECTOR_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: PowerDirectorConfig): "open" | "allowlist" | "disabled" {
  const account = getAccountConfig(cfg, DEFAULT_ACCOUNT_ID);
...
const twitchDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg) => {
    const account = getAccountConfig(cfg as PowerDirectorConfig, DEFAULT_ACCOUNT_ID);
    if (account?.allowedRoles?.includes("all")) {
      return "open";
    }
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as PowerDirectorConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as PowerDirectorConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
const twitchGroupAccess: NonNullable<ChannelSetupWizard["groupAccess"]> = {
  ...
  currentPolicy: ({ cfg }) => resolveTwitchGroupPolicy(cfg as PowerDirectorConfig),
  currentEntries: ({ cfg }) => {
    const account = getAccountConfig(cfg as PowerDirectorConfig, DEFAULT_ACCOUNT_ID);
  ...
  setPolicy: ({ cfg, policy }) => setTwitchGroupPolicy(cfg as PowerDirectorConfig, policy),
  resolveAllowlist: async () => [],
  applyAllowlist: ({ cfg }) => cfg as PowerDirectorConfig,
};
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.POWERDIRECTOR_TWITCH_ACCESS_TOKEN?.trim();
    ...
    const cfgWithAccount = setTwitchAccount(cfg as PowerDirectorConfig, {
      username,
      accessToken: token,
      clientId,
      channel: channelName,
