import type { AnyAgentTool, PowerDirectorPluginApi } from "../runtime-api";
...
export function createToolFactoryHarness(cfg: PowerDirectorPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<PowerDirectorPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as PowerDirectorPluginApi,
    resolveTool,
  };
}
