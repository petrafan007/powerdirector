...
import { formatError, getStatusCode } from "./session-errors";
export { formatError, getStatusCode } from "./session-errors";
...
export async function createWaSocket(
  ...
  const sock = makeWASocket({
    ...
    browser: ["powerdirector", "cli", VERSION],
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("powerdirector channels login")}`,
              ),
            );
          }
        }
