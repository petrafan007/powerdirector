...
// instance.  See: https://github.com/powerdirector/powerdirector/issues/14406
const GLOBAL_KEY = "__powerdirector_wa_listeners" as const;
const GLOBAL_CURRENT_KEY = "__powerdirector_wa_current_listener" as const;
...
export function requireActiveWebListener(accountId?: string | null): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`powerdirector channels login --channel whatsapp --account ${id}`)}.`,
    );
  }
