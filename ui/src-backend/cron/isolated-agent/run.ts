...
import { resolveNestedAgentLane } from "../../agents/lanes";
...
import type { CliDeps } from "../../cli/outbound-send-deps";
...
import { resolveCronAgentSessionKey } from "./session-key";
import { resolveCronSession } from "./session";
import { resolveCronSkillsSnapshot } from "./skills-snapshot";
import { isLikelyInterimCronMessage } from "./subagent-followup";
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.POWERDIRECTOR_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.powerdirector/agents/<agentId>/agent/).
  const agentId = normalizedRequested ?? defaultAgentId;
  const agentCfg = buildCronAgentDefaultsConfig({
    defaults: params.cfg.agents?.defaults,
    agentConfigOverride,
  });
  const cfgWithAgentDefaults: PowerDirectorConfig = {
    ...params.cfg,
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: async (providerOverride, modelOverride, runOptions) => {
          ...
          if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
            ...
            // the configured timeoutSeconds. See: https://github.com/powerdirector/powerdirector/issues/29774
          ...
          const result = await runEmbeddedPiAgent({
            ...
            // inherit owner-only tooling like local `powerdirector agent` runs.
            senderIsOwner: true,
            messageChannel,
