...
import { sleepWithAbort } from "../../infra/backoff";
import {
  deliverOutboundPayloads,
  type OutboundDeliveryResult,
} from "../../infra/outbound/deliver";
...
import type { CronJob, CronRunTelemetry } from "../types";
import type { DeliveryTargetResolution } from "./delivery-target";
import { pickSummaryFromOutput } from "./helpers";
import type { RunCronAgentTurnResult } from "./run";
import {
...
type DispatchCronDeliveryParams = {
  ...
  cfgWithAgentDefaults: PowerDirectorConfig;
  deps: CliDeps;
  job: CronJob;
  agentId: string;
...
function pruneCompletedDirectCronDeliveries(now: number) {
  const ttlMs = process.env.POWERDIRECTOR_TEST_FAST === "1" ? 60_000 : 24 * 60 * 60 * 1000;
...
function resolveDirectCronRetryDelaysMs(): readonly number[] {
  return process.env.NODE_ENV === "test" && process.env.POWERDIRECTOR_TEST_FAST === "1"
    ? [8, 16, 32]
    : [5_000, 10_000, 20_000];
}
    ...
    try {
      ...
      const runDelivery = async () =>
        await deliverOutboundPayloads({
          ...
          // See: https://github.com/powerdirector/powerdirector/issues/40545
          skipQueue: true,
        });
