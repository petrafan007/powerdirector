import { resolveFailoverReasonFromError } from "../../agents/failover-error";
import type { CronConfig, CronRetryOn } from "../../config/types.cron";
...
import { sweepCronRunSessions } from "../session-reaper";
import type {
  CronDeliveryStatus,
  CronJob,
  CronMessageChannel,
...
import type { CronEvent, CronServiceState } from "./state";
import { ensureLoaded, persist } from "./store";
import { DEFAULT_JOB_TIMEOUT_MS, resolveCronJobTimeoutMs } from "./timeout-policy";

export { DEFAULT_JOB_TIMEOUT_MS } from "./timeout-policy";
...
export async function onTimer(state: CronServiceState) {
  if (state.running) {
    ...
    // execution to finish.
    // See: https://github.com/powerdirector/powerdirector/issues/12025
    ...
    if (job.wakeMode === "now" && state.deps.runHeartbeatOnce) {
      ...
      for (;;) {
        ...
        heartbeatResult = await state.deps.runHeartbeatOnce({
          ...
          // See: https://github.com/powerdirector/powerdirector/issues/28508
