...
import type { QuickstartGatewayDefaults, WizardFlow } from "./setup.types";
...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "powerdirector security audit --fix",
      "",
      "Must read: https://docs.powerdirector.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("PowerDirector setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: PowerDirectorConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
  ...
  if (compatibilityNotices.length > 0) {
    await prompter.note(
      [
        ...
        `Inspect: ${formatCliCommand("powerdirector plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("powerdirector configure")}.`;
  ...
  const { applyLocalSetupWorkspaceConfig } = await import("../commands/onboard-config");
  ...
  const { configureGatewayForSetup } = await import("./setup.gateway-config");
