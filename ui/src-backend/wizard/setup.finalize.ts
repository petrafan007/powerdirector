...
import { setupWizardShellCompletion } from "./setup.completion";
import { resolveSetupSecretInputString } from "./setup.secret-input";
import type { GatewayWizardSettings, WizardFlow } from "./setup.types";
...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.powerdirector.ai/gateway/health",
          "https://docs.powerdirector.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.powerdirector.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        `Open the dashboard anytime: ${formatCliCommand("powerdirector dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control PowerDirector."
            : "Copy/paste this URL in a browser on this machine to control PowerDirector.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("powerdirector dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.powerdirector.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.powerdirector.ai/security",
    "Security",
  );
  ...
  if (webSearchProvider) {
    const { SEARCH_PROVIDER_OPTIONS, resolveExistingKey, hasExistingKey, hasKeyInEnv } =
      await import("../commands/onboard-search");
    ...
    if (webSearchEnabled !== false && hasKey) {
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("powerdirector configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.powerdirector.ai/tools/web"}`,
          "Docs: https://docs.powerdirector.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("powerdirector configure --section web")}`,
          "",
          "Docs: https://docs.powerdirector.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://powerdirector.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control PowerDirector."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control PowerDirector.",
  );
