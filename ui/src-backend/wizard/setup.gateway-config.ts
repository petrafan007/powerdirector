...
import type { GatewayAuthChoice, SecretInputMode } from "../commands/onboard-types";
import type { GatewayBindMode, GatewayTailscaleMode, PowerDirectorConfig } from "../config/config";
import { ensureControlUiAllowedOriginsForNonLoopbackBind } from "../config/gateway-control-ui-origins";
import {
...
import { DEFAULT_DANGEROUS_NODE_COMMANDS } from "../gateway/node-command-policy";
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "POWERDIRECTOR_GATEWAY_TOKEN",
          copy: {
            sourceMessage: "Where is this gateway token stored?",
            envVarPlaceholder: "POWERDIRECTOR_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.POWERDIRECTOR_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.POWERDIRECTOR_GATEWAY_TOKEN) ??
          "",
      });
    ...
    if (!password) {
      const selectedMode = await resolveSecretInputModeForEnvSelection({
        ...
        copy: {
          ...
          plaintextHint: "Stores the password directly in PowerDirector config",
        },
      });
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "POWERDIRECTOR_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: "Where is this gateway password stored?",
            envVarPlaceholder: "POWERDIRECTOR_GATEWAY_PASSWORD",
          },
        });
