...
<title>PowerDirector Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.powerdirectorCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.powerdirectorCanvasA2UIAction &&
        typeof window.powerdirectorCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.powerdirectorSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("powerdirector:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android PowerDirector node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.powerdirectorSendUserAction === "function"
        ? window.powerdirectorSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.POWERDIRECTOR_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.POWERDIRECTOR_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>PowerDirector Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
