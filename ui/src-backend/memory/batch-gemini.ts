...
import type { GeminiEmbeddingClient, GeminiTextEmbeddingRequest } from "./embeddings-gemini";
import { hashText } from "./internal";
import { withRemoteHttpResponse } from "./remote-http";
...
function buildGeminiUploadBody(params: { jsonl: string; displayName: string }): {
  ...
  const boundary = `powerdirector-${hashText(params.displayName)}`;
