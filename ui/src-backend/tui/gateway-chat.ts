...
import { resolveConfiguredSecretInputString } from "../gateway/resolve-configured-secret-input-string";
import { GATEWAY_CLIENT_MODES, GATEWAY_CLIENT_NAMES } from "../utils/message-channel";
import { VERSION } from "../version";
import type { ResponseUsageMode, SessionInfo, SessionScope } from "./tui-types";
...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set POWERDIRECTOR_GATEWAY_TOKEN/POWERDIRECTOR_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "powerdirector-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.POWERDIRECTOR_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.POWERDIRECTOR_GATEWAY_PASSWORD);
