...
import { resolveFailureDestination, sendFailureNotificationAnnounce } from "../cron/delivery";
import { runCronIsolatedAgentTurn } from "../cron/isolated-agent";
import { resolveDeliveryTarget } from "../cron/isolated-agent/delivery-target";
import {
  appendCronRunLog,
...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.POWERDIRECTOR_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;
