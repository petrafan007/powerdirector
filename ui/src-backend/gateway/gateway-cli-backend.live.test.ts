...
import { GATEWAY_CLIENT_NAMES } from '../utils/message-channel';
...
const CLI_LIVE = isTruthyEnvValue(process.env.POWERDIRECTOR_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("POWERDIRECTOR_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.POWERDIRECTOR_CONFIG_PATH,
      token: process.env.POWERDIRECTOR_GATEWAY_TOKEN,
      skipChannels: process.env.POWERDIRECTOR_SKIP_CHANNELS,
      skipGmail: process.env.POWERDIRECTOR_SKIP_GMAIL_WATCHER,
      skipCron: process.env.POWERDIRECTOR_SKIP_CRON,
      skipCanvas: process.env.POWERDIRECTOR_SKIP_CANVAS_HOST,
    ...
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = token;

    const rawModel = process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `POWERDIRECTOR_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `POWERDIRECTOR_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "POWERDIRECTOR_LIVE_CLI_BACKEND_ARGS",
        process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`POWERDIRECTOR_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "POWERDIRECTOR_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.POWERDIRECTOR_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "POWERDIRECTOR_LIVE_CLI_BACKEND_IMAGE_MODE requires POWERDIRECTOR_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }
