...
export async function withTempConfig(params: {
  ...
  const configPath = path.join(dir, "powerdirector.config.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.POWERDIRECTOR_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.POWERDIRECTOR_DISABLE_CONFIG_CACHE;
    } else {
      process.env.POWERDIRECTOR_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
