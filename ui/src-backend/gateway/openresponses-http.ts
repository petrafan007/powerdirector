 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for PowerDirector Gateway.
 *
...
import type { ImageContent } from "../agents/command/types";
...
import { resolveGatewayRequestContext } from "./http-utils";
import { normalizeInputHostnameAllowlist } from "./input-allowlist";
import {
...
import { buildAgentPrompt } from "./openresponses-prompt";
...
export { buildAgentPrompt } from "./openresponses-prompt";
      ...
      const content =
        Array.isArray(payloads) && payloads.length > 0
          ? payloads
              ...
              .filter(Boolean)
              .join("\n\n")
          : "No response from PowerDirector.";
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from PowerDirector.";
