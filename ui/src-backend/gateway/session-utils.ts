...
import { lookupContextTokens, resolveContextTokensForModel } from "../agents/context";
...
function listConfiguredAgentIds(cfg: PowerDirectorConfig): string[] {
...
export function listAgentsForGateway(cfg: PowerDirectorConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: PowerDirectorConfig): string {
...
function resolveSessionStoreAgentId(cfg: PowerDirectorConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: PowerDirectorConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: PowerDirectorConfig): GatewaySessionsDefaults {
