...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.POWERDIRECTOR_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.POWERDIRECTOR_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.POWERDIRECTOR_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.POWERDIRECTOR_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers POWERDIRECTOR_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
