...
import { normalizeModelRef, parseModelRef } from "../agents/model-selection";
import { primeConfiguredBindingRegistry } from "../channels/plugins/binding-registry";
...
import { getPluginRuntimeGatewayRequestScope } from "../plugins/runtime/gateway-request-scope";
import { setGatewaySubagentRuntime } from "../plugins/runtime/index";
import type { PluginRuntime } from "../plugins/runtime/types";
import { ADMIN_SCOPE, WRITE_SCOPE } from "./method-scopes";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "powerdirector.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "powerdirector.pluginSubagentOverridePolicyState",
);
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.powerdirector.ai/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadPowerDirectorPlugins({
    config: params.cfg,
