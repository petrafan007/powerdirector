import type { GatewayAuthConfig, PowerDirectorConfig } from "../config/config";
import { resolveSecretInputRef } from "../config/types.secrets";
import { resolveRequiredConfiguredSecretRefInputString } from "./resolve-configured-secret-input-string";

export function withGatewayAuthPassword(cfg: PowerDirectorConfig, password: string): PowerDirectorConfig {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
