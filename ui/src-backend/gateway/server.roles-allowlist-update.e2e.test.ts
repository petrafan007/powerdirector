...
import { connectGatewayClient } from './test-helpers.e2e';
import { connectOk, installGatewayTestHooks, onceMessage, rpcReq } from './test-helpers';
import { installConnectedControlUiServerSuite } from './test-with-server';
...
const connectNodeClient = async (params: {
  ...
  const token = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("POWERDIRECTOR_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".powerdirector", "restart-sentinel.json");
