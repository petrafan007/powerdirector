...
import { listAgentIds, resolveDefaultAgentId } from "../agents/agent-scope";
import { listChannelPlugins } from "../channels/plugins/index";
import type { ChannelId } from "../channels/plugins/types";
import type { PowerDirectorConfig } from "../config/config";
import { readJsonBodyWithLimit, requestBodyErrorToText } from "../infra/http-body";
...
import { type HookMappingResolved, resolveHookMappings } from "./hooks-mapping";
import { resolveAllowedAgentIds } from "./hooks-policy";
...
export function resolveHooksConfig(cfg: PowerDirectorConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: PowerDirectorConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-powerdirector-token"] === "string"
      ? req.headers["x-powerdirector-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-powerdirector-idempotency-key"]) ||
