...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/powerdirector/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (PowerDirector)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.POWERDIRECTOR_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "powerdirector");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.POWERDIRECTOR_TAILNET_DNS?.trim();
