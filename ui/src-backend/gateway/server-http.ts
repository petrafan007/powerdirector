...
import { normalizeCanvasScopedUrl } from "./canvas-capability";
import {
...
} from "./server/plugins-http";
import type { ReadinessChecker } from "./server/readiness";
import type { GatewayWsClient } from "./server/ws-types";
import { handleSessionKillHttpRequest } from "./session-kill-http";
import { handleSessionHistoryHttpRequest } from "./sessions-history-http";
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-PowerDirector-Token header (query parameters are not allowed).",
      );
      return true;
    }
...
export function createGatewayHttpServer(opts: {
  ...
  openAiChatCompletionsConfig?: import("../config/types.gateway").GatewayHttpChatCompletionsConfig;
