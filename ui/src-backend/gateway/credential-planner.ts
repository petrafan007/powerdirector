...
import { containsEnvVarReference } from "../config/env-substitution";
import { hasConfiguredSecretInput, resolveSecretInputRef } from "../config/types.secrets";
...
type GatewaySecretDefaults = NonNullable<PowerDirectorConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${POWERDIRECTOR_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["POWERDIRECTOR_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["POWERDIRECTOR_GATEWAY_PASSWORD", "CLAWDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
