...
import { deriveSessionTotalTokens, hasNonzeroUsage, normalizeUsage } from "../agents/usage";
import {
...
import { jsonUtf8Bytes } from "../infra/json-utf8-bytes";
import { hasInterSessionUserProvenance } from "../sessions/input-provenance";
import { stripInlineDirectiveTagsForDisplay } from "../utils/directive-tags";
...
export function attachPowerDirectorTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__powerdirector && typeof record.__powerdirector === "object" && !Array.isArray(record.__powerdirector)
      ? (record.__powerdirector as Record<string, unknown>)
      : {};
  return {
    ...record,
    __powerdirector: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachPowerDirectorTranscriptMeta(parsed.message, {
...
export function resolveSessionTranscriptCandidates(
  ...
  const home = resolveRequiredHomeDir(process.env, (typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function" ? ((typeof os.homedir === "function") ? os.homedir : (() => "")) : (() => "")));
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "powerdirector" && model === "delivery-mirror";
