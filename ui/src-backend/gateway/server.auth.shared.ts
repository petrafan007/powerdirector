...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  const deviceIdentityPath = nextAuthIdentityPath("powerdirector-auth-rate-limit");
  try {
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const deviceIdentityPath = nextAuthIdentityPath("powerdirector-auth-device");
...
export { ConnectErrorDetailCodes } from "./protocol/connect-error-details";
export { getHandshakeTimeoutMs } from "./server-constants";
export { PROTOCOL_VERSION } from "./protocol/index";
export { GATEWAY_CLIENT_MODES, GATEWAY_CLIENT_NAMES } from "../utils/message-channel";
