...
import { resolveSecretInputString } from "../secrets/resolve-secret-input-string";
import {
...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.POWERDIRECTOR_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.POWERDIRECTOR_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set POWERDIRECTOR_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: powerdirector doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__POWERDIRECTOR_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
