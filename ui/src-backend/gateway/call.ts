...
export function buildGatewayConnectionDetails(
  options: { config?: PowerDirectorConfig; url?: string; configPath?: string } = {},
  ...
  if (!isSecureWebSocketUrl(url) && process.env.POWERDIRECTOR_UPDATE_IN_PROGRESS !== "1") {
    throw new Error(
      [
...
function resolveGatewayCredentials(context: ResolvedGatewayCallContext): {
  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.POWERDIRECTOR_GATEWAY_TOKEN) ||
  ...
  const password =
    ...
    (!context.urlOverride
      ? trimToUndefined(process.env.POWERDIRECTOR_GATEWAY_PASSWORD) ||
