...
import { isRootHelpInvocation, isRootVersionInvocation } from "./cli/argv";
...
import { ensurePowerDirectorExecMarkerOnProcess } from "./infra/powerdirector-exec-env";
...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "powerdirector.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "powerdirector.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const { installGaxiosFetchCompat } = await import("./infra/gaxios-fetch-compat");
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.POWERDIRECTOR_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.POWERDIRECTOR_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.POWERDIRECTOR_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.POWERDIRECTOR_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[powerdirector] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version"), import("./infra/git-commit")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        const commit = resolveCommitHash({ moduleUrl: import.meta.url });
        console.log(commit ? `PowerDirector ${VERSION} (${commit})` : `PowerDirector ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[powerdirector] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[powerdirector] ${parsed.error}`);
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[powerdirector] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
...
function runMainOrRootHelp(argv: string[]): void {
  ...
  import("./cli/run-main")
    ...
    .catch((error) => {
      console.error(
        "[powerdirector] Failed to start CLI:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
