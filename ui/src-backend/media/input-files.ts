...
import { convertHeicToJpeg } from "./image-ops";
import { detectMime } from "./mime";
import { extractPdfContent, type PdfExtractedImage } from "./pdf-extract";
...
export async function fetchWithGuard(params: {
  ...
  const { response, release } = await fetchWithSsrFGuard({
    ...
    init: { headers: { "User-Agent": "PowerDirector-Gateway/1.0" } },
  });
