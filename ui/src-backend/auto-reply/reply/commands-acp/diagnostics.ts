...
import { resolveSessionStorePathForAcp } from "../../../acp/runtime/session-meta";
...
import { resolveAcpCommandBindingContext } from "./context";
import {
  ACP_DOCTOR_USAGE,
  ACP_INSTALL_USAGE,
  ACP_SESSIONS_USAGE,
...
import { resolveBoundAcpThreadSessionKey } from "./targets";
...
export async function handleAcpDoctorAction(
  ...
  try {
    ...
    lines.push(`next: powerdirector config set plugins.entries.${backendId}.enabled true`);
...
export function handleAcpInstallAction(
  ...
  const lines = [
    ...
    `run: ${installHint}`,
    `then: powerdirector config set plugins.entries.${backendId}.enabled true`,
    "then: /acp doctor",
  ];
  return stopWithText(lines.join("\n"));
}
