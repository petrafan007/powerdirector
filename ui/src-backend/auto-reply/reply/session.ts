...
import { resolveEffectiveResetTargetSessionKey } from "./acp-reset-target";
import { parseDiscordParentChannelFromSessionKey } from "./discord-parent-channel";
...
import { forkSessionFromParent, resolveParentForkMaxTokens } from "./session-fork";
import { buildSessionEndHookPayload, buildSessionStartHookPayload } from "./session-hooks";
  ...
  for (const trigger of resetTriggers) {
    ...
    if (trimmedBodyLower === triggerLower || strippedForResetLower === triggerLower) {
      if (shouldBypassAcpResetForTrigger(triggerLower)) {
        ...
        // normal PowerDirector session/transcript.
        break;
      }
      isNewSession = true;
      bodyStripped = "";
