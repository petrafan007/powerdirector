...
import { setPluginEnabledInConfig } from "../../plugins/toggle-config";
...
import { parsePluginsCommand } from "./plugins-commands";
  ...
  const lines = [
    ...
    ...report.plugins.map((plugin) => {
      const format = plugin.bundleFormat
        ? `${plugin.format ?? "powerdirector"}/${plugin.bundleFormat}`
        : (plugin.format ?? "powerdirector");
