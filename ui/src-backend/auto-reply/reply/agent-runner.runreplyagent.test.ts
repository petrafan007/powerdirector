...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { POWERDIRECTOR_STATE_DIR: process.env.POWERDIRECTOR_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.POWERDIRECTOR_STATE_DIR === undefined) {
      delete process.env.POWERDIRECTOR_STATE_DIR;
    } else {
      process.env.POWERDIRECTOR_STATE_DIR = snapshot.POWERDIRECTOR_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "powerdirector-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "powerdirector-memory-flush-"));
  });
