...
import { normalizeFastMode, normalizeUsageDisplay, resolveResponseUsageMode } from "../thinking";
import { isDiscordSurface, isTelegramSurface, resolveChannelAccountId } from "./channel-context";
import { handleAbortTrigger, handleStopCommand } from "./commands-session-abort";
import { persistSessionEntry } from "./commands-session-store";
import type { CommandHandler } from "./commands-types";
import { resolveTelegramConversationId } from "./telegram-context";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting PowerDirector in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPowerDirectorRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting PowerDirector via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
