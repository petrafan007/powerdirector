...
import { resolveActiveFallbackState } from "./fallback-state";
import { formatProviderModelRef, resolveSelectedAndActiveModel } from "./model-runtime";
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.powerdirector/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 PowerDirector ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: PowerDirectorConfig): string {
  const lines = ["ℹ️ Help", ""];
