...
import { runEmbeddedPiAgentMock } from "./reply.directive.directive-behavior.e2e-mocks";

export { loadModelCatalog } from "../agents/model-catalog";
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        POWERDIRECTOR_AGENT_DIR: (home) => path.join(home, ".powerdirector", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".powerdirector", "agent"),
      },
      prefix: "powerdirector-reply-",
    },
  );
}
