...
import { isValidEnvSecretRefId } from "../../../config/types.secrets";
import type { RuntimeEnv } from "../../../runtime";
import { resolveDefaultSecretProviderAlias } from "../../../secrets/ref-contract";
import { normalizeGatewayTokenInput, randomToken } from "../../onboard-helpers";
...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.POWERDIRECTOR_GATEWAY_TOKEN);
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like POWERDIRECTOR_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }
