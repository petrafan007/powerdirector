...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.powerdirector.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('powerdirector docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.powerdirector.ai/");
      runtime.log(`Search: ${formatCliCommand('powerdirector docs "your query"')}`);
    }
    return;
  }
