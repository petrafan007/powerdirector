...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.POWERDIRECTOR_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "PowerDirector update wizard" : "PowerDirector configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.powerdirector.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("powerdirector doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.POWERDIRECTOR_GATEWAY_TOKEN;
