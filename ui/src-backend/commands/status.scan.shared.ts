...
export function hasExplicitMemorySearchConfig(cfg: PowerDirectorConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: PowerDirectorConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: PowerDirectorConfig, agentId: string) => { store: { path: string } } | null;
