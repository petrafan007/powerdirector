...
import { redactSensitiveStatusSummary } from './status.summary';
import type { StatusSummary } from './status.types';
...
describe("redactSensitiveStatusSummary", () => {
  it("removes sensitive session and path details while preserving summary structure", () => {
    const input: StatusSummary = {
      ...
      sessions: {
        paths: ["/tmp/powerdirector/sessions.json"],
        count: 1,
        ...
        byAgent: [
          {
            agentId: "main",
            path: "/tmp/powerdirector/main-sessions.json",
            count: 1,
            recent: [createRecentSessionRow()],
          },
        ],
