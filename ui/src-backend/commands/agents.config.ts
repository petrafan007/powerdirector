...
function resolveAgentName(cfg: PowerDirectorConfig, agentId: string) {
...
function resolveAgentModel(cfg: PowerDirectorConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PowerDirectorConfig): AgentSummary[] {
