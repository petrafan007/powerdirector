...
beforeAll(() => {
  envSnapshot = captureEnv(["POWERDIRECTOR_PROFILE"]);
...
vi.mock("../infra/powerdirector-root.js", () => ({
  resolvePowerDirectorPackageRoot: vi.fn().mockResolvedValue("/tmp/powerdirector"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/powerdirector/pnpm-lock.yaml",
      markerPath: "/tmp/powerdirector/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("PowerDirector status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("powerdirector status --all") ||
          l.includes("powerdirector --profile isolated status --all") ||
          l.includes("powerdirector status --all") ||
          l.includes("powerdirector --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = "abcd1234";
    try {
