...
import { readConfigFileSnapshot, writeConfigFile, type PowerDirectorConfig } from "../config/config";
import { resolveSecretInputRef } from "../config/types.secrets";
import { shouldRequireGatewayTokenForInstall } from "../gateway/auth-install-policy";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("powerdirector config set gateway.auth.mode token")} or ${formatCliCommand("powerdirector config set gateway.auth.mode password")}.`,
  ].join(" ");
}
