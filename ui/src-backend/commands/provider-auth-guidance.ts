...
export function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`powerdirector models auth login --provider ${provider.id}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("powerdirector configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("powerdirector configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
