...
import { uninstallLegacySystemdUnits } from "../daemon/systemd";
...
import { resolveGatewayAuthTokenForService } from "./doctor-gateway-auth-token";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service POWERDIRECTOR_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (!tokenRefConfigured && !configuredGatewayToken && gatewayTokenForRepair) {
    const nextCfg: PowerDirectorConfig = {
      ...cfg,
      gateway: {
        ...cfg.gateway,
        auth: {
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing PowerDirector gateway next.");
      }
    }
  }
