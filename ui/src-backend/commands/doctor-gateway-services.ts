...
function resolveGatewayAuthToken(cfg: PowerDirectorConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.POWERDIRECTOR_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN;
