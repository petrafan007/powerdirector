...
function stripUnknownConfigKeys(config: PowerDirectorConfig): {
  config: PowerDirectorConfig;
  removed: string[];
} {
  const parsed = PowerDirectorSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: PowerDirectorConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("powerdirector doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
