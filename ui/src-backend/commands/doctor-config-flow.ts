...
import { CONFIG_PATH, migrateLegacyConfig, readConfigFileSnapshot } from "../config/config";
import { collectProviderDangerousNameMatchingScopes } from "../config/dangerous-name-matching";
import { formatConfigIssueLines } from "../config/issue-format";
import { applyPluginAutoEnable } from "../config/plugin-auto-enable";
import { parseToolsBySenderTypedKey } from "../config/types.tools";
import { resolveCommandResolutionFromArgv } from "../infra/exec-command-resolution";
import {
  listInterpreterLikeSafeBins,
  resolveMergedSafeBinProfileFixtures,
} from "../infra/exec-safe-bin-runtime-policy";
import {
  getTrustedSafeBinDirs,
  isTrustedSafeBinPath,
  normalizeTrustedSafeBinDirs,
} from "../infra/exec-safe-bin-trust";
import { readChannelAllowFromStore } from "../pairing/pairing-store";
import { resolveTelegramAccount } from "../plugin-sdk/account-resolution";
import {
  formatChannelAccountsDefaultPath,
  formatSetExplicitDefaultInstruction,
  formatSetExplicitDefaultToConfiguredInstruction,
} from "../routing/default-account-warnings";
import {
  DEFAULT_ACCOUNT_ID,
  normalizeAccountId,
  normalizeOptionalAccountId,
} from "../routing/session-key";
import { describeUnknownError } from "../secrets/shared";
import {
...
} from "../security/mutable-allowlist-detectors";
...
} from "./doctor-config-analysis";
import { normalizeCompatibilityConfigValues } from "./doctor-legacy-config";
...
export function collectMissingDefaultAccountBindingWarnings(cfg: PowerDirectorConfig): string[] {
...
export function collectMissingExplicitDefaultAccountWarnings(cfg: PowerDirectorConfig): string[] {
  const warnings: string[] = [];
...
function scanTelegramAllowFromUsernameEntries(cfg: PowerDirectorConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: PowerDirectorConfig): Promise<{
...
function scanDiscordNumericIdEntries(cfg: PowerDirectorConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: PowerDirectorConfig): {
  config: PowerDirectorConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: PowerDirectorConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: PowerDirectorConfig): {
  config: PowerDirectorConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: PowerDirectorConfig): Promise<{
  config: PowerDirectorConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: PowerDirectorConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    if (dmPolicy === "allowlist" && !hasAllowFromEntries(effectiveAllowFrom)) {
      warnings.push(
        `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("powerdirector doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
      );
    }
...
function collectExecSafeBinScopes(cfg: PowerDirectorConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: PowerDirectorConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: PowerDirectorConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: PowerDirectorConfig): {
  config: PowerDirectorConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: PowerDirectorConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: PowerDirectorConfig): {
  config: PowerDirectorConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("powerdirector doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("powerdirector doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("powerdirector doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("powerdirector doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("powerdirector doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("powerdirector doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("powerdirector doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "powerdirector doctor --fix" to remove these keys.');
    }
  }
