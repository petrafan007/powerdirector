...
import { isValidEnvSecretRefId, type SecretInput } from "../config/types.secrets";
import {
...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "POWERDIRECTOR_GATEWAY_TOKEN",
          placeholder: "POWERDIRECTOR_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like POWERDIRECTOR_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. PowerDirector will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: PowerDirector trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.powerdirector.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
