...
} from "../channels/plugins/setup-registry";
import type { ChannelSetupPlugin } from "../channels/plugins/setup-wizard-types";
import {
...
import { resolveChannelSetupEntries } from "./channel-setup/discovery";
import {
  ensureChannelSetupPluginInstalled,
  loadChannelSetupPluginRegistrySnapshotForChannel,
} from "./channel-setup/plugin-install";
import { resolveChannelSetupWizardAdapterForPlugin } from "./channel-setup/registry";
import type {
...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("powerdirector pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelSetupDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`powerdirector pairing approve ${policy.channel} <code>`)}`,
      ...
      if (adapter) {
        await prompter.note(
          `${channel} plugin not available (continuing with setup). If the channel still doesn't work after setup, run \`${formatCliCommand(
            "powerdirector plugins list",
          )}\` and \`${formatCliCommand("powerdirector plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
        await refreshStatus(channel);
        return true;
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("powerdirector channels add")}\``,
        },
      ],
