...
import type { AgentModelConfig } from "../../config/types.agents-shared";
...
export async function loadValidConfigOrThrow(): Promise<PowerDirectorConfig> {
...
export async function updateConfig(
  mutator: (cfg: PowerDirectorConfig) => PowerDirectorConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: PowerDirectorConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: PowerDirectorConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("powerdirector agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
