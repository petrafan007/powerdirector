...
import type { StatusScanResult } from "./status.scan";
import {
...
} from "./status.scan.shared";
import { getStatusSummary } from "./status.summary";
import { getUpdateCheckResult } from "./status.update";

let pluginRegistryModulePromise: Promise<typeof import("../cli/plugin-registry")> | undefined;
let configIoModulePromise: Promise<typeof import("../config/io")> | undefined;
let commandSecretTargetsModulePromise:
  | Promise<typeof import("../cli/command-secret-targets")>
  | undefined;
let commandSecretGatewayModulePromise:
  | Promise<typeof import("../cli/command-secret-gateway")>
  | undefined;
let memorySearchModulePromise: Promise<typeof import("../agents/memory-search")> | undefined;
let statusScanDepsRuntimeModulePromise:
  | Promise<typeof import("./status.scan.deps.runtime")>
  | undefined;

function loadPluginRegistryModule() {
  pluginRegistryModulePromise ??= import("../cli/plugin-registry");
...
function loadCommandSecretTargetsModule() {
  commandSecretTargetsModulePromise ??= import("../cli/command-secret-targets");
  return commandSecretTargetsModulePromise;
}

function loadCommandSecretGatewayModule() {
  commandSecretGatewayModulePromise ??= import("../cli/command-secret-gateway");
  return commandSecretGatewayModulePromise;
}

function loadMemorySearchModule() {
  memorySearchModulePromise ??= import("../agents/memory-search");
  return memorySearchModulePromise;
}

function loadStatusScanDepsRuntimeModule() {
  statusScanDepsRuntimeModulePromise ??= import("./status.scan.deps.runtime");
...
async function readStatusSourceConfig(): Promise<PowerDirectorConfig> {
...
}): Promise<{ resolvedConfig: PowerDirectorConfig; diagnostics: string[] }> {
  if (!shouldSkipMissingConfigFastPath() && !existsSync(resolveConfigPath(process.env))) {
