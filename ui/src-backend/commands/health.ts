...
import { inspectReadOnlyChannelAccount } from "../channels/read-only-account-inspect";
import { withProgress } from "../cli/progress";
import type { PowerDirectorConfig } from "../config/config";
import { loadConfig, readBestEffortConfig } from "../config/config";
...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.POWERDIRECTOR_DEBUG_HEALTH)) {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: PowerDirectorConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: PowerDirectorConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.POWERDIRECTOR_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
