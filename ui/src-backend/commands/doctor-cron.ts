...
import { normalizeStoredCronJobs } from "../cron/store-migration";
import { resolveCronStorePath, loadCronStore, saveCronStore } from "../cron/store";
import type { CronJob } from "../cron/types";
import { note } from "../terminal/note";
import { shortenHomePath } from "../utils";
import type { DoctorPrompter, DoctorOptions } from "./doctor-prompter";
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  note(
    [
      ...
      ...previewLines,
      `Repair with ${formatCliCommand("powerdirector doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
