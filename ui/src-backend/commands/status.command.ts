...
import { groupChannelIssuesByChannel } from "./status-all/channel-issues";
...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByPowerDirector ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByPowerDirector ? "installed · " : "";
  ...
  runtime.log(theme.heading("PowerDirector status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`powerdirector devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("powerdirector devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("powerdirector devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("powerdirector security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("powerdirector security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.powerdirector.ai/faq");
  runtime.log("Troubleshooting: https://docs.powerdirector.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("powerdirector status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("powerdirector logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("powerdirector status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("powerdirector gateway probe")}`);
  }
}
