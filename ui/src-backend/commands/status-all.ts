...
import { getStatusCommandSecretTargetIds } from "../cli/command-secret-targets";
...
import { inspectPortUsage } from "../infra/ports";
import { readRestartSentinel } from "../infra/restart-sentinel";
import { getRemoteSkillEligibility } from "../infra/skills-remote";
import { readTailscaleStatusJson } from "../infra/tailscale";
import { normalizeUpdateChannel, resolveUpdateChannelDisplay } from "../infra/update-channels";
import { checkUpdateStatus, formatGitInstallLabel } from "../infra/update-check";
import { buildPluginCompatibilityNotices } from "../plugins/status";
import { runExec } from "../process/exec";
import type { RuntimeEnv } from "../runtime";
import { VERSION } from "../version";
import { resolveControlUiLinks } from "./onboard-helpers";
import { getAgentLocalStatuses } from "./status-all/agents";
import { buildChannelsTable } from "./status-all/channels";
import { formatDurationPrecise, formatGatewayAuthUsed } from "./status-all/format";
import { pickGatewaySelfPresence } from "./status-all/gateway";
import { buildStatusAllReportLines } from "./status-all/report-lines";
import { readServiceStatusSummary } from "./status.service-summary";
import { formatUpdateOneLiner } from "./status.update";
  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const overviewRows = [
      ...
      ...(probeAuthResolution.warning
        ...
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("powerdirector security audit --deep")}` },
      gatewaySelfLine
      ...
      daemon
        ? {
            ...
            Value: !daemon.installed
              ...
              : `${daemon.label} ${daemon.managedByPowerDirector ? "installed · " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` · ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`,
          }
      ...
      nodeService
        ? {
            ...
            Value: !nodeService.installed
              ...
              : `${nodeService.label} ${nodeService.managedByPowerDirector ? "installed · " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` · ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`,
          }
