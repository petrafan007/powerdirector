...
import { applyPrimaryModel } from "../plugins/provider-model-primary";
...
export { applyPrimaryModel } from "../plugins/provider-model-primary";
...
type PromptDefaultModelResult = { model?: string; config?: PowerDirectorConfig };
type PromptModelAllowlistResult = { models?: string[] };

async function loadModelPickerRuntime() {
  return import("./model-picker.runtime");
}
...
function resolveConfiguredModelRaw(cfg: PowerDirectorConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: PowerDirectorConfig): string[] {
...
export function applyModelAllowlist(cfg: PowerDirectorConfig, models: string[]): PowerDirectorConfig {
