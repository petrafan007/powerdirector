...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: PowerDirectorConfig): string[] {
...
export async function noteSecurityWarnings(cfg: PowerDirectorConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("powerdirector security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.powerdirector/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("powerdirector approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.powerdirector.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("powerdirector configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("powerdirector config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("powerdirector doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "powerdirector config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("powerdirector config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('powerdirector config set session.dmScope "per-channel-peer"') +
