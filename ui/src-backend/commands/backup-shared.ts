...
import { formatSessionArchiveTimestamp } from "../config/sessions/artifacts";
import { pathExists, shortenHomePath } from "../utils";
import { buildCleanupPlan, isPathWithin } from "./cleanup-utils";
...
export function buildBackupArchiveRoot(nowMs = Date.now()): string {
  return `${formatSessionArchiveTimestamp(nowMs)}-powerdirector-backup`;
}
...
export async function resolveBackupPlanFromDisk(
  ...
  if (includeWorkspace && configSnapshot.exists && !configSnapshot.valid) {
    throw new Error(
      `Config invalid at ${shortenHomePath(configSnapshot.path)}. PowerDirector cannot reliably discover custom workspaces for backup. Fix the config or rerun with --no-include-workspace for a partial backup.`,
    );
  }
