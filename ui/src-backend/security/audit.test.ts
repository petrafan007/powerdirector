...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: PowerDirectorConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withStateDir = async (label: string, fn: (tmp: string) => Promise<void>) => {
    ...
    process.env.POWERDIRECTOR_STATE_DIR = tmp;
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.POWERDIRECTOR_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.POWERDIRECTOR_GATEWAY_PASSWORD;
      } else {
        process.env.POWERDIRECTOR_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env password over remote password", async () => {
      process.env.POWERDIRECTOR_GATEWAY_PASSWORD = "env-pass";
      const { probeGatewayFn, getAuth } = makeProbeCapture();
      const cfg: PowerDirectorConfig = {
        gateway: {
          mode: "remote",
