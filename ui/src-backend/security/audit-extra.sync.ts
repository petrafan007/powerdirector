import { resolveSandboxConfigForAgent } from "../agents/sandbox/config";
import { isDangerousNetworkMode, normalizeNetworkMode } from "../agents/sandbox/network-mode";
/**
 * Synchronous security audit collector functions.
 *
 * These functions analyze config-based security properties without I/O.
 */
import { resolveSandboxToolPolicyForAgent } from "../agents/sandbox/tool-policy";
...
import { isToolAllowedByPolicies } from "../agents/tool-policy-match";
...
import { resolveAllowedAgentIds } from "../gateway/hooks-policy";
import {
...
function summarizeGroupPolicy(cfg: PowerDirectorConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: PowerDirectorConfig): boolean {
...
function collectModels(cfg: PowerDirectorConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: PowerDirectorConfig): Set<string> {
  const baseCfg: PowerDirectorConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: PowerDirectorConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: PowerDirectorConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: PowerDirectorConfig): boolean {
...
function isBrowserEnabled(cfg: PowerDirectorConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: PowerDirectorConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: PowerDirectorConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: PowerDirectorConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep POWERDIRECTOR_STATE_DIR on a local-only volume and re-run "${formatCliCommand("powerdirector security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer POWERDIRECTOR_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const powerdirectorGatewayToken =
    typeof env.POWERDIRECTOR_GATEWAY_TOKEN === "string" && env.POWERDIRECTOR_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-powerdirector-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: powerdirector-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "PowerDirector's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
