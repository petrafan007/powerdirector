...
import { redactCdpUrl } from "../browser/cdp.helpers";
...
import type { ConfigFileSnapshot, PowerDirectorConfig } from "../config/config";
...
import { normalizeTrustedSafeBinDirs } from "../infra/exec-safe-bin-trust";
import { isBlockedHostnameOrIp, isPrivateNetworkAllowedByPolicy } from "../infra/net/ssrf";
import {
...
type ExecDockerRawFn = typeof import("../agents/sandbox/docker").execDockerRaw;
type ProbeGatewayFn = typeof import("../gateway/probe").probeGateway;
...
let channelPluginsModulePromise: Promise<typeof import("../channels/plugins/index")> | undefined;
let auditNonDeepModulePromise: Promise<typeof import("./audit.nondeep.runtime")> | undefined;
let auditDeepModulePromise: Promise<typeof import("./audit.deep.runtime")> | undefined;
let auditChannelModulePromise:
  | Promise<typeof import("./audit-channel.collect.runtime")>
  | undefined;
let gatewayProbeDepsPromise:
  | Promise<{
      buildGatewayConnectionDetails: typeof import("../gateway/call").buildGatewayConnectionDetails;
      resolveGatewayProbeAuthSafe: typeof import("../gateway/probe-auth").resolveGatewayProbeAuthSafe;
      probeGateway: typeof import("../gateway/probe").probeGateway;
    }>
  | undefined;

async function loadChannelPlugins() {
  channelPluginsModulePromise ??= import("../channels/plugins/index");
  return await channelPluginsModulePromise;
}

async function loadAuditNonDeepModule() {
  auditNonDeepModulePromise ??= import("./audit.nondeep.runtime");
  return await auditNonDeepModulePromise;
}

async function loadAuditDeepModule() {
  auditDeepModulePromise ??= import("./audit.deep.runtime");
  return await auditDeepModulePromise;
}

async function loadAuditChannelModule() {
  auditChannelModulePromise ??= import("./audit-channel.collect.runtime");
...
function isFeishuDocToolEnabled(cfg: PowerDirectorConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your PowerDirector state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your PowerDirector state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.POWERDIRECTOR_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.POWERDIRECTOR_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("powerdirector security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.POWERDIRECTOR_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: PowerDirectorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("powerdirector status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("powerdirector security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set POWERDIRECTOR_GATEWAY_TOKEN/POWERDIRECTOR_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("powerdirector security audit --deep")}".`,
    });
  }
