...
import { ignoreCiaoCancellationRejection } from './bonjour-ciao';
import { formatBonjourError } from './bonjour-errors';
import { isTruthyEnvValue } from './env';
import { registerUnhandledRejectionHandler } from './unhandled-rejections';
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.POWERDIRECTOR_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "PowerDirector";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(PowerDirector\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.POWERDIRECTOR_MDNS_HOSTNAME?.trim() ||
