...
import { resolveCronSession } from "../cron/isolated-agent/session";
...
import { formatErrorMessage, hasErrnoCode } from "./errors";
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: PowerDirectorConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: PowerDirectorConfig) {
...
function resolveHeartbeatAgents(cfg: PowerDirectorConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: PowerDirectorConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: PowerDirectorConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: PowerDirectorConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
