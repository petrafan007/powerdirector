...
import { writeJsonAtomic } from "./json-files";
...
import { compareSemverStrings, resolveNpmChannelTag, checkUpdateStatus } from "./update-check";
...
async function runAutoUpdateCommand(params: {
  ...
  if (argv.length === 0) {
    argv.push("powerdirector", ...baseArgs);
  }
...
export async function runGatewayUpdateCheck(params: {
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("powerdirector update")}`,
      );
