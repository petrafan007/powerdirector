...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.POWERDIRECTOR_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.POWERDIRECTOR_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.POWERDIRECTOR_TEST_TAILSCALE_BINARY;
    } else {
      process.env.POWERDIRECTOR_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
