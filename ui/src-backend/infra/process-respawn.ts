...
import { scheduleDetachedLaunchdRestartHandoff } from "../daemon/launchd-restart-handoff";
import { triggerPowerDirectorRestart } from "./restart";
import { detectRespawnSupervisor } from "./supervisor-markers";
 ...
 * - POWERDIRECTOR_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.POWERDIRECTOR_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerPowerDirectorRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
