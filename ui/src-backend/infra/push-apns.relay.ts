...
import type { GatewayConfig } from "../config/types.gateway";
import {
...
const GATEWAY_DEVICE_ID_HEADER = "x-powerdirector-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-powerdirector-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-powerdirector-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.POWERDIRECTOR_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "POWERDIRECTOR_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or POWERDIRECTOR_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.POWERDIRECTOR_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require POWERDIRECTOR_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.POWERDIRECTOR_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
