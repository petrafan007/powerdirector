...
describe("installFromNpmSpecArchive", () => {
  ...
  it("returns resolution metadata and installer result on success", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/powerdirector-test.tgz",
      metadata: {
        name: "@powerdirector/test",
        version: "1.0.0",
    ...
    expect(result.npmResolution.resolvedSpec).toBe("@powerdirector/test@1.0.0");
  ...
  it("returns installer failures to callers for domain-specific handling", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/powerdirector-test.tgz",
      metadata: { resolvedSpec: "@powerdirector/test@1.0.0", integrity: "sha512-same" },
    });
