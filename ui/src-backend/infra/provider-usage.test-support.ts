...
import { createProviderUsageFetch } from "../test-utils/provider-usage-fetch";
import type { ProviderAuth } from "./provider-usage.auth";
import type { UsageSummary } from "./provider-usage.types";
...
export async function loadUsageWithAuth<T extends ProviderUsageLoader>(
  ...
  return await loadProviderUsageSummary({
    ...
    config: { plugins: { enabled: false } } as PowerDirectorConfig,
  });
}
