...
import { createAsyncLock, readJsonFile, writeJsonAtomic } from './json-files';
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.POWERDIRECTOR_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.POWERDIRECTOR_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.POWERDIRECTOR_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.POWERDIRECTOR_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.POWERDIRECTOR_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set POWERDIRECTOR_APNS_PRIVATE_KEY_P8 or POWERDIRECTOR_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading POWERDIRECTOR_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createPowerDirectorPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    powerdirector: createPowerDirectorPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
