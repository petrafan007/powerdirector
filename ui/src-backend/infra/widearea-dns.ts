...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.POWERDIRECTOR_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*powerdirector-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "powerdirector";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "powerdirector");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "powerdirector-gw");
  ...
  records.push(`_powerdirector-gw._tcp IN PTR ${instanceLabel}._powerdirector-gw._tcp`);
  records.push(`${instanceLabel}._powerdirector-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._powerdirector-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; powerdirector-content-hash: ${contentHash}\n${contentBody}`;
}
