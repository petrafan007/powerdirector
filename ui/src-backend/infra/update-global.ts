...
import { applyPathPrepend } from "./path-prepend";
...
const PRIMARY_PACKAGE_NAME = "powerdirector";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const POWERDIRECTOR_MAIN_PACKAGE_SPEC = "github:powerdirector/powerdirector#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "PowerDirector", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.POWERDIRECTOR_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.POWERDIRECTOR_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return POWERDIRECTOR_MAIN_PACKAGE_SPEC;
  }
