...
describe("device identity state dir defaults", () => {
  ...
  it("writes the default identity file under POWERDIRECTOR_STATE_DIR", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-identity-state-"));
