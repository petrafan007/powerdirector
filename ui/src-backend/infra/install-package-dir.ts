...
import { fileExists } from "./archive";
import { assertCanonicalPathWithinBase } from "./install-safe-path";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".powerdirector-install-stage-"));
  ...
  if (params.mode === "update" && (await fileExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".powerdirector-install-backups");
