...
} from "../commands/backup-shared";
import { isPathWithin } from "../commands/cleanup-utils";
import { resolveHomeDir, resolveUserPath } from "../utils";
...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No PowerDirector config file was found to back up."
        : "No local PowerDirector state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), "powerdirector-backup-"));
