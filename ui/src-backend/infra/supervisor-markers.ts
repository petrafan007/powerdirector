const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "POWERDIRECTOR_LAUNCHD_LABEL"],
  systemd: ["POWERDIRECTOR_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["POWERDIRECTOR_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "POWERDIRECTOR_SERVICE_MARKER",
  "POWERDIRECTOR_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.POWERDIRECTOR_SERVICE_MARKER?.trim();
    const serviceKind = env.POWERDIRECTOR_SERVICE_KIND?.trim();
