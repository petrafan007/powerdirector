...
import { SafeOpenError, openFileWithinRoot, readLocalFileSafely } from './fs-safe';
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await makeTempDir("powerdirector-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await makeTempDir("powerdirector-fs-safe-root-");
    const outside = await makeTempDir("powerdirector-fs-safe-outside-");
