...
import { resolveChannelAllowFromPath } from "../pairing/pairing-store";
import {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set POWERDIRECTOR_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.POWERDIRECTOR_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
