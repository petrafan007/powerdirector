...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __powerdirectorDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__powerdirectorDiagnosticEventsState) {
    globalStore.__powerdirectorDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__powerdirectorDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: PowerDirectorConfig): boolean {
