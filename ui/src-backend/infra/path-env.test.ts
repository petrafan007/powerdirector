...
describe("ensurePowerDirectorCliOnPath", () => {
  const envKeys = [
    "PATH",
    "POWERDIRECTOR_PATH_BOOTSTRAPPED",
    "POWERDIRECTOR_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  it("prepends the bundled app bin dir when a sibling powerdirector exists", () => {
    const tmp = abs("/tmp/powerdirector-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "powerdirector");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.POWERDIRECTOR_PATH_BOOTSTRAPPED;
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/powerdirector-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "powerdirector");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/powerdirector-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "powerdirector");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "powerdirector");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/powerdirector-path/case-linuxbrew");
