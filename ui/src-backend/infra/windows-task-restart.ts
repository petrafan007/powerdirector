...
import { resolveGatewayWindowsTaskName } from "../daemon/constants";
import type { RestartAttempt } from "./restart";
import { resolvePreferredPowerDirectorTmpDir } from "./tmp-powerdirector-dir";
...
export function relaunchGatewayScheduledTask(env: NodeJS.ProcessEnv = process.env): RestartAttempt {
  ...
  const scriptPath = path.join(
    ...
    `powerdirector-schtasks-restart-${randomUUID()}.cmd`,
  );
