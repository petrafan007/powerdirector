...
describe("shared/frontmatter", () => {
  ...
  test("resolvePowerDirectorManifestBlock parses JSON5 metadata and picks powerdirector block", () => {
    const frontmatter = {
      metadata: "{ powerdirector: { foo: 1, bar: 'baz' } }",
    };
    expect(resolvePowerDirectorManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });
