...
describe("oauth paths", () => {
  it("prefers POWERDIRECTOR_OAUTH_DIR over POWERDIRECTOR_STATE_DIR", () => {
    const env = {
      POWERDIRECTOR_OAUTH_DIR: "/custom/oauth",
  ...
  it("derives oauth path from POWERDIRECTOR_STATE_DIR when unset", () => {
    const env = {
      POWERDIRECTOR_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("state + config path candidates", () => {
  function expectPowerDirectorHomeDefaults(env: NodeJS.ProcessEnv): void {
    ...
    if (!configuredHome) {
      throw new Error("POWERDIRECTOR_HOME must be set for this assertion helper");
    }
  ...
  it("uses POWERDIRECTOR_STATE_DIR when set", () => {
    const env = {
      POWERDIRECTOR_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;

    expect(resolveStateDir(env, () => "/home/test")).toBe(path.resolve("/new/state"));
  });

  it("uses POWERDIRECTOR_HOME for default state/config locations", () => {
    const env = {
      POWERDIRECTOR_HOME: "/srv/powerdirector-home",
    } as NodeJS.ProcessEnv;
    expectPowerDirectorHomeDefaults(env);
  });

  it("prefers POWERDIRECTOR_HOME over HOME for default state/config locations", () => {
    const env = {
      POWERDIRECTOR_HOME: "/srv/powerdirector-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv;
  ...
  it("prefers ~/.powerdirector when it exists and legacy dir is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-state-"));
    try {
  ...
  it("CONFIG_PATH prefers existing config when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-config-"));
    try {
  ...
  it("respects state dir overrides when config is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-config-override-"));
    try {
      const legacyDir = path.join(root, ".powerdirector");
      await fs.mkdir(legacyDir, { recursive: true });
