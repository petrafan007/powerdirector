...
import { maintainConfigBackups } from "./backup-rotation";
import {
...
import { normalizeExecSafeBinProfilesInConfig } from "./normalize-exec-safe-bin";
...
import { isBlockedObjectKey } from "./prototype-keys";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "POWERDIRECTOR_GATEWAY_PASSWORD",
];
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  powerdirector config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  powerdirector config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: PowerDirectorConfig,
  pathSegments: string[],
): { changed: boolean; next: PowerDirectorConfig } {
...
function coerceConfig(value: unknown): PowerDirectorConfig {
...
function stampConfigVersion(cfg: PowerDirectorConfig): PowerDirectorConfig {
...
function warnIfConfigFromFuture(cfg: PowerDirectorConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = comparePowerDirectorVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer PowerDirector (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as PowerDirectorConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PowerDirectorConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PowerDirectorConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PowerDirectorConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as PowerDirectorConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.POWERDIRECTOR_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.POWERDIRECTOR_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.POWERDIRECTOR_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.POWERDIRECTOR_WATCH_SESSION === "string" &&
        deps.env.POWERDIRECTOR_WATCH_SESSION.trim().length > 0
          ? deps.env.POWERDIRECTOR_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.POWERDIRECTOR_WATCH_COMMAND === "string" &&
        deps.env.POWERDIRECTOR_WATCH_COMMAND.trim().length > 0
          ? deps.env.POWERDIRECTOR_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `POWERDIRECTOR_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: PowerDirectorConfig | null = null;
let runtimeConfigSourceSnapshot: PowerDirectorConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.POWERDIRECTOR_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.POWERDIRECTOR_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): PowerDirectorConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): PowerDirectorConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: PowerDirectorConfig;
  candidate: PowerDirectorConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: PowerDirectorConfig): PowerDirectorConfig {
...
export function loadConfig(): PowerDirectorConfig {
...
export async function readBestEffortConfig(): Promise<PowerDirectorConfig> {
...
export async function writeConfigFile(
  ...
  // edits to powerdirector.json remain visible (no stale snapshot).
}
