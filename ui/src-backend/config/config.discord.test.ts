...
import { loadConfig, validateConfigObject } from './config';
...
describe("config discord", () => {
  ...
  it("loads discord guild map + dm group settings", async () => {
    await withTempHome(async (home) => {
            ...
            channels: {
              discord: {
                ...
                dm: {
                  ...
                  allowFrom: ["steipete"],
                  groupEnabled: true,
                  groupChannels: ["powerdirector-dm"],
                },
                actions: {
                  emojiUploads: true,
      ...
      expect(cfg.channels?.discord?.dm?.groupChannels).toEqual(["powerdirector-dm"]);
      ...
      expect(cfg.channels?.discord?.guilds?.["123"]?.slug).toBe("friends-of-powerdirector");
