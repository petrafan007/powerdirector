...
import type { ChannelPlugin } from "../channels/plugins/index";
import { resolvePowerDirectorPackageRootSync } from "../infra/powerdirector-root";
import { loadPluginManifestRegistry } from "../plugins/manifest-registry";
import { FIELD_HELP } from "./schema.help";
import { buildConfigSchema, type ConfigSchemaResponse } from "./schema";
import { findWildcardHintMatch, schemaHasChildren } from "./schema.shared";
...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.POWERDIRECTOR_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolvePowerDirectorPackageRootSync({
...
function loadPackageChannelMetadata(rootDir: string): PackageChannelMetadata | null {
  try {
    ...
    const channel = packageJson.powerdirector?.channel;
    if (!channel) {
      return null;
    }
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const loadChannelsSequentiallyForDebug = process.env.POWERDIRECTOR_CONFIG_DOC_BASELINE_DEBUG === "1";
