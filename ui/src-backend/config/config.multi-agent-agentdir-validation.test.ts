...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "powerdirector-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.powerdirector/agents/shared/agent" },
                { id: "b", agentDir: "~/.powerdirector/agents/shared/agent" },
              ],
            },
