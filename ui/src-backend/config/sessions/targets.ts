...
} from "../../agents/session-dirs";
import { DEFAULT_AGENT_ID, normalizeAgentId } from "../../routing/session-key";
import { resolveStateDir } from "../paths";
import type { PowerDirectorConfig } from "../types.powerdirector";
import { resolveAgentsDirFromSessionStorePath, resolveStorePath } from "./paths";
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "powerdirector agents list" to see configured agents.`,
      );
    }
    return [
      {
