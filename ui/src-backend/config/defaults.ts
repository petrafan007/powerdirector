...
import { normalizeProviderId, parseModelRef } from "../agents/model-selection";
import { DEFAULT_AGENT_MAX_CONCURRENT, DEFAULT_SUBAGENT_MAX_CONCURRENT } from "./agent-limits";
import { resolveAgentModelPrimaryValue } from "./model-input";
import {
...
import { hasConfiguredSecretInput } from "./types.secrets";
...
function resolveAnthropicDefaultAuthMode(cfg: PowerDirectorConfig): AnthropicAuthDefaultsMode | null {
...
export function applyMessageDefaults(cfg: PowerDirectorConfig): PowerDirectorConfig {
...
export function applyTalkApiKey(config: PowerDirectorConfig): PowerDirectorConfig {
...
export function applyTalkConfigNormalization(config: PowerDirectorConfig): PowerDirectorConfig {
  return normalizeTalkConfig(config);
}

export function applyModelDefaults(cfg: PowerDirectorConfig): PowerDirectorConfig {
  let mutated = false;
  let nextCfg = cfg;
...
export function applyAgentDefaults(cfg: PowerDirectorConfig): PowerDirectorConfig {
...
export function applyLoggingDefaults(cfg: PowerDirectorConfig): PowerDirectorConfig {
...
export function applyContextPruningDefaults(cfg: PowerDirectorConfig): PowerDirectorConfig {
...
export function applyCompactionDefaults(cfg: PowerDirectorConfig): PowerDirectorConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
