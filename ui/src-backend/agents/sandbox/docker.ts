...
} from "../../plugin-sdk/windows-spawn";
import { sanitizeEnvVars } from "./sanitize-env-vars";
import type { EnvSanitizationOptions } from "./sanitize-env-vars";
...
import { markPowerDirectorExecEnv } from "../../infra/powerdirector-exec-env";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "powerdirector.sandbox=1");
  args.push("--label", `powerdirector.sessionKey=${params.scopeKey}`);
  args.push("--label", `powerdirector.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `powerdirector.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markPowerDirectorExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "powerdirector.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`powerdirector sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`powerdirector sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("powerdirector sandbox recreate --all");
}
