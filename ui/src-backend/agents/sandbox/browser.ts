...
import { DEFAULT_SANDBOX_BROWSER_IMAGE, SANDBOX_BROWSER_SECURITY_HASH_EPOCH } from "./constants";
import {
...
import { validateNetworkMode } from "./validate-sandbox-security";
import { appendWorkspaceMountArgs } from "./workspace-mounts";

const HOT_BROWSER_WINDOW_MS = 5 * 60 * 1000;
const CDP_SOURCE_RANGE_ENV_KEY = "POWERDIRECTOR_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_POWERDIRECTOR_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "powerdirector.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `powerdirector sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `powerdirector sandbox recreate --browser --agent ${agentId}`;
          }
          return "powerdirector sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        "powerdirector.sandboxBrowser": "1",
        "powerdirector.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `POWERDIRECTOR_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `POWERDIRECTOR_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `POWERDIRECTOR_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `POWERDIRECTOR_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `POWERDIRECTOR_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "POWERDIRECTOR_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME)
    : null;
