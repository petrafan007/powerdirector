...
vi.mock("../../infra/boundary-file-read", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../infra/boundary-file-read")>();
  return {
    ...actual,
...
import { openBoundaryFile } from "../../infra/boundary-file-read";
import { execDockerRaw } from "./docker";
import * as fsBridgeModule from "./fs-bridge";
...
export async function withTempDir<T>(
  ...
  const stateDir = await fs.mkdtemp(path.join(((typeof ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp")) === "function") ? ((typeof os.tmpdir === "function") ? os.tmpdir : (() => "/tmp"))() : "/tmp"), prefix));
  try {
...
export function installDockerReadMock(params?: { canonicalPath?: string }) {
  ...
  mockedExecDockerRaw.mockImplementation(async (args) => {
    ...
    if (script.includes("mktemp")) {
      return dockerExecResult("/workspace/.powerdirector-write-b.txt.ABC123\n");
    }
...
export async function expectMkdirpAllowsExistingDirectory(params?: {
  ...
  await withTempDir("powerdirector-fs-bridge-mkdirp-", async (stateDir) => {
    const workspaceDir = path.join(stateDir, "workspace");
