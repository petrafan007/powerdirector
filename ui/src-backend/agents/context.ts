...
import { computeBackoff, type BackoffPolicy } from "../infra/backoff";
import { consumeRootOptionToken, FLAG_TERMINATOR } from "../infra/cli-root-options";
...
let configuredConfig: PowerDirectorConfig | undefined;
...
function primeConfiguredContextWindows(): PowerDirectorConfig | undefined {
  if (configuredConfig) {
  ...
  loadPromise = (async () => {
    ...
    try {
      const { discoverAuthStorage, discoverModels } =
        await import("./pi-model-discovery-runtime");
