...
import { isSecretRefHeaderValueMarker } from "../model-auth-markers";
...
} from "../model-suppression";
import { discoverAuthStorage, discoverModels } from "../pi-model-discovery";
import { normalizeResolvedProviderModel } from "./model.provider-normalization";
...
export function resolveModel(
  ...
  const resolvedAgentDir = agentDir ?? resolvePowerDirectorAgentDir();
 ...
 * a hint.
 *
 * See: https://github.com/powerdirector/powerdirector/issues/17328
 */
const LOCAL_PROVIDER_HINTS: Record<string, string> = {
  ollama:
    "Ollama requires authentication to be registered as a provider. " +
    'Set OLLAMA_API_KEY="ollama-local" (any value works) or run "powerdirector configure". ' +
    "See: https://docs.powerdirector.ai/providers/ollama",
  vllm:
    "vLLM requires authentication to be registered as a provider. " +
    'Set VLLM_API_KEY (any value works) or run "powerdirector configure". ' +
    "See: https://docs.powerdirector.ai/providers/vllm",
};
