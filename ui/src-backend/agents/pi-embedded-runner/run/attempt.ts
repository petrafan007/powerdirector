...
} from "../../../infra/net/undici-global-dispatcher";
import { MAX_IMAGE_BYTES } from "../../../media/constants";
import { resolveSignalReactionLevel } from "../../../plugin-sdk/signal";
...
import { joinPresentTextSegments } from "../../../shared/text/join-segments";
...
import { ensureCustomApiRegistered } from "../../custom-api-registry";
...
import { resolveToolCallArgumentsEncoding } from "../../model-compat";
import { normalizeProviderId, resolveDefaultModelForAgent } from "../../model-selection";
import { supportsModelTools } from "../../model-tool-support";
import { createConfiguredOllamaStreamFn } from "../../ollama-stream";
import { createOpenAIWebSocketStreamFn, releaseWsSession } from "../../openai-ws-stream";
import { resolveOwnerDisplaySetting } from "../../owner-display";
import { createBundleLspToolRuntime } from "../../pi-bundle-lsp-runtime";
import { createBundleMcpToolRuntime } from "../../pi-bundle-mcp-tools";
import {
...
import { createPreparedEmbeddedPiSettingsManager } from "../../pi-project-settings";
import { applyPiAutoCompactionGuard } from "../../pi-settings";
...
import { sanitizeToolCallIdsForCloudCodeAssist } from "../../tool-call-id";
import { resolveEffectiveToolFsWorkspaceOnly } from "../../tool-fs-policy";
...
import type { CompactEmbeddedPiSessionParams } from "../compact";
import { buildEmbeddedCompactionRuntimeContext } from "../compaction-runtime-context";
import { resolveCompactionTimeoutMs } from "../compaction-safety-timeout";
import { buildEmbeddedExtensionFactories } from "../extensions";
...
import { buildEmbeddedMessageActionDiscoveryInput } from "../message-action-discovery-input";
...
import { resolveEmbeddedRunSkillEntries } from "../skills-runtime";
import {
  applySystemPromptOverrideToSession,
  buildEmbeddedSystemPrompt,
  createSystemPromptOverride,
} from "../system-prompt";
import { dropThinkingBlocks } from "../thinking";
import { collectAllowedToolNames } from "../tool-name-allowlist";
...
import { waitForCompactionRetryWithAggregateTimeout } from "./compaction-retry-aggregate-timeout";
import {
...
import { pruneProcessedHistoryImages } from "./history-image-prune";
...
const SESSIONS_YIELD_INTERRUPT_CUSTOM_TYPE = "powerdirector.sessions_yield_interrupt";
const SESSIONS_YIELD_CONTEXT_CUSTOM_TYPE = "powerdirector.sessions_yield";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolvePowerDirectorAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : createPowerDirectorCodingTools({
    ...
    const docsPath = await resolvePowerDirectorDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/powerdirector/powerdirector/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("powerdirector:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/powerdirector/powerdirector/issues/8643
