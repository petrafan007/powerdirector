...
export async function loadRunOverflowCompactionHarness(): Promise<{
  runEmbeddedPiAgent: typeof import("./run").runEmbeddedPiAgent;
}> {
  ...
  vi.doMock("../../plugins/provider-runtime", () => ({
  ...
  vi.doMock("./run/payloads", () => ({
    buildEmbeddedRunPayloads: vi.fn(() => []),
  }));

  vi.doMock("./tool-result-truncation", () => ({
  ...
  const { runEmbeddedPiAgent } = await import("./run");
