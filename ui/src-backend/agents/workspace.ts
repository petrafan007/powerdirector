...
export function resolveDefaultAgentWorkspaceDir(
  ...
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".powerdirector", `workspace-${profile}`);
  }
  return path.join(home, ".powerdirector", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".powerdirector";
    ...
    if (pattern.includes("*") || pattern.includes("?") || pattern.includes("{")) {
      try {
        const globFn = (fs as unknown as {
          glob?: (globPattern: string, opts?: { cwd?: string }) => AsyncIterable<string>;
        }).glob;
        if (typeof globFn === "function") {
          const matches = globFn(pattern, { cwd: resolvedDir });
