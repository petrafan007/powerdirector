...
import { ensureAuthProfileStore } from './store';
...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolvePowerDirectorAgentDir() returns mainAgentDir
    process.env.POWERDIRECTOR_STATE_DIR = tmpDir;
    process.env.POWERDIRECTOR_AGENT_DIR = mainAgentDir;
