...
import { resolveOAuthPath } from "../../config/paths";
import { withFileLock } from "../../infra/file-lock";
import { loadJsonFile, saveJsonFile } from "../../infra/json-file";
import { AUTH_STORE_LOCK_OPTIONS, AUTH_STORE_VERSION, log } from "./constants";
import { syncExternalCliCredentials } from "./external-cli-sync";
import { ensureAuthStoreFile, resolveAuthStorePath, resolveLegacyAuthStorePath } from "./paths";
import type { AuthProfileCredential, AuthProfileStore, ProfileUsageStats } from "./types";
 ...
 * A common mistake — caused by the similarity with the `powerdirector.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (powerdirector.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.POWERDIRECTOR_AUTH_STORE_READONLY === "1";
