...
type CreateSessionsSpawnTool =
  (typeof import("./tools/sessions-spawn-tool"))["createSessionsSpawnTool"];
export type CreatePowerDirectorToolsOpts = Parameters<CreateSessionsSpawnTool>[0];
...
export async function getSessionsSpawnTool(opts: CreatePowerDirectorToolsOpts) {
  ...
  vi.resetModules();
  const { createSessionsSpawnTool } = await import("./tools/sessions-spawn-tool");
...
// Some tools import callGateway via "../../gateway/call" (from nested folders). Mock that too.
vi.mock("../../gateway/call", () => ({
  callGateway: (opts: unknown) => hoisted.callGatewayMock(opts),
}));
