...
import { DEFAULT_SUBAGENT_MAX_SPAWN_DEPTH } from "../config/agent-limits";
...
import { createBoundDeliveryRouter } from "../infra/outbound/bound-delivery-router";
import type { ConversationRef } from "../infra/outbound/session-binding-service";
import { getGlobalHookRunner } from "../plugins/hook-runner-global";
import { normalizeAccountId, normalizeMainKey } from "../routing/session-key";
import { defaultRuntime } from "../runtime";
import { isCronSessionKey } from "../sessions/session-key-utils";
...
import { formatAgentInternalEventsForPrompt, type AgentInternalEvent } from "./internal-events";
import {
...
} from "./subagent-announce-dispatch";
...
import type { SpawnSubagentMode } from "./subagent-spawn";
import { readLatestAssistantReply } from "./tools/agent-step";
import { sanitizeTextContent, extractAssistantText } from "./tools/sessions-helpers";
import { isAnnounceSkip } from "./tools/sessions-send-helpers";

const FAST_TEST_MODE = process.env.POWERDIRECTOR_TEST_FAST === "1";
const FAST_TEST_RETRY_INTERVAL_MS = 8;
const DEFAULT_SUBAGENT_ANNOUNCE_TIMEOUT_MS = 90_000;
const MAX_TIMER_SAFE_TIMEOUT_MS = 2_147_000_000;
const GATEWAY_TIMEOUT_PATTERN = /gateway timeout/i;
let subagentRegistryRuntimePromise: Promise<
  typeof import("./subagent-registry-runtime")
> | null = null;
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to PowerDirector sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`powerdirector ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for PowerDirector subagents (`runtime: "subagent"`).',
