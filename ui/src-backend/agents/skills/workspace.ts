...
import { isPathInside } from "../../infra/path-guards";
...
function resolveSkillsLimits(config?: PowerDirectorConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const workspaceSkills = loadSkills({
    ...
    source: "powerdirector-workspace",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolvePowerDirectorMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: PowerDirectorConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`powerdirector skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`powerdirector skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
