...
import { PROVIDER_ENV_API_KEY_CANDIDATES } from "./model-auth-env-vars";
import {
  CUSTOM_LOCAL_AUTH_MARKER,
  isKnownEnvApiKeyMarker,
  isNonSecretApiKeyMarker,
  OLLAMA_LOCAL_AUTH_MARKER,
} from "./model-auth-markers";
import { normalizeProviderId, normalizeProviderIdForAuth } from "./model-selection";
...
export async function resolveApiKeyForProvider(params: {
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("powerdirector agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
