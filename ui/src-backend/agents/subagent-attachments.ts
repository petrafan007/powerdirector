...
function resolveAttachmentLimits(config: PowerDirectorConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".powerdirector", "attachments");
  const relDir = path.posix.join(".powerdirector", "attachments", attachmentId);
