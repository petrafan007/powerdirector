...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("powerdirector-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("powerdirector-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("powerdirector-sbx-");
