...
import { buildGroupChatContext, buildGroupIntro } from "../auto-reply/reply/groups";
import {
  buildInboundMetaSystemPrompt,
  buildInboundUserContextPrefix,
} from "../auto-reply/reply/inbound-meta";
import type { TemplateContext } from "../auto-reply/templating";
...
import { resolveBootstrapContextForRun } from "./bootstrap-files";
import { buildEmbeddedSystemPrompt } from "./pi-embedded-runner/system-prompt";
import { buildAgentSystemPrompt } from "./system-prompt";
import { createStubTool } from "./test-helpers/pi-tool-stubs";
import { buildToolSummaryMap } from "./tool-summaries";
...
async function createToolRichScenario(workspaceDir: string): Promise<PromptScenario> {
  const skillsPrompt = [
    ...
    "<skill><name>release</name><description>Release PowerDirector safely.</description><location>/skills/release/SKILL.md</location></skill>",
    "</available_skills>",
  ].join("\n");
...
export async function createWorkspaceWithPromptCompositionFiles(): Promise<string> {
  const workspaceDir = await makeTempWorkspace("powerdirector-prompt-cache-");
  await writeWorkspaceFile({
    dir: workspaceDir,
    name: "AGENTS.md",
    content: [
...
export async function createPromptCompositionScenarios(): Promise<{
  ...
  const warningWorkspaceDir = await makeTempWorkspace("powerdirector-prompt-cache-warning-");
  const scenarios = [
