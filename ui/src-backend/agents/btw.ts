...
import { diagnosticLogger as diag } from "../logging/diagnostic";
import { resolveSessionAuthProfileOverride } from "./auth-profiles/session-override";
import { getApiKeyForModel, requireApiKey } from "./model-auth";
import { ensurePowerDirectorModelsJson } from "./models-config";
import { EmbeddedBlockChunker, type BlockReplyChunking } from "./pi-embedded-block-chunker";
import { resolveModelWithRegistry } from "./pi-embedded-runner/model";
import { getActiveEmbeddedRunSnapshot } from "./pi-embedded-runner/runs";
import { mapThinkingLevel } from "./pi-embedded-runner/utils";
import { discoverAuthStorage, discoverModels } from "./pi-model-discovery";
import { stripToolResultDetails } from "./session-transcript-repair";
...
async function resolveRuntimeModel(params: {
  ...
  await ensurePowerDirectorModelsJson(params.cfg, params.agentDir);
