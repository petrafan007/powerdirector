...
import { resolveAcpAgentPolicyError, resolveAcpDispatchPolicyError } from "../acp/policy";
import { toAcpRuntimeError } from "../acp/runtime/errors";
import { resolveAcpSessionCwd } from "../acp/runtime/session-identifiers";
import { createSubsystemLogger } from "../logging/subsystem";

const log = createSubsystemLogger("agents/agent-command");
import { normalizeReplyPayload } from "../auto-reply/reply/normalize-reply";
import {
...
import {
  ...
  isSilentReplyText,
  SILENT_REPLY_TOKEN,
} from "../auto-reply/tokens";
import { formatCliCommand } from "../cli/command-format";
import { resolveCommandSecretRefsViaGateway } from "../cli/command-secret-gateway";
import { getAgentRuntimeCommandSecretTargetIds } from "../cli/command-secret-targets";
...
import { resolveSessionTranscriptFile } from "../config/sessions/transcript";
import {
  clearAgentRunContext,
  emitAgentEvent,
  registerAgentRunContext,
} from "../infra/agent-events";
import { buildOutboundSessionContext } from "../infra/outbound/session-context";
...
import { sanitizeForLog } from "../terminal/ansi";
...
import { clearSessionAuthProfileOverride } from "./auth-profiles/session-override";
import { resolveBootstrapWarningSignaturesSeen } from "./bootstrap-budget";
import { runCliAgent } from "./cli-runner";
import { getCliSessionId, setCliSessionId } from "./cli-session";
import { deliverAgentCommandResult } from "./command/delivery";
import { resolveAgentRunContext } from "./command/run-context";
import { updateSessionStoreAfterAgentRun } from "./command/session-store";
import { resolveSession } from "./command/session";
import type { AgentCommandIngressOpts, AgentCommandOpts } from "./command/types";
import { DEFAULT_MODEL, DEFAULT_PROVIDER } from "./defaults";
import { FailoverError } from "./failover-error";
import { formatAgentInternalEventsForPrompt } from "./internal-events";
...
import { prepareSessionManagerForRun } from "./pi-embedded-runner/session-manager-init";
import { runEmbeddedPiAgent } from "./pi-embedded";
import { buildWorkspaceSkillSnapshot } from "./skills";
import { getSkillsSnapshotVersion } from "./skills/refresh";
import { normalizeSpawnedRunMetadata } from "./spawned-context";
...
function prependInternalEventContext(
  ...
  if (body.includes("PowerDirector runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("powerdirector agents list")}" to see configured agents.`,
      );
    }
  }
