...
describe("buildWorkspaceSkillStatus", () => {
  it("reports missing requirements and install options", async () => {
    ...
    await writeSkill({
      ...
      metadata:
        '{"powerdirector":{"requires":{"bins":["fakebin"],"env":["ENV_KEY"],"config":["browser.enabled"]},"install":[{"id":"brew","kind":"brew","formula":"fakebin","bins":["fakebin"],"label":"Install fakebin"}]}}',
    });
  ...
  it("respects OS-gated skills", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"powerdirector":{"os":["darwin"]}}',
    });
  ...
  it("marks bundled skills blocked by allowlist", async () => {
    ...
    try {
      ...
      if (originalBundled === undefined) {
        ...
        process.env.POWERDIRECTOR_BUNDLED_SKILLS_DIR = originalBundled;
      }
    }
  });
  ...
  it("filters install options by OS", async () => {
    ...
    await writeSkill({
      ...
      metadata:
        '{"powerdirector":{"requires":{"bins":["missing-bin"]},"install":[{"id":"mac","kind":"download","os":["darwin"],"url":"https://example.com/mac.tar.bz2"},{"id":"linux","kind":"download","os":["linux"],"url":"https://example.com/linux.tar.bz2"},{"id":"win","kind":"download","os":["win32"],"url":"https://example.com/win.tar.bz2"}]}}',
    });
