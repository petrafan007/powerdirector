...
import { prepareCliBundleMcpConfig } from "./cli-runner/bundle-mcp";
import {
...
export async function runCliAgent(params: {
  ...
  streamParams?: import("./command/types").AgentStreamParams;
  ownerNumbers?: string[];
  cliSessionId?: string;
  ...
  const executeCliWithSession = async (
    ...
    try {
      const output = await enqueueCliRun(queueKey, async () => {
        ...
        const logOutputText = isTruthyEnvValue(process.env.POWERDIRECTOR_CLAUDE_CLI_LOG_OUTPUT);
        if (logOutputText) {
