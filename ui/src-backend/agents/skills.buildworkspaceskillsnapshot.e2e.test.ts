...
describe("buildWorkspaceSkillSnapshot", () => {
  ...
  it("limits discovery for nested repo-style skills roots (dir/skills/*)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-"));
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-skills-repo-"));
  ...
  it("enforces maxSkillFileBytes for root-level SKILL.md", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-"));
    const rootSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-root-skill-"));
