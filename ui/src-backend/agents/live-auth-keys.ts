...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "POWERDIRECTOR_LIVE_ANTHROPIC_KEY",
    listVar: "POWERDIRECTOR_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "POWERDIRECTOR_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `POWERDIRECTOR_LIVE_${base}_KEY`;
