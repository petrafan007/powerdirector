...
import { resolveAgentConfig, resolveAgentModelPrimary } from './agent-scope';
import { DEFAULT_MODEL, DEFAULT_PROVIDER } from './defaults';
import type { ModelCatalogEntry } from './model-catalog';
import { normalizeGoogleModelId } from './models-config.providers';
...
export function isCliProvider(provider: string, cfg?: PowerDirectorConfig): boolean {
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      console.warn(
        `[powerdirector] Model "${trimmed}" specified without provider. Falling back to "anthropic/${trimmed}". Please use "anthropic/${trimmed}" in your config.`,
      );
