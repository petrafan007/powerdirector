...
import { assertNoPathAliasEscape, type PathAliasPolicy } from "../infra/path-alias-guards";
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const powerDirectorTmpDir = path.resolve(resolvePreferredPowerDirectorTmpDir());
  if (!isPathInside(powerDirectorTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: powerDirectorTmpDir });
  return resolved;
}
