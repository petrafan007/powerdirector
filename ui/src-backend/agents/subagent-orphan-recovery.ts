 ...
 *
 * @see https://github.com/powerdirector/powerdirector/issues/47711
 */
...
import { readSessionMessages } from "../gateway/session-utils.fs";
import { createSubsystemLogger } from "../logging/subsystem";
import { replaceSubagentRunAfterSteer } from "./subagent-registry";
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /powerdirector\.json|powerdirector gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify powerdirector.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
