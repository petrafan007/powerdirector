...
import { resolvePluginWebSearchConfig } from "../../config/legacy-web-search";

type ConfiguredWebSearchProvider = NonNullable<
  NonNullable<NonNullable<PowerDirectorConfig["tools"]>["web"]>["search"]
>["provider"];

export type WebSearchConfig = NonNullable<PowerDirectorConfig["tools"]>["web"] extends infer Web
...
export function resolveSearchConfig(cfg?: PowerDirectorConfig): WebSearchConfig {
...
export function setProviderWebSearchPluginConfigValue(
  configTarget: PowerDirectorConfig,
  pluginId: string,
  key: string,
  value: unknown,
): void {
