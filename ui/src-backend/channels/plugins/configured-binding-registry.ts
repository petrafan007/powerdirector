...
} from "./configured-binding-compiler";
import {
  materializeConfiguredBindingRecord,
  resolveMatchingConfiguredBinding,
  toConfiguredBindingConversationRef,
} from "./configured-binding-match";
import { resolveConfiguredBindingRecordBySessionKeyFromRegistry } from "./configured-binding-session-lookup";

export function primeConfiguredBindingRegistry(params: { cfg: PowerDirectorConfig }): {
  bindingCount: number;
  channelCount: number;
} {
