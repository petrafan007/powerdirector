...
import type { SecretInput } from "../../config/types.secrets";
import {
...
import type { ChannelSetupWizard, ChannelSetupWizardAllowFromEntry } from "./setup-wizard";
...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: PowerDirectorConfig) => Array<string | number> | undefined;
}): PowerDirectorConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: PowerDirectorConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: PowerDirectorConfig) => Array<string | number> | undefined;
}): (cfg: PowerDirectorConfig, dmPolicy: DmPolicy) => PowerDirectorConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: PowerDirectorConfig, allowFrom: string[]) => PowerDirectorConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: PowerDirectorConfig, groupPolicy: "open" | "allowlist" | "disabled") => PowerDirectorConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: PowerDirectorConfig) => PowerDirectorConfig | Promise<PowerDirectorConfig>;
  applySet?: (
    cfg: PowerDirectorConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => PowerDirectorConfig | Promise<PowerDirectorConfig>;
}): Promise<{
  cfg: PowerDirectorConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in PowerDirector config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "POWERDIRECTOR_SECRET",
      envVarFormatError:
        'Use an env var name like "POWERDIRECTOR_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends PowerDirectorConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: PowerDirectorConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: PowerDirectorConfig) => Array<string | number>;
