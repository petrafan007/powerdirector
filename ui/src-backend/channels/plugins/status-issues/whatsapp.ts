...
import { asString, collectIssuesForEnabledAccounts, isRecord } from "./shared";
    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("powerdirector channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (running && !connected) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("powerdirector doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
      }
    },
  });
