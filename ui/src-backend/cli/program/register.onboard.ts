...
import { formatAuthChoiceChoicesForCli } from "../../commands/auth-choice-options";
import type { GatewayDaemonRuntime } from "../../commands/daemon-runtime";
import { CORE_ONBOARD_AUTH_FLAGS } from "../../commands/onboard-core-auth-flags";
import type {
  AuthChoice,
  GatewayAuthChoice,
  GatewayBind,
...
import { resolveManifestProviderOnboardAuthFlags } from "../../plugins/provider-auth-choices";
...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.powerdirector.ai/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.powerdirector/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. POWERDIRECTOR_GATEWAY_TOKEN)",
    )
