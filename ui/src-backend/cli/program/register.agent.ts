...
import { agentCliCommand } from '../../commands/agent-via-gateway';
import {
  agentsAddCommand,
  agentsDeleteCommand,
  agentsListCommand,
...
${formatHelpExamples([
  ['powerdirector agent --to +15555550123 --message "status update"', "Start a new session."],
  ['powerdirector agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'powerdirector agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'powerdirector agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['powerdirector agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'powerdirector agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.powerdirector.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.powerdirector.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['powerdirector agents set-identity --agent main --name "PowerDirector" --emoji "🦞"', "Set name + emoji."],
  ["powerdirector agents set-identity --agent main --avatar avatars/powerdirector.png", "Set avatar path."],
  [
    "powerdirector agents set-identity --workspace ~/.powerdirector/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "powerdirector agents set-identity --identity-file ~/.powerdirector/workspace/IDENTITY.md --agent main",
