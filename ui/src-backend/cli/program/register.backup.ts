...
import { backupVerifyCommand } from "../../commands/backup-verify";
import { backupCreateCommand } from "../../commands/backup";
...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.powerdirector.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["powerdirector backup create", "Create a timestamped backup in the current directory."],
          [
            "powerdirector backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "powerdirector backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "powerdirector backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "powerdirector backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["powerdirector backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "powerdirector backup verify ./2026-03-09T00-00-00.000Z-powerdirector-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "powerdirector backup verify ~/Backups/latest.tar.gz --json",
