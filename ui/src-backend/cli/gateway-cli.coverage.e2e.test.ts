...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio powerdirector.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.powerdirector.internal",
        "  ws: ws://studio.powerdirector.internal:18789",
      ],
    },
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (PowerDirector)",
        displayName: "Studio",
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ POWERDIRECTOR_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
