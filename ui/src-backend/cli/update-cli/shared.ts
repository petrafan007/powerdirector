...
import { normalizePackageTagInput } from "../../infra/package-tag";
...
export function resolveGitInstallDir(): string {
  const override = process.env.POWERDIRECTOR_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  return resolveStateDir(process.env, (typeof ((typeof os.homedir === "function") ? os.homedir : (() => "")) === "function" ? ((typeof os.homedir === "function") ? os.homedir : (() => "")) : (() => "")));
}
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolvePowerDirectorPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", POWERDIRECTOR_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`POWERDIRECTOR_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "powerdirector.mjs");
