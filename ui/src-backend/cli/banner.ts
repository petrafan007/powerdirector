...
import { hasRootVersionAlias } from "./argv";
import { readCliBannerTaglineMode } from "./banner-config-lite";
import { pickTagline, type TaglineMode, type TaglineOptions } from "./tagline";
...
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  const colored = LOBSTER_ASCII.map((line) => {
    if (line.includes("POWERDIRECTOR")) {
      return (
        theme.muted("              ") +
        theme.accent("🦞") +
        theme.info(" POWERDIRECTOR ") +
        theme.accent("🦞")
      );
    }
