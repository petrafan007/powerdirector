...
import { parseCanvasSnapshotPayload } from './nodes-canvas';
import { parseByteSize } from './parse-bytes';
import { parseDurationMs } from './parse-duration';
import { shouldSkipRespawnForArgv } from './respawn-policy';
import { waitForever } from './wait';

const { registerDnsCli } = await import('./dns-cli');
...
describe("shouldSkipRespawnForArgv", () => {
  it("skips respawn for help/version calls", () => {
    expect(shouldSkipRespawnForArgv(["node", "powerdirector", "--help"])).toBe(true);
    expect(shouldSkipRespawnForArgv(["node", "powerdirector", "-V"])).toBe(true);
  });

  it("keeps respawn path for normal commands", () => {
    expect(shouldSkipRespawnForArgv(["node", "powerdirector", "status"])).toBe(false);
  });
});
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "powerdirector.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("powerdirector.internal");
    } finally {
      log.mockRestore();
    }
  });
