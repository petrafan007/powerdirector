...
import { readGatewayPasswordEnv, readGatewayTokenEnv } from "../gateway/credentials";
...
import { getQrRemoteCommandSecretTargetIds } from "./command-secret-targets";
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.powerdirector.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the PowerDirector iOS app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("powerdirector devices list")}`,
          `  ${theme.command("powerdirector devices approve <requestId>")}`,
        );
