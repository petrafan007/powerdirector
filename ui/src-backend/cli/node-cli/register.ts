    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "powerdirector node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["powerdirector node status", "Check node host service status."],
          ["powerdirector node install", "Install the node host service."],
          ["powerdirector node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.powerdirector.ai/cli/node")}\n`,
    );
