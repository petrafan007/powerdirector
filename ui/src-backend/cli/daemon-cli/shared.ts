...
import { createDaemonActionContext } from "./response";
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("powerdirector gateway install", env)}`);
    if (fileLog) {
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("powerdirector gateway install", env),
    startCommand: formatCliCommand("powerdirector gateway", env),
