...
import { readBestEffortConfig, readConfigFileSnapshot } from "../../config/config";
...
import type { GatewayServiceRestartResult } from "../../daemon/service-types";
import { describeGatewayServiceRestart } from "../../daemon/service";
...
import { isGatewaySecretRefUnavailableError } from "../../gateway/credentials";
import { isWSL } from "../../infra/wsl";
import { defaultRuntime } from "../../runtime";
import { resolveGatewayTokenForDriftCheck } from "./gateway-token-drift";
import {
...
export async function runServiceRestart(params: {
  ...
  if (loaded && params.checkTokenDrift) {
    ...
    try {
      ...
      const serviceToken = command?.environment?.POWERDIRECTOR_GATEWAY_TOKEN;
