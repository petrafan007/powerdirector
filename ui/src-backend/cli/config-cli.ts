...
import { OLLAMA_DEFAULT_BASE_URL } from "../agents/ollama-defaults";
import type { PowerDirectorConfig } from "../config/config";
import { readConfigFileSnapshot, writeConfigFile } from "../config/config";
import { formatConfigIssueLines, normalizeConfigIssues } from "../config/issue-format";
import { CONFIG_PATH } from "../config/paths";
import { isBlockedObjectKey } from "../config/prototype-keys";
import { redactConfigObject } from "../config/redact-snapshot";
import {
  coerceSecretRef,
...
import { validateConfigObjectRaw } from "../config/validation";
import { SecretProviderSchema } from "../config/zod-schema.core";
...
} from "../secrets/ref-contract";
import { resolveSecretRefValue } from "../secrets/resolve";
import {
  discoverConfigSecretTargets,
  resolveConfigSecretTargetByPath,
} from "../secrets/target-registry";
...
} from "./config-set-input";
import { resolveConfigSetMode } from "./config-set-parser";
...
const CONFIG_SET_EXAMPLE_VALUE = formatCliCommand(
  "powerdirector config set gateway.port 19001 --strict-json",
);
const CONFIG_SET_EXAMPLE_REF = formatCliCommand(
  "powerdirector config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
);
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "powerdirector config set secrets.providers.vault --provider-source file --provider-path /etc/powerdirector/secrets.json --provider-mode json",
);
const CONFIG_SET_EXAMPLE_BATCH = formatCliCommand(
  "powerdirector config set --batch-file ./config-set.batch.json --dry-run",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("powerdirector doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: PowerDirectorConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as PowerDirectorConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "powerdirector.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.powerdirector.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing powerdirector.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
