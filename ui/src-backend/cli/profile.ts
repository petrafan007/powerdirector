...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.powerdirector${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.POWERDIRECTOR_PROFILE = profile;

  const stateDir = env.POWERDIRECTOR_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.POWERDIRECTOR_STATE_DIR?.trim()) {
    env.POWERDIRECTOR_STATE_DIR = stateDir;
  }
  ...
  if (profile === "dev" && !env.POWERDIRECTOR_GATEWAY_PORT?.trim()) {
    env.POWERDIRECTOR_GATEWAY_PORT = "19001";
  }
}
