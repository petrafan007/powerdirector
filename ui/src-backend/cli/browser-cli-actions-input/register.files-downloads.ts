...
export function registerBrowserFilesAndDownloadsCommands(
  ...
  browser
    ...
    .argument(
      "<paths...>",
      "File paths to upload (must be within PowerDirector temp uploads dir, e.g. /tmp/powerdirector/uploads/file.pdf)",
    )
  ...
  browser
    ...
    .argument(
      "<path>",
      "Save path within powerdirector temp downloads dir (e.g. report.pdf or /tmp/powerdirector/downloads/report.pdf)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
