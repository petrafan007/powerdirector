...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    POWERDIRECTOR_CONFIG_PATH: process.env.POWERDIRECTOR_CONFIG_PATH,
    POWERDIRECTOR_GATEWAY_PORT: process.env.POWERDIRECTOR_GATEWAY_PORT,
    POWERDIRECTOR_PROFILE: process.env.POWERDIRECTOR_PROFILE,
  };
  ...
  afterEach(() => {
    if (originalEnv.POWERDIRECTOR_STATE_DIR !== undefined) {
      process.env.POWERDIRECTOR_STATE_DIR = originalEnv.POWERDIRECTOR_STATE_DIR;
    } else {
      delete process.env.POWERDIRECTOR_STATE_DIR;
    }

    if (originalEnv.POWERDIRECTOR_CONFIG_PATH !== undefined) {
      process.env.POWERDIRECTOR_CONFIG_PATH = originalEnv.POWERDIRECTOR_CONFIG_PATH;
    } else {
      delete process.env.POWERDIRECTOR_CONFIG_PATH;
    }

    if (originalEnv.POWERDIRECTOR_GATEWAY_PORT !== undefined) {
      process.env.POWERDIRECTOR_GATEWAY_PORT = originalEnv.POWERDIRECTOR_GATEWAY_PORT;
    } else {
      delete process.env.POWERDIRECTOR_GATEWAY_PORT;
    }

    if (originalEnv.POWERDIRECTOR_PROFILE !== undefined) {
      process.env.POWERDIRECTOR_PROFILE = originalEnv.POWERDIRECTOR_PROFILE;
    } else {
      delete process.env.POWERDIRECTOR_PROFILE;
    }
  });
