...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "powerdirector";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "powerdirector"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "powerdirector" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (powerdirector|existing-session). Default: powerdirector")
    .action(
      async (
        opts: {
          name: string;
