...
import { sanitizeHostExecEnv } from "../infra/host-env-security";
import { runBrowserProxyCommand } from "./invoke-browser";
import { buildSystemRunApprovalPlan, handleSystemRunInvoke } from "./invoke-system-run";
import type {
  ExecEventPayload,
...
const execHostEnforced = process.env.POWERDIRECTOR_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.POWERDIRECTOR_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
export type { SkillBinsProvider } from "./invoke-types";
