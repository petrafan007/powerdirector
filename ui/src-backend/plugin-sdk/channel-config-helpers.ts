...
import { buildAccountScopedDmSecurityPolicy } from "../channels/plugins/helpers";
import { normalizeWhatsAppAllowFromEntries } from "../channels/plugins/normalize/whatsapp";
import { getChannelPlugin } from "../channels/plugins/registry";
import type { ChannelConfigAdapter } from "../channels/plugins/types.adapters";
...
export function createScopedAccountConfigAccessors<
  ResolvedAccount,
  Config extends PowerDirectorConfig = PowerDirectorConfig,
>(params: {
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: PowerDirectorConfig; accountId?: string | null }) =>
      mapAllowFromEntries(
...
function setTopLevelChannelEnabledInConfigSection<Config extends PowerDirectorConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends PowerDirectorConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends PowerDirectorConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
