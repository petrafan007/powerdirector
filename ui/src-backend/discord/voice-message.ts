...
import { parseFfprobeCodecAndSampleRate, runFfmpeg, runFfprobe } from "../media/ffmpeg-exec";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "../media/ffmpeg-limits";
import { unlinkIfExists } from "../media/temp-files";
...
async function generateWaveformFromPcm(filePath: string): Promise<string> {
  const tempDir = resolvePreferredPowerDirectorTmpDir();
