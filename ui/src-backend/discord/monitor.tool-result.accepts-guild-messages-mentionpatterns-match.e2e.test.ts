  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/powerdirector",
      },
    },
    session: { store: "/tmp/powerdirector-sessions.json" },
...
describe("discord tool result dispatch", () => {
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "PowerDirector" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/powerdirector" },
      session: { store: "/tmp/powerdirector-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
