...
import { createSentMessageCache } from "./echo-cache";
import {
  buildIMessageInboundContext,
  resolveIMessageInboundDecision,
} from "./inbound-processing";
import { createLoopRateLimiter } from "./loop-rate-limiter";
 ...
 *   exec ssh -T powerdirector@192.168.64.3 /opt/homebrew/bin/imsg "$@"
...
async function detectRemoteHostFromCliPath(cliPath: string): Promise<string | undefined> {
  try {
    ...
    // Match user@host pattern first (e.g., powerdirector@192.168.64.3)
