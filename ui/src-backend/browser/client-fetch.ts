...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-powerdirector-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-powerdirector-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-powerdirector-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the PowerDirector gateway (PowerDirector.app menubar, or \`${formatCliCommand("powerdirector gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the PowerDirector browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the PowerDirector browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
