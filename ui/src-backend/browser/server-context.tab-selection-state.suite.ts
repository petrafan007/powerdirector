...
import "./server-context.chrome-test-harness";
import * as cdpModule from "./cdp";
import { InvalidBrowserNavigationUrlError } from "./navigation-guard";
import { createBrowserRouteContext } from "./server-context";
import {
  makeManagedTabsWithNew,
  makeState,
  originalFetch,
} from "./server-context.remote-tab-ops.harness";
...
async function openManagedTabWithRunningProfile(params: {
  ...
  const state = makeState("powerdirector");
  seedRunningProfileState(state);
  const ctx = createBrowserRouteContext({ getState: () => state });
  const powerdirector = ctx.forProfile("powerdirector");
  return await powerdirector.openTab(params.url ?? "http://127.0.0.1:3009");
}
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const opened = await powerdirector.openTab("http://127.0.0.1:8080");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("powerdirector")?.lastTargetId).toBe("CREATED");
  ...
  it("does not fail tab open when managed-tab cleanup list fails", async () => {
    ...
    const opened = await powerdirector.openTab("http://127.0.0.1:3009");
    expect(opened.targetId).toBe("NEW");
  });
  ...
  it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
    ...
    await expect(powerdirector.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(
