...
import type { BrowserActRequest, BrowserFormField } from "../client-actions-core";
import { normalizeBrowserFormField } from "../form-fields";
import { getBrowserProfileCapabilities } from "../profile-capabilities";
import type { BrowserRouteContext } from "../server-context";
import { matchBrowserUrlPattern } from "../url-pattern";
import { registerBrowserAgentActDownloadRoutes } from "./agent.act.download";
import { registerBrowserAgentActHookRoutes } from "./agent.act.hooks";
import {
  type ActKind,
  isActKind,
  parseClickButton,
...
function browserEvaluateDisabledMessage(action: "wait" | "evaluate"): string {
  return [
    ...
    "Docs: /gateway/configuration#browser-powerdirector-managed-browser",
  ].join("\n");
}
