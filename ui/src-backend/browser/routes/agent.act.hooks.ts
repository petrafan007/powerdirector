import { evaluateChromeMcpScript, uploadChromeMcpFile } from "../chrome-mcp";
...
import { DEFAULT_UPLOAD_DIR, resolveExistingPathsWithinRoot } from "./path-output";
      ...
      run: async ({ profileCtx, cdpUrl, tab }) => {
        if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
          ...
          await evaluateChromeMcpScript({
            ...
            targetId: tab.targetId,
            fn: `() => {
              const state = (window.__powerdirectorDialogHook ??= {});
              if (!state.originals) {
                state.originals = {
              ...
              const restore = () => {
                ...
                delete window.__powerdirectorDialogHook;
              };
              window.alert = (...args) => {
                try {
                  return undefined;
