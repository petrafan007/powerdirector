import { getChromeMcpPid } from "../chrome-mcp";
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    if (driver && driver !== "powerdirector" && driver !== "clawd" && driver !== "existing-session") {
      return jsonError(
        res,
        400,
        `unsupported profile driver "${driver}"; use "powerdirector", "clawd", or "existing-session"`,
      );
    }
        ...
        await service.createProfile({
          ...
          driver:
            driver === "existing-session"
              ? "existing-session"
              : driver === "powerdirector" || driver === "clawd"
                ? "powerdirector"
                : undefined,
        }),
    });
