...
export function resolvePowerDirectorUserDataDir(profileName = DEFAULT_POWERDIRECTOR_BROWSER_PROFILE_NAME) {
...
export async function launchPowerDirectorChrome(
  ...
  const userDataDir = resolvePowerDirectorUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_POWERDIRECTOR_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decoratePowerDirectorProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 powerdirector browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`powerdirector browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`powerdirector browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 powerdirector browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopPowerDirectorChrome(running: RunningChrome, timeoutMs = 2500) {
