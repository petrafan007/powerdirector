...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    previousGatewayToken = process.env.POWERDIRECTOR_GATEWAY_TOKEN;
  ...
  afterEach(async () => {
    ...
    if (previousGatewayToken === undefined) {
      ...
      process.env.POWERDIRECTOR_GATEWAY_TOKEN = previousGatewayToken;
    }
  });
  ...
  it("uses gateway token for relay auth headers on loopback URLs", async () => {
    ...
    expect(headers["x-powerdirector-relay-token"]).toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    process.env.POWERDIRECTOR_GATEWAY_TOKEN = "test-gateway-token";
    try {
      cdpUrl = `http://127.0.0.1:${port}`;
      ...
      if (prev === undefined) {
        ...
        process.env.POWERDIRECTOR_GATEWAY_TOKEN = prev;
      }
