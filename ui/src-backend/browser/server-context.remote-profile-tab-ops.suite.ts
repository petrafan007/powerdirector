...
describe("browser server-context remote profile tab operations", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    ...
    state.resolved.profiles.powerdirector = {
      cdpPort: 18800,
      attachOnly: true,
      color: "#FF4500",
    };
    ...
    const launchMock = vi.mocked(chromeModule.launchPowerDirectorChrome);
    const ctx = createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("powerdirector").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote powerdirector profiles", async () => {
