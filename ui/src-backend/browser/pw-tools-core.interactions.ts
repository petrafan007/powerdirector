...
import { DEFAULT_FILL_FIELD_TYPE } from "./form-fields";
...
import { closePageViaPlaywright, resizeViewportViaPlaywright } from "./pw-tools-core.snapshot";
  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-powerdirector-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-powerdirector-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-powerdirector-labels", "1");
          ...
          tag.setAttribute("data-powerdirector-labels", "1");
