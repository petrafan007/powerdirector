...
import { normalizeSecretInputString, resolveSecretInputRef } from "../config/types.secrets";
import { secretRefKey } from "../secrets/ref-contract";
import { resolveSecretRefValues } from "../secrets/resolve";

const RELAY_TOKEN_CONTEXT = "powerdirector-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const POWERDIRECTOR_RELAY_BROWSER = "PowerDirector/extension-relay";
...
async function resolveGatewayAuthToken(): Promise<string | null> {
  const envToken =
    process.env.POWERDIRECTOR_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
    ...
    if (tokenRef) {
      ...
      throw new SecretRefUnavailableError(
        `extension relay requires a resolved gateway token, but gateway.auth.token SecretRef is unavailable (${refLabel}). Set POWERDIRECTOR_GATEWAY_TOKEN or resolve your secret provider.`,
      );
    }
...
export async function resolveRelayAcceptedTokensForPort(port: number): Promise<string[]> {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or POWERDIRECTOR_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedPowerDirectorRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === POWERDIRECTOR_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
