...
import { BrowserProfileUnavailableError } from "./errors";
import { getBrowserProfileCapabilities } from "./profile-capabilities";
import {
...
export function createProfileAvailability({
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopPowerDirectorChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchPowerDirectorChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchPowerDirectorChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopPowerDirectorChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    if (!profileState.running) {
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by powerdirector. ` +
          `Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
    }

    await stopPowerDirectorChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchPowerDirectorChrome(current.resolved, profile);
