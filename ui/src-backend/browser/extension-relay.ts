...
import { isLoopbackAddress, isLoopbackHost } from '../gateway/net';
...
const RELAY_AUTH_HEADER = "x-powerdirector-relay-token";
...
async function looksLikePowerDirectorRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/PowerDirector-Extension-Relay",
          revision: "0",
          userAgent: "PowerDirector-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "PowerDirector/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikePowerDirectorRelay(info.baseUrl))) {
