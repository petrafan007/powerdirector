...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => PowerDirectorConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("powerdirector-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): PowerDirectorConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
