...
This directory contains hooks that ship with PowerDirector. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
```bash
powerdirector hooks enable session-memory
```
...
```bash
powerdirector hooks enable bootstrap-extra-files
```
...
**Output**: `~/.powerdirector/logs/commands.log`

**Enable**:

```bash
powerdirector hooks enable command-logger
```
...
```bash
powerdirector hooks enable boot-md
```
...
homepage: https://docs.powerdirector.ai/automation/hooks#my-hook
metadata:
  { "powerdirector": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.powerdirector/hooks/` (shared across workspaces)
...
```bash
powerdirector hooks list
```

Show hook details:

```bash
powerdirector hooks info session-memory
```

Check hook status:

```bash
powerdirector hooks check
```
...
```bash
powerdirector hooks enable session-memory
powerdirector hooks disable command-logger
```
...
2. Restart gateway: `pkill -9 -f 'powerdirector.*gateway' && pnpm powerdirector gateway`
3. Enable the hook: `powerdirector hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.powerdirector.ai/automation/hooks
