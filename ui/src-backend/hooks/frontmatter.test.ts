...
`;
    ...
    expect(parsed.powerdirector.emoji).toBe("💾");
    expect(parsed.powerdirector.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.powerdirector.emoji).toBe("📝");
    expect(parsed.powerdirector.events).toEqual(["command"]);
    expect(parsed.powerdirector.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.powerdirector.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"powerdirector": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"powerdirector": {"events": ["test"]}}');
  });
...
describe("resolvePowerDirectorMetadata", () => {
  it("extracts powerdirector metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvePowerDirectorMetadata(frontmatter);
  ...
  it("returns undefined when powerdirector key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        powerdirector: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with PowerDirector" },
            { id: "npm", kind: "npm", package: "@powerdirector/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@powerdirector/hook");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const powerdirector = resolvePowerDirectorMetadata(frontmatter);
    expect(powerdirector?.emoji).toBe("disk");
    expect(powerdirector?.events).toEqual(["command:new"]);
  });
});
