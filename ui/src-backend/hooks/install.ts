...
} & Partial<Record<typeof POWERDIRECTOR_MANIFEST_KEY, { hooks?: string[] }>>;
...
async function ensurePowerDirectorHooks(manifest: HookPackageManifest) {
  const hooks = manifest[POWERDIRECTOR_MANIFEST_KEY]?.hooks;
...
export async function installHooksFromArchive(params: {
  ...
  return await withTempDir("powerdirector-hook-", async (tmpDir) => {
