...
import { installFromValidatedNpmSpecArchive } from "../infra/install-from-npm-spec";
import {
...
} from "../infra/install-package-dir";
...
async function ensurePowerDirectorHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing powerdirector.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json powerdirector.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensurePowerDirectorHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `powerdirector.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `powerdirector.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "powerdirector-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await installFromValidatedNpmSpecArchive({
    tempDirPrefix: "powerdirector-hook-pack-",
    spec,
    timeoutMs,
