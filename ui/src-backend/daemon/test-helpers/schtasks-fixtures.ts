...
import type { PortUsage } from "../../infra/ports-types";
import type { killProcessTree as killProcessTreeImpl } from "../../process/kill-tree";
import type { MockFn } from "../../test-utils/vitest-mock-fn";
import { resolveTaskScriptPath } from "../schtasks";
...
export async function writeGatewayScript(
  ...
  port = Number(env.POWERDIRECTOR_GATEWAY_PORT || "18789"),
) {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "POWERDIRECTOR_GATEWAY_PORT=${port}"`,
      `"C:\\Program Files\\nodejs\\node.exe" "C:\\Users\\steipete\\AppData\\Roaming\\npm\\node_modules\\powerdirector\\dist\\index.js" gateway --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
