...
import { isGatewayArgv } from "../infra/gateway-process-argv";
import { findVerifiedGatewayListenerPidsOnPortSync } from "../infra/gateway-processes";
...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.POWERDIRECTOR_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.POWERDIRECTOR_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.POWERDIRECTOR_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.POWERDIRECTOR_GATEWAY_PORT) ??
