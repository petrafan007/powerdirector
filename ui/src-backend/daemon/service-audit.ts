...
import { isBunRuntime, isNodeRuntime } from "./runtime-binary";
import {
  isSystemNodePath,
...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds POWERDIRECTOR_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `powerdirector gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.POWERDIRECTOR_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.POWERDIRECTOR_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `powerdirector gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
