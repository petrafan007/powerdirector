...
import { parseStrictInteger, parseStrictPositiveInteger } from "../infra/parse-finite-number";
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  ...
  return resolveGatewayLaunchAgentLabel(args?.env?.POWERDIRECTOR_PROFILE);
}
...
export function resolveGatewayLogPaths(env: GatewayServiceEnv): {
  ...
  const prefix = env.POWERDIRECTOR_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.POWERDIRECTOR_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
export async function findLegacyLaunchAgents(env: GatewayServiceEnv): Promise<LegacyLaunchAgent[]> {
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.POWERDIRECTOR_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.POWERDIRECTOR_PROFILE)) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    plistPath,
    actionHint: "powerdirector gateway install --force",
  });
...
export async function restartLaunchAgent({
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "powerdirector gateway restart",
  });
