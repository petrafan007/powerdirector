...
export async function loginMiniMaxPortalOAuth(params: {
    ...
    const noteLines = [
        ...
        `Expires in: ${Math.round((oauth.expired_in - Date.now() / 1000))} seconds`,
    ];
    ...
    // oauth.expired_in is unix timestamp in seconds or ms? 
    // PowerDirector code treated it as future timestamp? 
    // "Expires at: ${oauth.expired_in} unix timestamp" suggest it's absolute.
    // Let's assume it is absolute ms based on PowerDirector usage "Date.now() < expireTimeMs".
    // Wait, in requestOAuthCode payload it says "expired_in: number". 
    // Standard OAuth device flow returns seconds duration. 
    // But PowerDirector code: `const expireTimeMs = oauth.expired_in; while (Date.now() < expireTimeMs)` 
    // This implies `expired_in` from Minimax is an absolute timestamp (ms). 
    // Let's trust PowerDirector's implementation.
