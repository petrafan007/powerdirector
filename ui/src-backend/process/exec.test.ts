...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["POWERDIRECTOR_BASE_ENV"]);
    process.env.POWERDIRECTOR_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.POWERDIRECTOR_BASE_ENV ?? "") + "|" + (process.env.POWERDIRECTOR_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { POWERDIRECTOR_TEST_ENV: "ok" },
        },
      );
