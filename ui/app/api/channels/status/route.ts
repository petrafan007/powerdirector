...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (fs.existsSync(configPath)) {
            ...
            gatewayPort = fullConfig.gateway?.port || 3012;
            gatewayToken = fullConfig.gateway?.auth?.token || '';
        }
    } catch (err) {
    ...
    try {
        ...
        if (probe) gatewayQuery.set('probe', 'true');
        if (typeof channel === 'string' && channel.trim().length > 0) gatewayQuery.set('channel', channel.trim());
        if (typeof accountId === 'string' && accountId.trim().length > 0) gatewayQuery.set('accountId', accountId.trim());
        const gatewaySuffix = gatewayQuery.toString() ? `?${gatewayQuery.toString()}` : '';

        const res = await fetch(`http://127.0.0.1:${gatewayPort}/channels/status${gatewaySuffix}`, {
            cache: 'no-store',
            headers: {
        ...
        if (!res.ok) {
            console.warn(`Gateway returned non-OK status ${res.status}: ${bodyText.slice(0, 100)}`);
            throw new Error(`Gateway returned ${res.status}`);
        }

        if (!contentType.includes('application/json')) {
            console.error(`Gateway returned non-JSON content type ${contentType}: ${bodyText.slice(0, 500)}`);
            throw new Error(`Gateway returned non-JSON data: ${contentType}`);
        }

        const data = JSON.parse(bodyText);
        const liveChannels = Array.isArray(data?.channels) ? data.channels : [];
