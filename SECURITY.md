...
If you believe you've found a security issue in PowerDirector, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [powerdirector/powerdirector](https://github.com/powerdirector/powerdirector)
- **macOS desktop app** — [powerdirector/powerdirector](https://github.com/powerdirector/powerdirector) (apps/macos)
- **iOS app** — [powerdirector/powerdirector](https://github.com/powerdirector/powerdirector) (apps/ios)
- **Android app** — [powerdirector/powerdirector](https://github.com/powerdirector/powerdirector) (apps/android)
- **ClawHub** — [powerdirector/clawhub](https://github.com/powerdirector/clawhub)
- **Trust and threat model** — [powerdirector/trust](https://github.com/powerdirector/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@powerdirector.ai](mailto:security@powerdirector.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.powerdirector.ai).
...
- Tested version details (PowerDirector version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to PowerDirector's documented trust boundaries.
- For exposed-secret reports: proof the credential is PowerDirector-owned (or grants access to PowerDirector-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at PowerDirector. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

PowerDirector is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
PowerDirector does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- PowerDirector can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need PowerDirector, use one VPS (or host/OS user boundary) per user.
...
- This is expected in PowerDirector's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Out of Scope

- Public Internet Exposure
- Using PowerDirector in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.powerdirector`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not PowerDirector-owned and not granting access to PowerDirector-operated infrastructure/services) without demonstrated PowerDirector impact
...
PowerDirector's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
PowerDirector separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the PowerDirector process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Operational Guidance

For threat model + hardening guidance (including `powerdirector security audit --deep` and `--fix`), see:

- `https://docs.powerdirector.ai/gateway/security`
...
PowerDirector's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `powerdirector gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - PowerDirector keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `powerdirector security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__powerdirector__/canvas/`, `/__powerdirector__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__powerdirector__/canvas/`, `/__powerdirector__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

PowerDirector requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running PowerDirector in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v powerdirector-data:/app/data \
  powerdirector/powerdirector:latest
```
