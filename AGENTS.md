...
PowerDirector preserves the PowerDirector agent model (gateway, sessions, tools/skills, bindings). Use the PowerDirector CLI (`powerdirector` or `pdir`) and the UI pages (`/agents`, `/skills`, `/tools`, `/bindings`) to configure agents.

## What works
- Agent defaults, profiles, tool/skill policies, bindings, sessions.
- UI parity with PowerDirector agents page.
- Skills/config storage wired to PowerDirector gateway runtime.

## Upstream/In-Progress Components
The repo includes PowerDirector agent-related docs/assets for continuity. They are “upstream/inherited, not supported or still in progress” under PowerDirector. Verify compatibility before relying on:
