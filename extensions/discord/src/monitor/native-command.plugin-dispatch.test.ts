...
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("powerdirector/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
...
async function createNativeCommand(cfg: PowerDirectorConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: PowerDirectorConfig; name: string }) {
  const createDiscordNativeCommand = await loadCreateDiscordNativeCommand();
  return createDiscordNativeCommand({
    command: {
      name: params.name,
...
async function createStatusCommand(cfg: PowerDirectorConfig) {
