...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): PowerDirectorConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("powerdirector/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("powerdirector/plugin-sdk/plugin-runtime")>(
    "powerdirector/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
