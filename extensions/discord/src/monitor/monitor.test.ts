...
import { buildPluginBindingApprovalCustomId } from "powerdirector/plugin-sdk/conversation-runtime";
...
describe("agent components", () => {
  const createCfg = (): PowerDirectorConfig => ({}) as PowerDirectorConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): PowerDirectorConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/powerdirector-sessions-test.json");
    ...
    resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
      ...
      binding: {
        ...
        pluginName: "PowerDirector App Server",
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    const presence = resolveDiscordPresenceUpdate({
      activity: "Live",
      activityType: 1,
      activityUrl: "https://twitch.tv/powerdirector",
    });
    expect(presence).not.toBeNull();
    expect(presence?.activities).toHaveLength(1);
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/powerdirector",
    });
  });
});
