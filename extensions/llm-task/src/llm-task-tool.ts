...
import Ajv from "ajv";
import { resolvePreferredPowerDirectorTmpDir } from "powerdirector/plugin-sdk/llm-task";
// NOTE: This extension is intended to be bundled with PowerDirector.
// When running from source (tests/dev), PowerDirector internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { PowerDirectorPluginApi } from "powerdirector/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: PowerDirectorPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via powerdirector.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredPowerDirectorTmpDir(), "powerdirector-llm-task-"),
        );
