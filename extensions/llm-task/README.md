...
`powerdirector.invoke --each`) without adding new PowerDirector code per workflow.
...
This extension depends on PowerDirector internal modules (the embedded agent runner).
It is intended to ship as a **bundled** PowerDirector extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.powerdirector/extensions` as a standalone plugin directory.
