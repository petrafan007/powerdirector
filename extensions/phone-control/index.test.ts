...
} from "powerdirector/plugin-sdk/phone-control";
...
function createApi(params: {
  ...
  return {
    ...
    } as PowerDirectorPluginApi["runtime"],
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-phone-control-test-"));
    try {
      ...
      let command: PowerDirectorPluginCommandDefinition | undefined;
