...
import type { PowerDirectorPluginApi, PowerDirectorPluginService } from "powerdirector/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: PowerDirectorPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: PowerDirectorPluginApi["config"]): string[] {
...
export default function register(api: PowerDirectorPluginApi) {
  ...
  const timerService: PowerDirectorPluginService = {
