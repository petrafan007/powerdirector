...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): PowerDirectorConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as PowerDirectorConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): PowerDirectorConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as PowerDirectorConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: PowerDirectorConfig, accountId = "media") {
    await handleTelegramAction(
