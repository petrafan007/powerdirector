...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("powerdirector.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("powerdirector.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("powerdirector.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("powerdirector.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("powerdirector.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("powerdirector.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("powerdirector.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("powerdirector.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("powerdirector.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("powerdirector.webhook.processed");
    expect(spanNames).toContain("powerdirector.message.processed");
    expect(spanNames).toContain("powerdirector.session.stuck");
