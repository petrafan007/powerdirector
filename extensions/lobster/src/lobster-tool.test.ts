...
import type { PowerDirectorPluginApi, PowerDirectorPluginToolContext } from "powerdirector/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<PowerDirectorPluginApi> = {}): PowerDirectorPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<PowerDirectorPluginToolContext> = {}): PowerDirectorPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: PowerDirectorPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
