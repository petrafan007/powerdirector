...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("powerdirector-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("powerdirector-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("powerdirector-openshell-remote-agent-");
