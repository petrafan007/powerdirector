...
vi.mock("powerdirector/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("powerdirector/plugin-sdk/runtime-env")>();
  const logger = {
    child: () => logger,
    info: vi.fn(),
    warn: vi.fn(),
...
vi.mock("../auth-store.js", () => ({
  WA_WEB_AUTH_DIR: "/tmp/powerdirector-oauth/whatsapp/default",
  resolveDefaultWebAuthDir: () => "/tmp/powerdirector-oauth/whatsapp/default",
