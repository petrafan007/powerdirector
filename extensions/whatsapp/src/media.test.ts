...
import { resolvePreferredPowerDirectorTmpDir } from "../../../src/infra/tmp-powerdirector-dir.js";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredPowerDirectorTmpDir(), "powerdirector-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub POWERDIRECTOR_STATE_DIR.
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredPowerDirectorTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default PowerDirector state workspace and sandbox roots", async () => {
  ...
  it("rejects default PowerDirector state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
