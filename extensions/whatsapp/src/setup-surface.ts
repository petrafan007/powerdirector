...
function mergeWhatsAppConfig(
  ...
  patch: Partial<NonNullable<NonNullable<PowerDirectorConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: PowerDirectorConfig, dmPolicy: DmPolicy): PowerDirectorConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: PowerDirectorConfig, allowFrom?: string[]): PowerDirectorConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: PowerDirectorConfig, selfChatMode: boolean): PowerDirectorConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: PowerDirectorConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so PowerDirector can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for PowerDirector" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("powerdirector channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
