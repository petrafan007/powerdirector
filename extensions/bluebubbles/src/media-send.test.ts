...
import type { PowerDirectorConfig, PluginRuntime } from "powerdirector/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): PowerDirectorConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
