...
function resolvePowerDirectorRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.POWERDIRECTOR_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["powerdirector"]) {
  ...
  throw new Error("Unable to resolve core root. Set POWERDIRECTOR_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolvePowerDirectorRoot(), "dist", "extensionAPI.js");
