...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-powerdirector-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-powerdirector-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
function buildHtmlDocument(params: {
  ...
  return `<!doctype html>
<html lang="en"${params.runtimeMode === "image" ? ' data-powerdirector-diffs-ready="true"' : ""}>
  <head>
    <meta charset="utf-8" />
  ...
  <body data-theme="${params.theme}">
    <main class="oc-frame" data-render-mode="${params.runtimeMode}">
      <div data-powerdirector-diff-root>
        ${params.bodyHtml}
      </div>
    </main>
