...
import type { PowerDirectorPluginApi, PowerDirectorPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<PowerDirectorPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: PowerDirectorPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<PowerDirectorPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as PowerDirectorPluginApi);
