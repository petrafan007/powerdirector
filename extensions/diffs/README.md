...
Read-only diff viewer plugin for **PowerDirector** agents.
...
Set plugin-wide defaults in `~/.powerdirector/powerdirector.config.json`:
...
Before:
PowerDirector supports plugins.

After:
PowerDirector supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/powerdirector-diffs`).
