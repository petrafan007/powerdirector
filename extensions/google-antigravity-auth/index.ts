...
const CLIENT_ID = (
  process.env.POWERDIRECTOR_GOOGLE_ANTIGRAVITY_CLIENT_ID ??
  process.env.OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_ID ??
  ""
).trim();
const CLIENT_SECRET = (
  process.env.POWERDIRECTOR_GOOGLE_ANTIGRAVITY_CLIENT_SECRET ??
  process.env.OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_SECRET ??
  ""
).trim();
...
function assertOauthClientConfig() {
  if (CLIENT_ID && CLIENT_SECRET) {
    return;
  }
  throw new Error(
    [
      "Google Antigravity OAuth is not configured.",
      "Set POWERDIRECTOR_GOOGLE_ANTIGRAVITY_CLIENT_ID and POWERDIRECTOR_GOOGLE_ANTIGRAVITY_CLIENT_SECRET.",
    ].join(" "),
  );
}
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>PowerDirector Antigravity OAuth</title>
  </head>
  <body>
    <main>
