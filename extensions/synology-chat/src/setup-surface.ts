...
function getChannelConfig(cfg: PowerDirectorConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: PowerDirectorConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: PowerDirectorConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: PowerDirectorConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL PowerDirector uses to send replies back to Chat.",
      ],
