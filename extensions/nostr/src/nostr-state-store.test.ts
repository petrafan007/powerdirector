...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powerdirector-nostr-"));
  process.env.POWERDIRECTOR_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.POWERDIRECTOR_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".powerdirector");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.POWERDIRECTOR_STATE_DIR = previous;
    }
